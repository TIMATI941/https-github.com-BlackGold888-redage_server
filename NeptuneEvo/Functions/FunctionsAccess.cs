    ...
    class FunctionsAccess : Script
    {
        ...
        public void CMD_Refreshclothes(ExtPlayer player)
        {
            try
            {
                ...
                UpdateSystemState();
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Aktualisiert .", 3000);
            }
            catch (Exception e)
            {
        ...
        public void CMD_Enablefunc(ExtPlayer player, string name, bool toogled)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben die Funktion {name} auf {toogled} geändert", 10000);
            }
            catch (Exception e)
            {
