...
namespace NeptuneEvo.Inventory.Tent
{
    class Repository
    {
        ...
        public static void OnResourceStart()
        {
            ...
            Main.CreateBlip(new Main.BlipData(628, LangFunc.GetText(LangType.De, DataName.GlavRinok), TentList.PositionGps[0], 36, true));
            CustomColShape.CreateCylinderColShape(TentList.PositionGps[0], 90, 30, 0, colShapeEnums: ColShapeEnums.SafeZoneTent);

            Main.CreateBlip(new Main.BlipData(484, LangFunc.GetText(LangType.De, DataName.CherRinok), TentList.PositionGps[1], 54, true));
        ...
        public static void OpenMenuTent(ExtPlayer player, bool isPhone = false)
        {
            try
            {
                ...
                frameList.Header = LangFunc.GetText(LangType.De, DataName.Palatka);
                frameList.Callback = callback_menutent;
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.RentTentDo, tentData.RentTime), "info"));
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.RentTentExp, MoneySystem.Wallet.Format(TentPrice)), "rentTime"));
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.CancelRentTent), "remove"));
                

                if (!isPhone)
                {
                    frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.Naming), "name"));
                    frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.Inventory), "inventory"));
                }
            ...
            try
            {
                ...
                switch (listItem)
                {
                    ...
                    case "rentTime":
                        ...
                        EventSys.SendCoolMsg(player,"Markt", "Zelt mieten", $"{LangFunc.GetText(LangType.De, DataName.RentTentExpand)}", "", 7000);
                            //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RentTentExpand), 6000);
                        BattlePass.Repository.UpdateReward(player, 49);

                        tentData.RentTime = tentData.RentTime.AddHours(1);                            
                        break;
                    case "name":
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.De, DataName.TentName), LangFunc.GetText(LangType.De, DataName.NameTentInput), 35, "rentname");
                        break;
                }
        ...
        public static void UpdateTentLabel(int index, string text = "")
        {
            ...
            if (player.IsCharacterData()) tentData.label.Text = LangFunc.GetText(LangType.De, DataName.SellerTent, text, player.Name);
        ...
        public static void OpenRentTent(ExtPlayer player, int index)
        {
            try
            {
                ...
                var frameList = new FrameListData(); 
                frameList.Header = LangFunc.GetText(LangType.De, DataName.Tent); 
                frameList.Callback = callback_renttent;
                
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.ArendaTelephone, MoneySystem.Wallet.Format(TentPrice)), index)); // почему-то не уверен
        ...
        private static void callback_renttent(ExtPlayer player, object listItem) /// Никитос Чини 
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyTentYou), 6000);
                    return;
                }
                else if (tentData.player != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyTent), 6000);
                    return;
                }
                ...
                //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouRentedTent), 10000);
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Tent, LangFunc.GetText(LangType.De, DataName.YouRentedTent), DateTime.Now);  
        ...
        public static void RemoveTent(int index, string type)
        {
            try
            {
                ...
                {
                    case "time":
                        //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TentRentGo), 10000);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Tent, LangFunc.GetText(LangType.De, DataName.TentRentGo), DateTime.Now);
                        break;
                    case "remove":
                        //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TentRentCancel), 10000);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Tent, LangFunc.GetText(LangType.De, DataName.TentRentCancel), DateTime.Now);
                        break;
                    /*case "SafeZoneTent":
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TentZoneCancel), 10000);
                        break;*/
                }
