...
namespace NeptuneEvo.Quests
{
    class Wedding : Script
    {
        ...
        {
            Main.CreateBlip(new Main.BlipData(305, "Kirche", new Vector3(-759.22766, -709.2103, 30.061632), 59, true));
            PedSystem.Repository.CreateQuest("cs_priest", new Vector3(-786.86f, -708.91, 30.322592), -94f, title: "~y~NPC~w~ Pfarrer Louis\nMachmut", colShapeEnums: ColShapeEnums.QuestWedding);
        }
            ...
            switch (type)
            {
                case 0:
                    if (typeSurname == 0 || typeSurname == 2) 
                        Trigger.ClientEvent(player, "openDialog", "Wedding", LangFunc.GetText(LangType.De, DataName.YouWantWedding, target.Name, MoneySystem.Wallet.Format(price)));
                    else 
                        Trigger.ClientEvent(player, "openDialog", "Wedding", LangFunc.GetText(LangType.De, DataName.YouWantWeddingFamily, target.Name, MoneySystem.Wallet.Format(price), target.Name.Split("_")[1]));
                    break;
                case 1:
                    Trigger.ClientEvent(player, "openDialog", "Wedding", LangFunc.GetText(LangType.De, DataName.YouDivorce, target.Name, MoneySystem.Wallet.Format(price)));
                    break;
                case 2:
                    Trigger.ClientEvent(player, "openDialog", "Wedding", LangFunc.GetText(LangType.De, DataName.YouDivorceSolo, MoneySystem.Wallet.Format(price)));
                    break;
            }
        ...
        public static void OnBoda(ExtPlayer player, ExtPlayer target, int typeSurname)
        {
            ...
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouTooFar), 6000);
                return;
            }
            
            if (typeSurname == 0 && Main.PlayerNames.Values.Contains($"{targetCharacterData.FirstName}_{characterData.LastName}"))
            {
                Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NameExists), 6000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WeddingSuc, target.Name), 10000);
            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WeddingSuc, player.Name), 10000);
            NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.SucWedding,Chars.Models.ChatColors.AMP, target.Name, player.Name));
        ...
        public static void OnDivorcio(ExtPlayer player, bool isOne = false)
        {
            ...
            if (targetCharacterData != null)
            {
                ...
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YourDivorced, player.Name), 10000);
            }
            else
            {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouDivorced, target.Name), 10000);
        }
    }
}
