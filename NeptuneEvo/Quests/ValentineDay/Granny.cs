        ...
        public void take_quest_item(ExtPlayer player, int index)
        {
            ...
            Timers.StartOnce(5000, () =>
            {
                try
                {
                    ...
                    if (index >= 2)
                    {
                        Trigger.SendChatMessage(player, "Sie haben alle Bären gesammelt! Bringen Sie diese zu Oma Granny");
                    }
                    else if (index == 1)
                    {
                        Trigger.SendChatMessage(player, "Sie haben 1/2 Bären gesammelt. Der letzte Bär blieb übrig. Der ungefähre Standort des Bären wurde auf der Karte markiert!");
                    }
