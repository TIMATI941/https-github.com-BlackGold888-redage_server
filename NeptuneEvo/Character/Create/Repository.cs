    ...
    public class Repository
    {
        ...
        public static async Task<int> Create(ExtPlayer player, string firstName, string lastName)
        {
            try
            {
                ...
                {
                    Trigger.ClientEvent(player, "client.characters.create.error", LangFunc.GetText(LangType.De, DataName.ErrorLenghtName));
                    return -1;
                }
                if (Main.PlayerNames.Values.Contains($"{firstName}_{lastName}"))
                {
                    Trigger.ClientEvent(player, "client.characters.create.error", LangFunc.GetText(LangType.De, DataName.NameUsed), 6000);
                    return -1;
                }
                ...
                NAPI.Task.Run(() =>
                {
                    try
                    {
                        ...
                        GameLog.AccountLog(accountData.Login, accountData.HWID, accountData.IP, accountData.SocialClub, $"Charakter erstellt {characterData.FirstName} {characterData.LastName}");
