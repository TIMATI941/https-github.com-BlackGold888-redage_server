    ...
    class Repository
    {
        ...
        public static void Handshake(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyHi), 5000);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyHi), 5000);
                    return;
                }
                ...
                if (!characterData.Friends.ContainsKey(secondName) || !targetCharacterData.Friends.ContainsKey(firstName))
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HiSecond, secondName.Split('_')[0]), 5000);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HiFirst, firstName.Split('_')[0]), 5000);
