    ...
    class Repository
    {
        ...
        public static async Task IsDeleteCharacter(ExtPlayer player, int slot)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DeleteError), 6000);
                        return;
                    }
        ...
        public static async Task CancelDeleteCharacter(ServerBD db, ExtPlayer player, int slot)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DeleteCancel), 10000);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                NAPI.Task.Run(() =>
                {
                    foreach (var charData in chars)
                    {
                        if (!bansUUID.Contains(charData.Uuid))
                        {
                            BusinessManager.changeOwner($"{charData.Firstname}_{charData.Lastname}", "Staat");
                            ...
                            if (target != null)
                            {
                                ...
                                if (targetAccountData != null)
                                {
                                    ...
                                    GameLog.AccountLog(targetAccountData.Login, targetAccountData.HWID, targetAccountData.IP, targetAccountData.SocialClub, $"Charakter gelöscht {charData.Firstname}_{charData.Lastname}");
                                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CharDeleted, charData.Firstname, charData.Lastname), 6000);
                                }
                            }
