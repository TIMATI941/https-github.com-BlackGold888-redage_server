        ...
        public static async Task Load(ExtPlayer player, int uuid, int spawnid)
        {
            try
            {
                ...
                if (GameLog.OnlineQueue.ContainsKey(uuid))
                {
                    ...
                    GameLog.Disconnected(uuid, player.Value, "Entfernen der alten Verbindung (Fehler)", accountData.Login);       
                    //return;
                }
                ...
                if (ban)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Error", 4000);
                    return;
                }
                ...
                if (character != null)
                {
                    ...
                    if (characterData.AdminLVL >= 1 && characterData.AdminLVL <= 5)
                        Trigger.SendToAdmins(1, $"!{{#FFB833}}[A] {fullname} Registriert ({characterData.AdminLVL} lvl)");
