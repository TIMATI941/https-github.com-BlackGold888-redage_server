        ...
        public void CMD_Refreshclothes(ExtPlayer player, string name)
        {
            try
            {
                ...
                switch (name)
                {
                    case "clothes":
                        OnResourceStart();
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Der Preis für Kleidung wurde aktualisiert", 3000);
                        break;
                    ...
                    case "prod":
                        ...
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Refreshprod), 6000);
                        break;
                    case "ores":
                        Jobs.Miner.UpdateOres();
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Refreshore), 6000);
                        break;
                    case "settings":
                    case "st":
                        Main.LoadServerSettings();
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Eistellungen wurden aktualisiert", 3000);
                        break;
                    case "roulette":
                        Repository.InitRoulette();
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Roulette wurde aktualisiert", 3000);
