    ...
    class UpdateData : Script
    {
        ...
        public static byte CanIChange(ExtPlayer player, int value, bool errortext = false)
        {
            try
            {
                ...
                {
                    if (errortext) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ServRestarting), 6000);
                    return 2;
                }
                if (0 > value)
                    return 3;
                ...
                {
                    if (errortext) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoMoneyForIt, MoneySystem.Wallet.Format(value - characterData.Money)), 5000);
                    return 0;
                }
        ...
        public static void Exp(ExtPlayer player, int value, int salary = 0)
        {
            try
            {
                ...
                if (characterData.EXP >= 3 + characterData.LVL * 3)
                {
                    ...
                    Level(player, 1);
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NewLvl, characterData.LVL), 15000);
