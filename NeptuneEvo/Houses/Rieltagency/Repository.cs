...
namespace NeptuneEvo.Houses.Rieltagency
{
    public class Repository
    {
        ...
        {
            Main.CreateBlip(new Main.BlipData(374, LangFunc.GetText(LangType.De, DataName.Rieltorskoye), new Vector3(-250.94254, -915.1024, 32.313484), 2, true));
            PedSystem.Repository.CreateQuest("a_m_y_business_02", new Vector3(-250.94254, -915.1024, 32.313484), 64.50f, title: "~y~NPC~w~ Sandra Poco\nMakler", colShapeEnums: ColShapeEnums.Rieltagency);
        }
        ...
        public static void OnBuy(ExtPlayer player, int id, int type)
        {
            ...
            if (type == 0)
            {
                ...
                if (house == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NotFindHome), 6000);
                    return;
                }
                
                if (house.Owner != String.Empty)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HomeAlreadyBought), 6000);
                    return;
                }
                ...
                Trigger.ClientEvent(player, "client.rieltagency.addBlip", 374, 82, id, house.Position.X, house.Position.Y, LangFunc.GetText(LangType.De, DataName.Rieltorskoye));
            }
            else
            {
                if (!BusinessManager.BizList.ContainsKey(id))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NotFindBiz), 6000);
                    return;
                }
                ...
                if (business.IsOwner())
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BizAlreadyBought), 6000);
                    return;
                }
                ...
                Trigger.ClientEvent(player, "client.rieltagency.addBlip", 464, 82, id, business.EnterPoint.X, business.EnterPoint.Y, LangFunc.GetText(LangType.De, DataName.Rieltorskoye));
                
            }
            EventSys.SendCoolMsg(player,"Makler", "Informationskauf", $"{LangFunc.GetText(LangType.De, DataName.YouBoughtInfo)}", "", 15000);
           // Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouBoughtInfo), 10000);
