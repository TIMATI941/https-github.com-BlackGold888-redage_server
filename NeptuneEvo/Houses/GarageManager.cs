    ...
    class GarageManager : Script
    {
        ...
        public static void OnEnterGarage(ExtPlayer player, int index)
        {
            ...
            if (player.IsInVehicle)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantEnterGarageVeh), 6000);
                    return;
                }
                var vehicleLocalData = vehicle.GetVehicleLocalData();
                if (vehicleLocalData == null)
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarDestroyed), 6000);
                    return;
                }
            ...
            if (garage.Type == -1 || garage.Type == 6)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHaveNoCars), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarInState), 6000);
                    return;
                }
        ...
        public static void CMD_SetGarage(ExtPlayer player, int ID)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie müssen auf einer Grundstücksmarkierung stehen", 3000);
                    return;
                }
        ...
        public static void CMD_CreateGarage(ExtPlayer player, int type)
        {
            try
            {
                ...
                if (!player.IsInVehicle)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie müssen sich im Fahrzeug befinden", 3000);
                    return;
                }
                int id = 0;
                do
        ...
        public static void CMD_TpHouse(ExtPlayer player, int id)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) teleportiert sich zum Haus {id}");
        ...
        public static void CMD_tpBiz(ExtPlayer player, int id)
        {
            try
            {
                ...
                Trigger.Dimension(player, 0);
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) teleportiert sich zum Business {id}");
        ...
        public static void CMD_RemoveGarage(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie müssen auf der Garagenmarkierung stehen", 3000);
                    return;
                }
        ...
        {           
            ...
            if (house == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GarageError), 6000);
                return;
            }
            ...
            if (garage.CarSlots.ContainsKey(index))
            {
                ...
                if ((vehicle1 != null && vehicleLocalData1.Access != VehicleAccess.Garage) || (vehicle2 != null && vehicleLocalData2.Access != VehicleAccess.Garage))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TwoCarsInGarage), 6000);
                    Trigger.ClientEvent(player, "client.phone.cars.error");
                    return;
                }
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarMustInGarage), 6000);
                    Trigger.ClientEvent(player, "client.phone.cars.error");
                    return;
                }
