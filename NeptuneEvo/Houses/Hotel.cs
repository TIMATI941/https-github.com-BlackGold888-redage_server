...
namespace NeptuneEvo.Houses
{
    class Hotel : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                foreach (Vector3 pos in HotelEnters)
                {
                    ...
                    Main.CreateBlip(new Main.BlipData(475, LangFunc.GetText(LangType.De, DataName.Hotel), pos, 16, true));
                ...
                foreach (Vector3 pos in CarsGet)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pos - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1f, new Color(255, 255, 255, 220));
                    NAPI.TextLabel.CreateTextLabel("~w~Hotelroller", pos + new Vector3(0, 0, 0.5), 5f, 0.4f, 0, new Color(255, 255, 255));
                    HotelID++;
                }
            }
            catch (Exception e)
        ...
        public static void OnEnterHotel(ExtPlayer player, int Index)
        {
            ...
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben bereits ein Hotelzimmer gemietet", 3000);
        }
        ...
        public static void OnCarRentHotel(ExtPlayer player, int Index)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Die Nutzung im Fahrzeug ist nicht möglich.", 3000);
                return;
            }
            if (Index == characterData.HotelID)
            {
                if (sessionData.HotelData.Car != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Roller wurde zurückgebracht", 3000);
        ...
        public static void ExtendHotelRent(ExtPlayer player, int hours)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie sind in keinem Hotel untergebracht", 3000);
                    return;
                }

                if (characterData.HotelLeft + hours > 10)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Die Miete kann nur für 10 Stunden bezahlt werden", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Die haben die Mietdauer verlängert auf {hours} Stunden. Mietdauert endet in {characterData.HotelLeft} Stunden", 10000);
            }
            catch (Exception e)
            {
        ...
        public static void OpenHotelBuyMenu(ExtPlayer player)
        {
            try
            {
                ...
                frameList.Header = LangFunc.GetText(LangType.De, DataName.Hotel);
                frameList.Callback = callback_hotelbuy;
                
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.HotelPayday), "info"));
                
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.HotelRent, Main.HotelRent), "rent"));
            ...
            try
            {
                ...
                switch (listItem)
                {
                    case "rent":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie wohnen in einem Haus und können kein Hotelzimmer mieten", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sie haben für 1 Stunde ein Hotelzimmer gemietet. Sie können den Mietvertrag im Telefon (M) verlängern.", 3000);
        ...
        public static void OpenHotelManageMenu(ExtPlayer player)
        {
            try
            {
                ...
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.HotelPayday), "info"));
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.ExpandRent), "extend"));
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.Viselitsya), "moveout"));
                Players.Popup.List.Repository.Open(player, frameList); 
            }
            catch (Exception e)
            {
        ...
        private static void callback_hotelmanage(ExtPlayer player, object listItem)
        {
            try
            {
                ...
                {
                    case "extend":
                        Trigger.ClientEvent(player, "openInput", $"Продлить аренду ({Main.HotelRent}$/h)", "Stunden anzahl eingeben", 1, "extend_hotel_rent");
                        return;
                    case "moveout":
                        MoveOutPlayer(player);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Sie haben aus dem Hotel ausgecheckt", 3000);
                        return;
                }
            }
            catch (Exception e)
