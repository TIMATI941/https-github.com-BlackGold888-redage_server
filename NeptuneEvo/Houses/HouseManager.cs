...
namespace NeptuneEvo.Houses
{
    ...
    public class House
    {
        ...
        public House(int id, string owner, int type, Vector3 position, int price, bool locked, int garageID, int bank, Dictionary<string, ResidentData> roommates, int dimensionID, bool healkit, bool alarm)
        {
            ...
            label = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"Immobilie {id}"), position + new Vector3(0, 0, 1.5), 5f, 0.4f, 0, new Color(255, 255, 255), false, 0);
            UpdateLabel();
            UpdateColShapeHealkit();
            UpdateGarage();

            if (Main.ServerSettings.IsHouseBlips)
                blip = (ExtBlip)NAPI.Blip.CreateBlip(374, position, 0.45f, 82, "Immobilie", shortRange: true);
        }
            ...
            try
            {
                if (Type != 7)
                {
                    if (Healkit)
                    {
                        ...
                        Healkitlabel = (ExtTextLabel)NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Verbandskasten"), HouseManager.HouseHealkitPos[Type - 1], 5f, 0.3f, 0, new Color(255, 255, 255), false, (uint)Dimension);
        ...
        public void UpdateLabel()
        {
            Trigger.SetMainTask(() =>
            {
                try
                {
                    ...
                    else text += $"~y~Parkplatz\n";
                    if (!string.IsNullOrEmpty(Owner))
                    {
                        text += $"~p~{Owner}\n";
                        if (GarageID != 0 && GarageManager.Garages.ContainsKey(GarageID))
                            text += $"~w~Garage: ~o~{GarageManager.GarageTypes[GarageManager.Garages[GarageID].Type].MaxCars} Fahrzeug\n";
                        if (Type != 7) text += (Locked) ? "~r~Geschlossen~c~\n" : "~g~Offen~c~\n";
                    }
                    else if (IsAuction)
                    {
                        text += $"~w~Auktion erstellt";
                        if (GarageID != 0 && GarageManager.Garages.ContainsKey(GarageID))
                            text += $"~w~Garage: ~o~{GarageManager.GarageTypes[GarageManager.Garages[GarageID].Type].MaxCars} Fahrzeug\n";
                    }
                    else
                    {
                        text += $"~w~Preis: ~g~{MoneySystem.Wallet.Format(Price)}$\n";
                        if (GarageID != 0 && GarageManager.Garages.ContainsKey(GarageID))
                            text += $"~w~Garage: ~o~{GarageManager.GarageTypes[GarageManager.Garages[GarageID].Type].MaxCars} Fahrzeug\n";
                    }
                    text += $"~c~ID{ID}";
        ...
        public void CreateFurniture(HouseFurniture f)
        {
            try
            {
                ...
                switch (f.Name)
                {
                    ...
                    case "Schrank":
                    case "Tresor":
                        if (f.Name == "Waffentresor") name = "WeaponSafe";
                        else if (f.Name == "Kleiderschrank") name = "ClothesSafe";
                        else if (f.Name == "Schrank") name = "SubjectSafe";
                        else if (f.Name == "Tresor") name = "BurglarProofSafe";
            ...
            Trigger.SetMainTask(() =>
            {
                try
                {
                    ...
                    foreach (var name in names)
                    {
                        ...
                        {
                            if (ownerName != name)
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Du wurdest rausgeschmissen", 3000);
    ...
    class HouseManager : Script
    {
        ...
        {
            // name, position, INTERIOR 
            new HouseType(LangFunc.GetText(LangType.De, DataName.Trailer), new Vector3(1973.056, 3816.448, 32.728), new Vector3(), 0.0f, "trevorstrailer"),
            new HouseType(LangFunc.GetText(LangType.De, DataName.Economy), new Vector3(151.0683, -1007.486, -99.70), new Vector3(), 0.0f,"hei_hw1_blimp_interior_v_motel_mp_milo_"),
            new HouseType(LangFunc.GetText(LangType.De, DataName.EcenomyPlus), new Vector3(265.9691, -1007.078, -102.0758), new Vector3(), 0.0f,"hei_hw1_blimp_interior_v_studio_lo_milo_"),
            new HouseType(LangFunc.GetText(LangType.De, DataName.Comfort), new Vector3(346.6991, -1013.023, -100.3162), new Vector3(349.5223, -994.5601, -99.7562), 264.0f, "hei_hw1_blimp_interior_v_apart_midspaz_milo_"),
            new HouseType(LangFunc.GetText(LangType.De, DataName.ComfortPlus), new Vector3(-31.35483, -594.9686, 78.9109),  new Vector3(-25.42115, -581.4933, 79.12776), 159.84f, "hei_hw1_blimp_interior_32_dlc_apart_high2_new_milo_"),
            new HouseType(LangFunc.GetText(LangType.De, DataName.Premium), new Vector3(-17.85757, -589.0983, 88.99482), new Vector3(-38.84652, -578.466, 88.58952), 50.8f, "hei_hw1_blimp_interior_10_dlc_apart_high_new_milo_"),
            new HouseType(LangFunc.GetText(LangType.De, DataName.PremiumPlus), new Vector3(-173.9419, 497.8622, 136.8341), new Vector3(-164.9799, 480.7568, 137.1526), 40.0f, "apa_ch2_05e_interior_0_v_mp_stilts_b_milo_"),
            new HouseType(LangFunc.GetText(LangType.De, DataName.Parking), new Vector3(), new Vector3(), 0.0f, ""),
            new HouseType(LangFunc.GetText(LangType.De, DataName.PremiumPlusPlus), new Vector3(373.53, 423.47, 145.05), new Vector3(373.5995, 404.09, 145.407), 234.0f, ""),
            new HouseType(LangFunc.GetText(LangType.De, DataName.Lux), new Vector3(-774.0425, 342.072, 195.7), new Vector3(-778.88, 320.96, 195.32), 255.0f, "apa_v_mp_h_05_b"),
        };
        ...
        public static void OnHealkitHouse(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHasNoHouse), 6000);
                return;
            }
            else if (house.HealkitTime > DateTime.Now)
            {
                ...
                Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HealthkitUseCan, left), 1500);
                return;
            }
            house.HealkitTime = DateTime.Now.AddHours(1);
            player.Health = 100;
            Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HealthkitUse), 6000);
        }
            ...
            else
            {
                if (house.Type != 7)
                {
                    if (house.Locked)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SomebodyYouFollow), 6000);
                            return;
                        }
                        if (sessionData.Follower != null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OtpustiteChela), 6000);
                            return;
                        }
                        ...
                        else
                        {
                            ...
                            if (characterData.AdminLVL < 5)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NedavnoGrabili), 6000);
                                    return;
                                }
                                if (sessionData.ActiveWeap.Item != null)
                                {
                                    ...
                                    if (itemData.ItemId == ItemId.Crowbar)
                                    {
                                                ...
                                                case FractionsType.Bikers: // Bikers
                                                    if (memberFractionData.Rank < 3)
                                                    {
                                                        Notify.Send(player, NotifyType.Error,
                                                            ...
                                                            LangFunc.GetText(LangType.De, DataName.HomeGrabit3Rank),
                                                            3000);
                                                        return;
                                                    }
                                        ...
                                        Commands.RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.HackingHome));
                                    }
                                }
                            }
                            else
        ...
        public static void OnExitHouse(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustFinishEdit), 6000);
                return;
            }
        ...
        public static void houseHijack(ExtPlayer player, params object[] arguments)
        {
            try
            {
                ...
                if (sessionData.ActiveWeap.Item != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustHaveLom), 2000);
                        HouseHijackStop(player);
                        return;
                    }
                }
                ...
                if (!(bool)arguments[0])
                {
                    ...
                    HouseHijackStop(player);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LomBroken), 2000);
                    return;
                }
                var house = Houses.FirstOrDefault(h => h.ID == sessionData.HouseID);
                if (house == null)
                ...
                if (stage == 4)
                {
                    ...
                    Trigger.ClientEvent(player, "fullblockMove", false);
                    Trigger.ClientEvent(player, "freeze", false);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucVzlomDver), 2000);
                ...
                else
                {
                    ...
                    if (stage <= 4)
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Stage5Suc, stage), 2000);
        ...
        private static async void OnHouseInfoAction(ExtPlayer player, string action)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                switch (action)
                {
                    case "buy":
                        ...
                        if (!string.IsNullOrEmpty(house.Owner))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Diese Immobillie hat bereits einen Eigentümer", 3000);
                            return;
                        }
                        if (house.IsAuction)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie können die Immobillie nicht betreten weil es versteigert wird.", 7000);
                            return;
                        }
                        if (Players.Phone.Auction.Repository.IsBet(characterData.UUID, AuctionType.House))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du kannst dir keine Immobillie kaufen wenn deine gerade in der Auktion ist.", 6000);
                            return;
                        }
                        if (house.Price == 0 && characterData.AdminLVL <= 5)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Das Haus ist nicht käuflich zu erwerben.", 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du kannst nicht mehr als 1 Immobillie kaufen", 3000);
                            return;
                        }
                        ...
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.BuyHouse, house.Price), DateTime.Now);
                        ...
                        var houseBalances = MoneySystem.Bank.GetBalance(house.BankID);
                        if(houseBalances == 0){
                            int bankId = await MoneySystem.Bank.Create(string.Empty, 2, 0);
                            house.BankID = bankId;
                            Console.WriteLine("create new bankaccount for house "+house.ID+ " bank " + bankId);
                        }
                    ...
                    case "int":
                        ...
                        else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Dieser Parkplatz hat kein Interior", 3000);
                        return;
                }
            }
            catch (Exception e)
        ...
        private void OnHouseAction(ExtPlayer player, string action)
        {
            try
            {
                ...
                if (!action.Equals("openhouse") && !action.Equals("leavehouse"))
                {
                    ...
                    if (house == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du hast keine Immobilie", 3000);
                        return;
                    }
                }
                else
                {
                    ...
                    if (house == null)
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du wohnst in keinem Haus", 3000);
                        return;
                    }
                }
                switch (action)
                {
                    ...
                    case "removeall":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Für diese Aktion musst du dich im Haus befinden", 3000);
                            return;
                        }
                        if (house.Removedall > DateTime.Now)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du hast kürzlich Spieler aus dem Haus rausgeschmissen. Du kannst es erst wieder in 5 Minuten benutzen.", 7000);
                            return;
                        }
                        house.Removedall = DateTime.Now.AddMinutes(5);
                        house.RemoveAllPlayers(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast alle aus dem Haus rausgeschmissen", 3000);
                        return;
                    case "sell":
                        int price = 0;
                        ...
                        Trigger.ClientEvent(player, "openDialog", "HOUSE_SELL_TOGOV", $"Möchtest du wirklich deine Immobillie für ${MoneySystem.Wallet.Format(price)} verkaufen?");
                        return;
                }
            }
            catch (Exception e)
        ...
        public static void acceptHouseSellToGov(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du musst die Garage verlassen", 3000);
                    return;
                }
                ...
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.SellDomGos, MoneySystem.Wallet.Format(price)), DateTime.Now);
        ...
        private void GarageUpdateLvl(ExtPlayer player)
        {
            try
            {
                ...
                if (garage != null && garage.Type != 6)
                {
                    if (garage.Type == 9)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ihr Haus hat die größte Anzahl von Garagenplätzen.", 3000);
                        return;
                    }
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Nicht genug Rockford-Coins.", 3000);
                            return;
                        }
                        UpdateData.RedBucks(player, -garageType.Price, msg: "Garagen-Upgrade");
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben Ihre Garage erfolgreich auf {garageType.MaxCars} Plätze erweitert.", 5000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Wenn Sie ein Haus verkaufen, werden alle Garagen-Upgrades zurückgesetzt!", 9000);
                }
        ...
        private void OnHouseFurnitureBuy(ExtPlayer player, string name)
        {
            try
            {
                ...
                if (name == "Erste-Hilfe-Set" || name == "Alarmanlage")
                {
                    if (characterData.InsideHouseID == -1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Für diese Aktion musst du dich im Haus befinden", 3000);
                    ...
                    if (name == "Erste-Hilfe-Set")
                    {
                        if (house.Type == 0 || house.Type == 7) return;
                        if (house.Healkit)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben dieses Upgrade bereits erworben.", 3000);
                            return;
                        }
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", 2500, $"buyFurn({house.ID} | Erste-Hilfe-Set)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Herzlichen Glückwunsch zum kauf Ihres Erste-Hilfe-Sets für zu Hause!", 3000);
                    }
                    else if (name == "Alarmanlage")
                    {
                        if (house.Type == 0 || house.Type == 7) return;
                        if (house.Alarm)
                        {
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", 3000, $"buyFurn({house.ID} | Alarmanlage)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Herzlichen Glückwunsch zu Ihrer neuen Alarmanlage!", 3000);
                    }
                    return;
                }
        ...
        private void OnFurnitureBuy(ExtPlayer player, string name, int type = 0)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Für diese Aktion musst du dich im Haus oder einem Geschäft befinden", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Maximale Anzahl von Möbel erreicht! Verkaufe etwas", 3000);
                    return;
                }
                ...
                {
                    if (name == "Einbruchsicheren Tresor" && houseFurniture.Values.Any(x => x.Name == name))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "In einem Haus darf es nur einen einbruchsicheren Tresor geben", 3000);
                        return;
                    }
                    ...
                    if (type == 0)
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {name} gekauft", 3000);
                    }
                    else
                    {
                        foreach (var itemData in furnitureData.Value.Items)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast nicht genug {Chars.Repository.ItemsInfo[itemData.Key].Name} {itemData.Value - count} Stk.", 5000);
                                return;
                            }
                        }
                        foreach (var itemData in furnitureData.Value.Items)
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {name} hergestellt", 3000);
                    }
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Bei der Auswahl von Möbel ist ein fehler aufgetreten", 3000);
        ...
        private void OnHouseFurnitureSell(ExtPlayer player, int id, int type)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du hast keine Möbel", 3000);
                    return;
                }

                if (!houseFurniture.ContainsKey(id))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Fehler bei der Möbelauswahl! Bitte schreibe ein Ticket /report", 6000);
                    return;
                }
                ...
                switch (type)
                {
                    case 0:
                        if (furniture.IsSet)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Entferne die Möbel vor dem Verkauf", 3000);
                            return;
                        }
                        ...
                        if (FurnitureManager.NameModels.ContainsKey(furniture.Name))
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {furniture.Name} verkauft für {MoneySystem.Wallet.Format(price)}$", 3000);
                            MoneySystem.Wallet.Change(player, price);
                    ...
                    case 1:
                        if (furniture.IsSet)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du kannst die Möbel nicht entfernen wenn sich noch Gegenstände drin befinden", 3000);
                                return;
                            }
                        ...
                        else
                        {
                            if (sessionData.HouseData.Editing)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie müssen die Bearbeitung abschließen", 3000);
                                return;
                            }
        ...
        private void OnResidentDell(ExtPlayer player, string name)
        {
            try
            {
                ...
                if (house.Roommates.ContainsKey(name))
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {name} aus dem Haus rausgeschmissen", 3000);
        ...
        public void VehicleTuning(ExtPlayer player, ExtVehicle vehicle)
        {
            ...
            if (vehicleData == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Mit diesem Fahrzeug kannst du nicht in die Garage einfahren", 3000);
                return;
            }
            ...
            try
            {
                ...
                switch (action)
                {
                    case "tune":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du kannst das Fahrzeug neben den NPC reparieren", 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du musst dich neben einem Parkplatz befinden", 3000);
                            return;
                        }
                    ...
                    case "spawn":
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Dein Hubschrauber erwartet dich auf der Landebahn", 3000);
                        return;
                    case "sell":
                        if (vehicleData.Holder != sessionData.Name)
                        {
                        ...
                        Trigger.ClientEvent(player, "openDialog", "CAR_SELL_TOGOV", $"Möchtest du wirklich {vehicleData.Model} ({number}) an den Staat für ${MoneySystem.Wallet.Format(price)} verkaufen?");
                        return;
                    case "repair":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"{vehicleData.Model} ({number}) wurde abgeschleppt", 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"{vehicleData.Model} ({number}) benötigt keine Reperatur", 3000);
                            return;
                        }
                        ...
                        vehicleData.Health = 1000;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {vehicleData.Model} ({number}) repariert", 3000);
                        return;
                    case "evac":

                        if (Ticket.IsVehicleTickets(vehicleData.SqlId))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"{vehicleData.Model} ({number}) wurde abgeschleppt", 5000);
                            return;
                        }
                        if (characterData.Money < Main.EvacCar)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Nicht genug Geld! (Es fehlen {Main.EvacCar - characterData.Money}$)", 3000);
                            return;
                        }
                        ...
                        else
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du musst dich neben der Garage befinden", 3000);
                                return;
                            }

                            if (garage.IsGarageToNumber(vehicleData.SqlId))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Dieses Fahrzeug steht in der Garage", 3000);
                                return;
                            }
                        }
                        vehiclePlayer = VehicleData.LocalData.Repository.GetVehicleToNumber(VehicleAccess.Personal, number);
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du kannst das Fahrzeug nicht parken", 3000);
                            return;
                        }
                        if (isAir)
                        {
                            if (VehicleModel.AirAutoRoom.IsVehicleToSpawn(vehiclePlayer.Position))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Fahrzeug steht bereits auf einem Parkplatz", 3000);
                                return;
                            }
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du kannst das Fahrzeug nicht parken, wenn jemand anderes sich im Fahrzeug befindet", 7000);
                            return;
                        }
                        ...
                        if (isAir)
                        {
                            ...
                            vehiclePlayer = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(vehicleData.Model), positionData.Item1, positionData.Item2.Z, 0, 0, number, petrol: vehicleData.Fuel, acc: VehicleAccess.Personal, locked: true, dirt: vehicleData.Dirt);
                            VehicleManager.ApplyCustomization(vehiclePlayer);
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Ihr Hubschrauber wurde eingeparkt", 3000);
                        }
                        else
                        {
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Dein Fahrzeug wurde in die Garage gefahren", 3000);
                        }
                    ...
                    case "gps":
                        if (!isAir)
                        {
                            ...
                            if (garage.IsGarageToNumber(vehicleData.SqlId))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    "Dieses Fahrzeug steht in der Garage", 3000);
                                return;
                            }
                        }
                        ...
                        if (Ticket.IsVehicleTickets(vehicleData.SqlId))
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                "Ihr Auto wurde beschlagnahmt. Es wurde auf der Karte markiert.", 3000);
                        }
                        else
                        {
                            vehiclePlayer = VehicleData.LocalData.Repository.GetVehicleToNumber(VehicleAccess.Personal, number);
                            if (vehiclePlayer == null)
                            {
                                if (isAir)
                                {
                                    ...
                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                        "Dein Fahrzeug wurde im GPS markiert", 3000);
                                }
                                return;
                            }
                    ...
                    case "key":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Nicht genug Platz im Inventar", 3000);
                            return;
                        }
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast ein Schlüssel für das Fahrzeug {number} bekommen", 3000);
                    ...
                    case "changekey":
                        ...
                        if (!isAir)
                        {
                            if (garage?.Type == -1)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        $"Du musst dich neben der Garage befinden", 3000);
                                    return;
                                }
                            }
                            else if (garage?.Type == 6)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        $"Du musst dich neben einem Parkplatz befinden", 3000);
                                    return;
                                }
                            }
                            else
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        LangFunc.GetText(LangType.De, DataName.MustBeInGarage), 6000);
                                    return;
                                }
                            }
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Schloss austauschen kostet $100", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast das Schloss von {number} getauscht. Sie können die alten Schlüssel nicht mehr verwenden", 7000);
                        return;
                }
            }
            catch (Exception e)
        ...
        public static void InviteToRoom(ExtPlayer player, ExtPlayer guest)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Zurzeit nicht verfügbar", 3000);
                    return;
                }
                if (characterData.AdminLVL != 9 && house.Roommates.Count >= MaxRoommates[house.Type])
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Bei dir im Haus wohnen schon die maximale Anzahl an Spielern", 6000);
                    return;
                }
                if (GetHouse(guest) != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Spieler wohnt bereits in einem Haus", 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(guest, "openDialog", "ROOM_INVITE", $"({player.Value}) hat dir angeboten zu Ihm einziehen");

                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du hast ({guest.Value}) angeboten zu dir einzuziehen", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void acceptRoomInvite(ExtPlayer player)
        {
            try
            {
                ...
                if (house == null || house.Type == 7)
                {
                    ...
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Dieser Spieler hat keine eigene Immobilie", 3000);
                    return;
                }
                if (ownerCharacterData.AdminLVL != 9 && house.Roommates.Count >= MaxRoommates[house.Type])
                {
                    ownerSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In dieses Haus können keine weiteren Spieler mehr einziehen", 3000);
                    return;
                }
                if (house.Roommates.ContainsKey(player.Name))
                {
                    ownerSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du bist bereits in dieses Haus eingezogen", 3000);
                    return;
                }
                ...
                Notify.Send(owner, NotifyType.Info, NotifyPosition.BottomCenter, $"({player.Value}) ist zu dir eingezogen", 3000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du bist zum ({owner.Value}) gezogen", 3000);
        ...
        public static async void CMD_CreateHouse(ExtPlayer player, int type, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Falscher Typ", 3000);
                    return;
                }
                int bankId = await MoneySystem.Bank.Create(string.Empty, 2, 0);
                DimensionID++;
                NAPI.Task.Run(() => 
                {
                    House newHouse = new House(GetUID(), string.Empty, type, player.Position - new Vector3(0, 0, 1.12), price, false, 0, bankId, new Dictionary<string, ResidentData>(), DimensionID, false, false);
                    newHouse.Create();
                    FurnitureManager.Create(newHouse.ID);
                    newHouse.CreateInterior();
                    Houses.Add(newHouse);
                });
            }
            catch (Exception e)
        ...
        public static async void CMD_CreateParkPlace(ExtPlayer player, int price)
        {
            try
            {
                ...
                if (!player.IsInVehicle)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du musst dich im Fahrzeug befinden", 3000);
                    return;
                }
                int bankId = await MoneySystem.Bank.Create(string.Empty, 2, 0);
                DimensionID++;
        ...
        public static void CMD_HouseOwner(ExtPlayer player, string newOwner)
        {
            try
            {
                ...
                if (sessionData.HouseID == -1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du musst dich in der Markierung der Immobilie befinden", 3000);
                    return;
                }
                var house = Houses.FirstOrDefault(h => h.ID == sessionData.HouseID);
                if (house == null) return;
        ...
        public static void CMD_InvitePlayerToHouse(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (guest == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Spieler nicht gefunden", 3000);
                    return;
                }
                if (player.Position.DistanceTo(guest.Position) > 2)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du bist zu weit entfernt", 3000);
                    return;
                }
        ...
        public static void InvitePlayerToHouse(ExtPlayer player, ExtPlayer guest)
        {
            try
            {
                ...
                Notify.Send(guest, NotifyType.Info, NotifyPosition.BottomCenter, $"({player.Value}) hat Sie ins Haus eingeladen", 3000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du hast ({guest.Value}) in dein Haus eingeladen", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void OfferHouseSell(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du bist zu weit vom Käufer entfernt", 3000);
                    return;
                }
                var house = GetHouse(player, true);
                if (house == null)
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Dieser Spieler hat bereits eine Immobilie", 3000);
                    return;
                }
                ...
                if (house.Type <= 2)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Preis darf nicht weniger als {lowprice} und nicht höher als {highprice} betragen", 5000);
                        return;
                    }
                }
                else if (price < lowprice)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Preis darf nicht weniger als {lowprice} betragen", 5000);
                    return;
                }
                if (player.Position.DistanceTo(house.Position) > 30)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du bist zu weit vom Eigentum entfernt", 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(target, "openDialog", "HOUSE_SELL", $"({player.Value}) hat Ihnen seine Immobilien angeboten mit {maxCarsAmount} Garagenplätze für ${MoneySystem.Wallet.Format(price)}");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sie haben ({target.Value}) angeboten Ihre Immobilie mit {maxCarsAmount} Garagenplätze für ${MoneySystem.Wallet.Format(price)}", 5000);
            }
            catch (Exception e)
            {
        ...
        public static void acceptHouseSell(ExtPlayer player)
        {
            try
            {
                ...
                {
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SellerNotOnline), 6000);
                    return;
                }
                if (GetHouse(player, true) != null)
                {
                    ...
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du hast bereits ein Anwesen", 4500);
                    return;
                }
                ...
                if (MoneySystem.Bank.GetBalance(house.BankID) < (tax * 2))
                {
                    sellerSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Verkäufer muss für mindestens 2 Stunden Steuern bezahlen bevor er die Immobilie an Sie verkauft.", 7000);
                    return;
                }
                ...
                if (vehiclesCount > GarageManager.GarageTypes[GarageManager.Garages[house.GarageID].BDType].MaxCars)
                {
                    sellerSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Die Immobilie die Sie kaufen möchten, passen nicht alle Ihre Fahrzege rein", 7000);
                    return;
                }
                ...
                house.SetOwner(player.Name);
                Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter, $"({player.Value}) hat dein Anwesen gekauft", 3000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du hast ein Anwesen von ({seller.Value}) gekauft", 3000);
