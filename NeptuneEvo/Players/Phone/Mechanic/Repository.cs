...
namespace NeptuneEvo.Players.Phone.Mechanic
{
    public class Repository
    {
        ...
        public static void OnOrder(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyCallMechanik), 6000);
                return;
            }
            ...
            if (mechanicCount > 0)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WaitMechanikCall, mechanicCount), 6000);
            ...
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoMechanikNear), 6000);
        ...
        public static void OnCancel(ExtPlayer player, bool isDisconnect = false, bool isNotRemove = true)//Отмена для таксиста или вызывающего
        {
            ...
            {
                if (!isDisconnect) 
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben keinen Mechaniker bestellt", 3000);
            ...
            if (isDriver)
            {
                if (isNotRemove)
                {
                    ...
                    if (!isDisconnect) 
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TripCanceled), 6000);
                }
            ...
            else
            {
                if (isNotRemove)
                {
                    ...
                    if (!isDisconnect)
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCancelTripMech), 6000);
                }
            ...
            if (target != null)
            {
                ...
                if (isDriver)
                {
                    if (isNotRemove)
                    {
                        ...
                        Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DriverCancelTrip), 6000);
                ...
                {
                    if (isNotRemove)
                        Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PassengerCancel), 6000);
