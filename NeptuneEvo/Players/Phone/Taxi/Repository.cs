...
namespace NeptuneEvo.Players.Phone.Taxi
{
    public class Repository
    {
        ...
        public static void OnOrder(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyTaxiOrdered), 6000);
                return;
            }
            ...
            if (taxiCount > 0)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WaitForTaxi, taxiCount), 6000);
            ...
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoTaxiHere), 6000);
        ...
        public static void OnCancel(ExtPlayer player, bool isDisconnect = false, bool isNotRemove = true)//Отмена для таксиста или вызывающего
        {
            ...
            {
                if (!isDisconnect) 
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouDoesntCallTaxi), 6000);
                return;
            }
            ...
            else
            {
                if (isNotRemove)
                {
                    ...
                    if (!isDisconnect) 
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCancelTrip), 6000);
                }
                ...
                if (isDriver)
                {
                    if (isNotRemove)
                    {
                        ...
                        Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.De, DataName.DriverCancelTrip), 6000);
