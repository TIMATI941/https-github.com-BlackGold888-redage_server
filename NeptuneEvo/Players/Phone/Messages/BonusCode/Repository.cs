...
namespace NeptuneEvo.Players.Phone.Messages.BonusCode
{
    public class Repository
    {
        public static string Enter(ExtPlayer player, string text)
        {
            ...
            {
                return LangFunc.GetText(LangType.De, DataName.Bonus1lvl);
            }
            if (DateTime.Now < sessionData.TimingsData.NextBonusCode)
            {
                return LangFunc.GetText(LangType.De, DataName.Bonus30minCd);
            }
            text = text.ToLower();
            if (Main.BonusCodes.ContainsKey(text))
            {
                lock (Main.BonusCodes)
                {
                    ...
                    {
                        return LangFunc.GetText(LangType.De, DataName.BonusUsed);
                    }
                    ...
                    if (pcdata.UsedLimit != 0 && pcdata.UsedTimes >= pcdata.UsedLimit) 
                        return LangFunc.GetText(LangType.De, DataName.BonusEnd, pcdata.UsedLimit);
                }
            }
            return LangFunc.GetText(LangType.De, DataName.NoBonusMore);
        }
    }
}