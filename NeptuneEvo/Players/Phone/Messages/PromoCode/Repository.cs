            ...
            if (!int.TryParse(text, out uuid))
            {
                ...
                if (Main.PromoCodes.ContainsKey(text))
                {
                    if (accountData.PromoCodes[0] == "noref")
                    {
                        ...
                        return LangFunc.GetText(LangType.De, DataName.PromoAddedToAcc);
                    }

                    return LangFunc.GetText(LangType.De, DataName.AlreadyPromoEntered);
                }
                else if (Main.RefCodes.ContainsKey(text))
                {
                    if (accountData.RefferalId == 0)
                    {
                        ...
                        {
                            return LangFunc.GetText(LangType.De, DataName.CantEnterPromoPersa);
                        }
                        ...
                        return LangFunc.GetText(LangType.De, DataName.RefCodeUsed, text);
                    }

                    return LangFunc.GetText(LangType.De, DataName.RefCodeAlreadyUsed);
                }
                
                return LangFunc.GetText(LangType.De, DataName.NoCodeThisServ);
            }
            if (accountData.RefferalId == 0)
            {
                ...
                return LangFunc.GetText(LangType.De, DataName.RefCodeUsed, uuid);
            }
