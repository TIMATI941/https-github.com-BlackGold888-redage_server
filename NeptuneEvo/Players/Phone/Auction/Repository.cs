    ...
    public class Repository
    {
        ...
        public static void OnCreate(ExtPlayer player, AuctionType type, int elementId, string text, string image, int price)
        {
            ...
            if (type == AuctionType.House)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie müssen die Garage verlassen", 3000);
                    return;
                }
                title = $"Haus #{house.ID}";
            ...
            else if (type == AuctionType.Other)
            {
                ...
                if (Chars.Repository.ItemsData.ContainsKey(locationName) && Chars.Repository.ItemsData[locationName].ContainsKey("inventory"))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Gegenstand nicht gefunden", 3000);
                        return;
                    }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Auktion erstellt", 3000);
        }
        ...
        private static void Insert(ExtPlayer player, AuctionType type, int elementId, string title, string text, string image, int price)
        {
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    GameLog.Money($"player({characterData.UUID})", $"server", price, $"auction [#{autoId}]: Auktion erstellung ({type.ToString()})");
        ...
        private static void End(AuctionData auctionData)
        {
            ...
            Messages.Repository.AddSystemMessageToUuid(auctionData.CreateUUID, (int) DefaultNumber.Auction, $"Auktion auf wurde {auctionData.Title} beendet", DateTime.Now);
        ...
        public static void OnBet(ExtPlayer player, int id, int price)
        {
            ...
            {
                Init(player);
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Auktion nicht gefunden...", 3000);
                return;
            }
            
            if (auctionData.CreateUUID == characterData.UUID)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie können nicht auf Ihre eigene Auktion bieten", 3000);
                return;
            }

            if (auctionData.LastBetUUID == characterData.UUID)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie sind bereits der höchstbietende", 3000);
                return;
            }
            if (auctionData.LastPrice >= price)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Es liegt bereits ein höheres Gebot vor", 3000);
                return;
            }
            ...
            if (auctionData.Type == AuctionType.House)
            {
                ...
                if (house != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben bereits eine Immobilie", 3000);
                    return;
                }
            ...
            else if (auctionData.Type == AuctionType.Biz)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben bereits eine Business", 3000);
                    return;
                }
            }
            ...
            GameLog.Money($"player({characterData.UUID})", $"auction(Bet)", priceAndPercent, $"auction [#{auctionData.Id}]: Gebot ({auctionData.Type.ToString()})");
            ReturnMoney(auctionData.LastBetUUID, auctionData.LastPrice, auctionData.Id, auctionData.Type, "Ihr Gebot wurde angenommen!");
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Gebot abgegeben.", 3000);
            //Update(auctionData);
            if (!isAddMyList)
            {
