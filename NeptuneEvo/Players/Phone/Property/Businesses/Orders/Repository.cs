    ...
    public class Repository
    {
        ...
        public static void Open(ExtPlayer player)
        {
            ...
            if (characterData.WorkID != (int)JobsId.Trucker)
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouAreNotDalnoboy), 6000);
                return;
            }
            /*if (!sessionData.WorkData.OnWork)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouNotWorkingNow), 6000);
                return;
            }*/
        ...
        public static void Take(ExtPlayer player, int uid)
        {
            ...
            if (sessionData.OrderData.Order != -1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyTakeOrder), 6000);
                Open(player);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouMustBeAtZagruzka), 6000);
            ...
            if (!BusinessManager.Orders.ContainsKey(uid))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoOrderExists), 6000);
                Open(player);
                return;
            }
            ...
            if (bizOrder == null || bizOrder.Taked)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ThisOrderTaken), 6000);
                Open(player);
                return;
            }
            bizOrder.Taked = true;
            sessionData.OrderData.Order = uid;
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OrderDeliveryTaken, bizOrder.Name, BusinessManager.BusinessTypeNames[biz.Type]), 6000);
