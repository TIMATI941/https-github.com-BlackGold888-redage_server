...
namespace NeptuneEvo.Players.Phone.Property.Businesses
{
    public class Repository
    {
        ...
        public static void AddOrder(ExtPlayer player, string name, int value)
        {
            ...
            if (product.Ordered)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyOrderedThis), 6000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Enter10To, productsData.MaxCount - product.Lefts), 5000);
                return;
            }
                        
            if (product.Lefts + value > productsData.MaxCount)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MaxCol, productsData.MaxCount - product.Lefts), 5000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OrderedAmount, product.Name, value, order.UID), 6000);
            Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.ZakazNumber, order.UID));
        }
        ...
        public static void MaxProducts(ExtPlayer player)
        {
            ...
            var message = "Möchten Sie es wirklich kaufen:<br/>";
            ...
            foreach (var product in products)
            {
                ...
                if (count > 0)
                {
                    ...
                    message += LangFunc.GetText(LangType.De, DataName.AmountCosts, product.Name, count, (count * itemPrice));
                    success = true;
                }
            }
            ...
            if (!success)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoTovarZakaz), 6000);
                return;
            }
            ...
            Trigger.ClientEvent(player, "openDialog", "BIZ_CONFIRM_BUY_ORDERS", $"Möchten Sie Ware in <font style='color: red'>{whCount}</font> höhe von <font style='color: green'>{whCost}$</font> bestellen? <br/><br/>{message}");
        }
        public static void MaxProductsConfirm(ExtPlayer player)
        {
            ...
            if (!characterData.BizIDs.Contains(bizId))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                    LangFunc.GetText(LangType.De, DataName.YouHaveNoBusiness), 6000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AllTovarSucOrder), 6000);
        ...
        public static void CancelOrder(ExtPlayer player, int uid)
        {
            ...
            if (order == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie haben diese Ware nicht bestellt", 3000);
                return;
            }
            ...
            if (order.Taked)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie können diese Bestellung nicht Stornieren wenn diese bereits ausgeliefert wird", 3000);
                return;
            }
            ...
            biz.Zatratq -= price;
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Bestellung {order.Name} storniert", 3000);
        }
        ...
        public static void ExtraCharge(ExtPlayer player, string name, int value)
        {
            ...
            if (name == "Lottoschein" || name == "Verbrauchsmaterial")
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantLotteryPriceChange), 6000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantSetThisPrice), 6000);
                return;
            }
            
            product.Price = value;
            
            var ch = (biz.Type == 7 || biz.Type == 11 || biz.Type == 12 || product.Name == "Tätowierungen" || product.Name == "Perücken") ? "%" : "$";
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ThenThisCosts, product.Name, product.Price, ch), 6000);
            
            if (product.Name == "Benzin") 
                biz.UpdateLabel();
        ...
        public static void OnSell(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "openDialog", "BIZ_SELL_TOGOV", $"Möchten Sie Ihre Immobilie wirklich verkaufen? ${MoneySystem.Wallet.Format(price)}?");   
        }
        ...
        public static void OnSellConfirm(ExtPlayer player)
        {
            ...
            Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.SellBizGos, MoneySystem.Wallet.Format(price)), DateTime.Now);
        }
