                ...
                if (characterData != null)
                {
                    ...
                    if (sessionData.DicePlayingWith != null)
                    {
                        ...
                        if (targetSessionData != null)
                        {
                            ...
                            Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ChelVishelNoGame), 6000);
                        }
                    ...
                    if ((sessionData.SellItemData.Seller != null || sessionData.SellItemData.Buyer != null) && Chars.Repository.TradeGet(player))
                    {
                        ...
                        if (seller == player)
                        {
                            if (buyerSessionData != null)
                            {
                                ...
                                Notify.Send(buyer, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DisconnectedSellClose), 6000);
                            }
                        }
                        else if (buyer == player)
                        {
                            if (sellerSessionData != null)
                            {
                                ...
                                Notify.Send(seller, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DisconnectedSellClose), 6000);
                            }
                        }
                    ...
                    if (type != DisconnectionType.Kicked && characterData.AdminLVL == 0)
                    {
                        ...
                        if (cufdata.Cuffed && (cufdata.CuffedByMafia || cufdata.CuffedByCop) && characterData.DemorganTime <= 0)
                        {
                            ...
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.DemorganCuffs, player.Name));
                                GameLog.Admin($"server", $"demorgan(4ч,выход из игры в наручниках)", $"{player.Name}");
                            }
                            else if (cufdata.CuffedByMafia)
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.DemorganStyzahka, player.Name));
                    ...
                    Commands.RPChat("sb", player, LangFunc.GetText(LangType.De, DataName.PlayerDisconnect), optName: player.Name + $" ({player.Value})");
