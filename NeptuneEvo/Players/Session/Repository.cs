        ...
        {                
            try
            {
                ...
                var session = await db.Sessions
                    .Where(s => s.Hash == Accounts.Repository.GetMD5($"{sessionData.RealSocialClub}_{sessionData.Address}") && s.Data >= DateTime.Now)
                    .FirstOrDefaultAsync();
                ...
                if (session != null)
                {
                    ...
                    if (account != null)
                    {
                        ...
                        if (ban != null)
                        {
                            player.setBan($"Sie Bannen {ban.Time.Day} {ban.Time.ToString("MMMM")} {ban.Time.Year}Y. {ban.Time.Hour}:{ban.Time.Minute}:{ban.Time.Second} bis {ban.Until.Day} {ban.Until.ToString("MMMM")} {ban.Until.Year}Y. {ban.Until.Hour}:{ban.Until.Minute}:{ban.Until.Second} (UTC+3). Grund: {ban.Reason} ({ban.Byadmin})");
                        ...
                        if (session.OneTime)
                        {
                            ...
                            await db.Sessions
                                .Where(s => s.Hash == Accounts.Repository.GetMD5($"{sessionData.RealSocialClub}_{sessionData.Address}"))
                                .DeleteAsync();
                        }
                        else
                        {
        ...
        {               
            try
            {
                ...
                var isSession = await db.Sessions.AnyAsync(s =>
                    s.Hash == Accounts.Repository.GetMD5($"{sessionData.RealSocialClub}_{sessionData.Address}"));
                
                if (isSession)
                {
                    await db.Sessions
                        .Where(s => s.Hash ==
                                    Accounts.Repository.GetMD5($"{sessionData.RealSocialClub}_{sessionData.Address}"))
                ...
                else
                {
                    ...
                    {
                        Hash = Accounts.Repository.GetMD5($"{sessionData.RealSocialClub}_{sessionData.Address}"),
            ...
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    if (!isSession)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SessionSaved), 6000);
                    }
                    else
                    {
                        await db.Sessions
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SessionBroken), 6000);
                    }
                }
                catch (Exception e)
                {
