        ...
        public static void Open(ExtPlayer player, ushort objectId, ushort mapId, string mapName, Vector3 position, float range, WarType type, ushort attackingId, ushort protectingId)
        {
            try
            {
                ...
                if (type == WarType.Gangs)
                    title = $"Angriff auf Gebiet {mapName}";
                if (type == WarType.Mafia)
                    title = $"Angriff auf Business {mapName}";
                if (type == WarType.OrgHouse)
                    title = $"Angriff auf Immobilie {mapName}";
        ...
        public static ushort War(ExtPlayer player, Players.Models.WarData warData, sbyte typeBattle, sbyte composition, sbyte weaponsCategory, sbyte day = 0, sbyte hour = 0, sbyte min = 0, bool isCheckTime = true, bool isWarInterface = true)
        {
            ...
            try
            {
                ...
                if (isCheckTime)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Falsche Uhrzeit", 6000);
                        //if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                        return 0;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Стрелку можно забить минимум за два часа", 3000); //TODO
                        //if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                        return 0;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Стрелку можно забить минимум за 5 минут", 3000); //TODO
                        //if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                        return 0;
                    }
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Diese Gebiet wird bereits erobert.", 8000);
                    if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                    return 0;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie vereinbarn ein Treffen", 15000);
        ...
        public static void InWarZone(ExtPlayer player, WarType type, int index)
        {
            try
            {
                ...
                else
                {
                    ...
                    frameList.Header = "Warzone auswählen";
        ...
        private static void UpdateWar(ushort warId)
        {
            try
            {
                    ...
                    if (war.Time <= time)
                    {
                        ...
                        else
                        {
                            if (war.GripType == WarGripType.RetentionTerritory)
                            {
                                ...
                                Notification(war.Id, "Das gegnerische Team kann das Gebiet betreten");
                            }
                            else
                            {
                ...
                else if (war.Status == WarStatus.Protection)
                {
                        ...
                        war.Status = WarStatus.War;
                        war.Counting = WarCounting;
                        Notification(war.Id, "Das Angriffs Team kann das Gebiet betreten");
            ...
            try
            {
                ...
                switch (war.Type)
                {
                    case WarType.OrgWarZone:
                        if (draw)
                        {
                            Fractions.Manager.SendCoolOrganizationMsg(war.ProtectingId,"Krieg", "Unentschieden", "Es ist ein Unentschieden", "", 10000);   
                            Fractions.Manager.SendCoolOrganizationMsg(war.AttackingId,"Krieg", "Unentschieden", "Es ist ein Unentschieden", "", 10000); 
                            //Fractions.Manager.sendOrganizationMessage(war.AttackingId, "Es ist ein Unentschieden");
                            //Fractions.Manager.sendOrganizationMessage(war.ProtectingId, "Es ist ein Unentschieden");
                        }
                        else if (victoryAttackers)
                        {
                            Fractions.Manager.SendCoolOrganizationMsg(war.ProtectingId,"Krieg", "Verloren", "Sie haben den Kampf verloren und verlieren das Gebiet :(", "", 10000);   
                            Fractions.Manager.SendCoolOrganizationMsg(war.AttackingId,"Krieg", "Gewonnen", "Sie haben Gewonnen! Das Gebiet steht jetzt unter Ihrer Kontrolle", "", 10000); 
                            //Fractions.Manager.sendOrganizationMessage(war.AttackingId, "Вы победили! Территория переходит под ваш контроль, поздравляем!");
                           // Fractions.Manager.sendOrganizationMessage(war.ProtectingId, "Sie haben den Kampf verloren und verlieren das Gebiet :(");
                        }
                        else
                        {
                            Fractions.Manager.SendCoolOrganizationMsg(war.ProtectingId,"Krieg", "Gewonnen", "Sie haben das Gebiet verteidigt", "", 10000);   
                            Fractions.Manager.SendCoolOrganizationMsg(war.AttackingId,"Krieg", "Verloren", "Sie haben Verloren! Das Gebiet wurde nicht erobert", "", 10000); 
                            //Fractions.Manager.sendOrganizationMessage(war.AttackingId, "Sie haben Verloren! Das Gebiet wurde nicht erobert");
                            //Fractions.Manager.sendOrganizationMessage(war.ProtectingId, "Sie haben das Gebiet verteidigt");
                        }
                        break;
                    ...
                    case WarType.Gangs:
                        if (victoryAttackers)
                        {
                            Fractions.Manager.SendCoolFractionMsg(war.ProtectingId,"Eroberung", "Verloren", LangFunc.GetText(LangType.De, DataName.CaptureLoseByDef), "", 10000);   
                            Fractions.Manager.SendCoolFractionMsg(war.AttackingId,"Eroberung", "Gewonnen", LangFunc.GetText(LangType.De, DataName.CaptureWinByAttack), "", 10000); 
                            //Fractions.Manager.sendFractionMessage(war.ProtectingId, LangFunc.GetText(LangType.De, DataName.CaptureLoseByDef));
                            //Fractions.Manager.sendFractionMessage(war.AttackingId, LangFunc.GetText(LangType.De, DataName.CaptureWinByAttack));
                        }
                        else
                        {
                            Fractions.Manager.SendCoolFractionMsg(war.ProtectingId,"Eroberung", "Gewonnen", LangFunc.GetText(LangType.De, DataName.CaptureWinByDef), "", 10000);   
                            Fractions.Manager.SendCoolFractionMsg(war.AttackingId,"Eroberung", "Verloren", LangFunc.GetText(LangType.De, DataName.CaptureLoseByAttack), "", 10000);   
                            //Fractions.Manager.sendFractionMessage(war.ProtectingId, LangFunc.GetText(LangType.De, DataName.CaptureWinByDef));
                           // Fractions.Manager.sendFractionMessage(war.AttackingId, LangFunc.GetText(LangType.De, DataName.CaptureLoseByAttack));
                        }
                    ...
                    case WarType.Mafia:
                        if (victoryAttackers)
                        {
                            Fractions.Manager.sendFractionMessage(war.ProtectingId, LangFunc.GetText(LangType.De, DataName.DefLoseBizWar));
                            Fractions.Manager.sendFractionMessage(war.AttackingId, LangFunc.GetText(LangType.De, DataName.AttackWinBizWar));

                        }
                        else
                        {
                            Fractions.Manager.sendFractionMessage(war.ProtectingId, LangFunc.GetText(LangType.De, DataName.DefWinBizWar));
                            Fractions.Manager.sendFractionMessage(war.AttackingId, LangFunc.GetText(LangType.De, DataName.AttackLoseBizWar));
                        }
