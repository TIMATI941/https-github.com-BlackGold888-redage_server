    ...
    public class Repository
    {
        ...
        public static void UpdateRank(ExtPlayer player, string number, int rank)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCantUpVehicleRank), 6000);
                    return;
                }
                if (rank < 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantSetNullRank), 6000);
                    return;
                }
                if (vehicleLocalData.Access == VehicleAccess.Organization && vehicleLocalData.Fraction == memberOrganizationData.Id)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SuccessEditRankVehWithPrikol, vData.model, number), 6000);
                }
                vData.rank = rank;
                GetVehicles(player);
        ...
        public static void Evacuation(ExtPlayer player, string number)
        {
            try
            {
                ...
                if (vehicleLocalData.Fraction == memberOrganizationData.Id && memberOrganizationData.Rank >= vehicleLocalData.MinRank)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoMoneyNeed, 20 - characterData.Money), 6000);
                        return;
                    }
                    if (vehicleLocalData.Occupants.Count >= 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarUsed), 6000);
                        return;
                    }
                    if (vehicleLocalData.Access == VehicleAccess.OrganizationGarage)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarNoNeedEvac), 6000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarOtognana), 6000);
                    GetVehicles(player);
                }
            }
            catch (Exception e)
        ...
        public static void Gps(ExtPlayer player, string number)
        {
            try
            {
                ...
                if (vehicleLocalData.Fraction == memberOrganizationData.Id && memberOrganizationData.Rank >= vehicleLocalData.MinRank)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GPSOtmechenoCarFraction), 6000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void DefaultRanks(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucResetDefaultVehRanks), 4500);
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
