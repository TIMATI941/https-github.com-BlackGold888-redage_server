    ...
    public class Repository
    {
        public static List<OrganizationUpdateData> Get(ExtPlayer player, OrganizationData organizationData)
        {
            ...
            if (player.IsOrganizationAccess(RankToAccess.OrgUpgrate, false))
            {
                ...
                if (upgraded == 0) 
                    updateInfo.Add(new OrganizationUpdateData("upgrade", "Office Upgrade", "organisationsicon-office", false, Main.PricesSettings.FirstOrgPrice));
                else if (upgraded == 1) 
                    updateInfo.Add(new OrganizationUpdateData("upgrade", "Office Upgrade", "organisationsicon-office", true, Main.PricesSettings.SecondOrgPrice));
                    
                if (!organizationData.Stock)
                    updateInfo.Add(new OrganizationUpdateData("upgrades", "Lager erstellen", "inv-safe", false, Main.PricesSettings.StockPrice));
                ...
                if (organizationData.CrimeOptions)
                {
                    foreach (var p in organizationData.Schemes)
                    {
                        if (Manager.WeaponsOrgPrice.ContainsKey(p.Key) && !p.Value)
                        {
                            ...
                            updateInfo.Add(new OrganizationUpdateData(p.Key, $"Bauplan {p.Key}", Chars.Repository.ItemsInfo[wType].Icon, false, Manager.WeaponsOrgPrice[p.Key]));
                        }
                    }
                }
                ...
                {
                    updateInfo.Add(new OrganizationUpdateData("updtype", "Organisationstyp ändern",
                        "fractionsicon-fractions", true, Main.PricesSettings.UpdateTypeOrganization));
                    
                    updateInfo.Add(new OrganizationUpdateData("newleader", "Leader ändern",
