    ...
    public class Repository
    {
        public static void UpdateStock(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Sie haben das Familien Lager geöffnet",
                        3000);
                    Organizations.Table.Logs.Repository.AddLogs(player, OrganizationLogsType.OpenStock, "Öffnet Lager");
                }
                else
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Sie haben das Familien Lager geschlossen", 3000);
                    Organizations.Table.Logs.Repository.AddLogs(player, OrganizationLogsType.CloseStock, "Lager geshlossen");
                }
        ...
        public static void SaveSetting(ExtPlayer player, string slogan, byte salary, string discord, int colorR, int colorG, int colorB)
        {
            try
            {
                ...
                if (slogan.Length > 65)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Der Slogan ist zu lang", 4500);
                    return;
                }
                ...
                if (Main.stringGlobalBlock.Any(c => testmsg.Contains(c)))
                {
                    ...
                    Trigger.SendToAdmins(3, "!{#636363}[A] " + LangFunc.GetText(LangType.De, DataName.AdminAlertFTableNews, player.Name, player.Value, slogan));
