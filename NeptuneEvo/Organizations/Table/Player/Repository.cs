    ...
    public class Repository
    {
        ...
        public static void Reprimand(ExtPlayer player, int uuid, string name, string text)
        {
            try
            {
                ...
                Logs.Repository.AddLogs(player, OrganizationLogsType.Reprimand, LangFunc.GetText(LangType.De, DataName.GivenVigovor, name, uuid, text));
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
        ...
        public static void Disolver(ExtPlayer player)
        {
            try
            {
                ...
                if (!organizationData.IsLeader(player.GetUUID()))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie sind nicht der Familieneigentümer", 3000);
                    return;
                }
                
                if (organizationData.Vehicles.Count >= 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Um eine Familie aufzulösen, müssen Sie alle Fahrzeuge der Familie verkaufen", 6000);
                    return;
                }
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    if (foreachPlayer != player) 
                        Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter, $"Sie wurden aus der Organisation {organizationData.Name} entlassen", 3000);
                }
                ...
                EventSys.SendCoolMsg(player,"Familie", "Familie aufgelöst!", $"Sie haben die Familie aufgelöst. Geld wurde Ihnen gutgeschrieben", "", 10000);
