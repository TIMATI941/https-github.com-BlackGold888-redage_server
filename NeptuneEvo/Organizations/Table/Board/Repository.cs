    ...
    public class Repository
    {
        ...
        public static void Add(ExtPlayer player, string title, string text)
        {
            try
            {
                ...
                if (text.Length > 100)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MaxTableNewsText), 4500);
                    return;
                }
                ...
                if (Main.stringGlobalBlock.Any(c => testmsg.Contains(c)))
                {
                    ...
                    Trigger.SendToAdmins(3, "!{#636363}[A] " + LangFunc.GetText(LangType.De, DataName.AdminAlertFTableNews, player.Name, player.Value, text));
                ...
                Fractions.Manager.sendOrganizationMessage(memberOrganizationData.Id, "[F] " + LangFunc.GetText(LangType.De, DataName.TableNewNews), true);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouAddedTableNews), 6000);
        ...
        public static void Update(ExtPlayer player, int index, string title, string text)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouEditedTableNews), 6000);
                Get(player, index + 1);
            }
            catch (Exception e)
            {
        ...
        public static void Delete(ExtPlayer player, int index)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouDeletedTableNews), 6000);
                Get(player, 1);
            }
            catch (Exception e)
            {
