    ...
    public class Repository
    {
        ...
        public static void CreateRank(ExtPlayer player, string name, int score)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MaxRanksAmount), 4500);
                    return;
                }
                ...
                if (Main.stringGlobalBlock.Any(c => testmsg.Contains(c)))
                {
                    ...
                    Trigger.SendToAdmins(3, "!{#636363}[A] " + LangFunc.GetText(LangType.De, DataName.AdminAlertFTableNews, player.Name, player.Value, name));
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ExistNameRank), 4500);
                    return;
                }
        ...
        public static void UpdateRankScore(ExtPlayer player, int id, int score)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Mindestwert: {NeptuneEvo.Table.Repository.MinScore}, und das Maximum: {NeptuneEvo.Table.Repository.MaxScore}", 5500);
                    return;
                }
        ...
        public static void DeletePlayer(ExtPlayer player, int uuid)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCantUvolit), 6000);
                        return;
                    }
                    
                    Organizations.Player.Repository.RemoveOrganizationMemberData(orgId, uuid);
                    
                    Logs.Repository.AddLogs(player, OrganizationLogsType.UnInvite, $"Entlässt {targetMemberOrganizationData.Name} ({targetMemberOrganizationData.UUID})");
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TiUvolil, targetMemberOrganizationData.Name), 6000);
                }
        ...
        public static void DefaultRanks(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucResetDefaultRanks), 6000);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Rangeinstellungen zurückgesetzt", 5000);
                var ranksName = NeptuneEvo.Table.Repository.GetRanksData(organizationData.Ranks);
                Trigger.ClientEvent(player, "client.org.main.setRanks", JsonConvert.SerializeObject(ranksName));
            }
            catch (Exception e)
        ...
        public static void SetLeader(ExtPlayer player, int uuid)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben nicht genug Rockford-Coins um den Leader zu ändern", 3000);
                    return;
                }
                UpdateData.RedBucks(player, -Main.PricesSettings.UpdateOrganizationLeader, msg:"Leader ändern");
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie übergeben den Leader an {targetMemberOrganizationData.Name}", 5000);
