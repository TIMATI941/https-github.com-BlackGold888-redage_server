    ...
    class Manager : Script
    {
        ...
        public static void onResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(525, "Organisationsmanagement", new Vector3(-595.1591, -346.04993, 35.154648), 4, true));
                ...
                //
                NAPI.TextLabel.CreateTextLabel("~w~Garage", new Vector3(-1000.14856, -774.0971, 61.734413), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                //
                NAPI.TextLabel.CreateTextLabel("~w~Garage", new Vector3(-1000.14856, -774.0971, 60.734413), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                NAPI.Marker.CreateMarker(1, new Vector3(-1000.14856, -774.0971, 60.734413), new Vector3(), new Vector3(), 2f, new Color(255, 255, 255, 60));
                CustomColShape.CreateCylinderColShape(new Vector3(-1000.14856, -774.0971, 60.734413), 2f, 2f, NAPI.GlobalDimension, ColShapeEnums.Organizations, 5);
                //
                NAPI.TextLabel.CreateTextLabel("~w~Garage", new Vector3(-1000.14856, -774.0971, 60.734413), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                //Office Exit
                NAPI.TextLabel.CreateTextLabel("~w~Straße", new Vector3(-1003.197, -774.2839, 61.8944), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                PedSystem.Repository.CreateQuest("ig_jackie", new Vector3(-773.945, 313.0294, 85.70606), 177.9122f, 0,"npc_org", ColShapeEnums.OrgCreate, "~y~NPC~w~ Jacky\n Organisations Direktor", false);
                ...
                foreach (DataRow Row in result.Rows)
                {
                    ...
                    {
                        if (stock) 
                            organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Lager {name}"), StockPositions[upgrade], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
                        else 
                            organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Lager {name}\n~r~nicht aktiv"), StockPositions[upgrade], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
        ...
        public static void DeleteVehicle(ExtPlayer player, string number, bool type)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ihre Familie hat nicht dieses Fahrzeug", 3000);
                    return;
                }
                if (!type)
                {
                    ...
                    Trigger.ClientEvent(player, "openDialog", "ORGCAR_SELL_TOGOV", $"Möchten Sie wirklich {model} ({number}) an den Staat für ${MoneySystem.Wallet.Format(price)} verkaufen?");
                }
                else
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie verkaufen {model} ({number}) für {MoneySystem.Wallet.Format(price)}$", 3000);
                    Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SellCar, $"verkauft {model} ({number})");
        ...
        public static void CMD_setfamily(ExtPlayer player, int orgId, int status) {
            try
            {
                ...
                if (organizationData == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Diese Familie existiert nicht", 3000);
                    return;
                }
                
                if (!organizationData.Status)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Diese Familie ist nicht aktiv", 3000);
                    return;
                }
                ...
                if (status == 1)
                {
                    if (organizationData.CrimeOptions)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Diese Familie hat die illegale Familienoptionen bereits aktiviert", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sie haben illegale Familienoptionen aktiviert #{orgId}.", 3000);
                }
                else if (status == 0)
                {
                    if (!organizationData.CrimeOptions)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Diese Familie hat die illegale Familienoptionen deaktiviert", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sie haben die illegale Familienoptionen deaktiviert #{orgId}.", 3000);
                }
        ...
        public static void CMD_SetOrganizationBlip(ExtPlayer player, int orgId, int sprite, byte color)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Dieser Sprite können Sie nicht auswählen", 3000);
                    return;
                }
                if (color  < 1 || color > 85)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Diese Farbe können Sie nicht auswählen", 3000);
                    return;
                }
                if(organizationData.Blip != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie haben bereits einen Blip", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Eine Markierung für die Familie {organizationData.Name} wurde erstellt", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_DeleteOrganizationBlip(ExtPlayer player, int orgId)
        {
            try
            {
                ...
                if (organizationData.Blip == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Diese Familie hat keinen Blip", 3000);
                    return;
                }
                Trigger.SetTask(async () =>
                {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Familienmarkierung {organizationData.Name} wurde gelöscht", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void SetFracRankOffline(ExtPlayer player, int uuid, int rank)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Spieler wurde in der Familie nicht gefunden", 3000);
                    return;
                }
                
                if (targetMemberOrganizationData.Rank >= memberOrganizationData.Rank)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du kannst diesen Rang nicht vergeben weil dieser einen höheren Rang als du hat", 8000);
                    return;
                }
                ...
                if (targetMemberOrganizationData.Rank > rank) Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SetRank, $"Herabstuft Offline {targetMemberOrganizationData.Name} ({memberOrganizationData.UUID}) zum ({targetMemberOrganizationData.Rank} -> {rank})");
                else Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SetRank, $"Befördert Offline {targetMemberOrganizationData.Name} ({memberOrganizationData.UUID}) zum ({targetMemberOrganizationData.Rank} -> {rank})");

                Player.Repository.SetRank(organizationData.Id, uuid, rank);
                
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben den Rang {targetMemberOrganizationData.Name} auf {organizationData.Ranks[rank].Name} geändert", 6000);
        ...
        public static void SetFracRank(ExtPlayer player, ExtPlayer target, int newRank)
        {
            try
            {
                ...
                if (targetMemberOrganizationData.Rank > newRank) Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SetRank, $"Herabgestuft {target.Name} ({targetMemberOrganizationData.UUID}) zum ({targetMemberOrganizationData.Rank} -> {newRank})");
                else Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SetRank, $"Befördert {target.Name} ({targetMemberOrganizationData.UUID}) zum ({targetMemberOrganizationData.Rank} -> {newRank})");
                ...
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie sind jetzt {organizationData.Ranks[newRank].Name}", 6000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben den Rang vom {target.Name} auf {organizationData.Ranks[newRank].Name} geändert", 6000);
        ...
        public static void InviteToOrganization(ExtPlayer sender, ExtPlayer target)
        {
            try
            {
                ...
                if (memberOrganizationData == null)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"Du bist in keiner Familie", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "Dieser Spieler ist bereits Mitglied in der Familie", 3000);
                    return;
                }
                if (targetCharacterData.AdminLVL >= 1 && targetCharacterData.AdminLVL <= 7) return;
                if (targetCharacterData.LVL < 1)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "Erfordert mindestens Level 1 um einen Spieler in eine Familie einzuladen", 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(target, "openDialog", "INVITED_ORG", $"{sender.Name} hat dich in die Familie {organizationData.Name} eingeladen");
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {target.Name} in die Familie eingeladen", 3000);
        ...
        public static void UnInviteFromOrganization(ExtPlayer sender, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"Du wurdest aus der Familie {organizationData.Name} entlassen", 3000);
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {target.Name} aus der Familie entlassen", 3000);
                Table.Logs.Repository.AddLogs(sender, OrganizationLogsType.UnInvite, $"entlassen {target.Name} ({target.GetUUID()})");
            }
            catch (Exception e)
            {
        ...
        public static bool StockBuy(ExtPlayer player)
        {
            try
            {
                ...
                if (organizationData == null) 
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du bist in keiner Familie", 3000);
                    return false;
                }
                if (organizationData.Stock)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Das Lager ist bereits für Ihre Familie erstellt.", 3000);
                    return false;
                }
                ...
                organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Lager {organizationData.Name}"), StockPositions[organizationData.OfficeUpgrade], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast ein Lager für die Familie erworben!", 3000);
        ...
        public static bool UpdateCrimeOptions(ExtPlayer player) 
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du hast nicht genug Rockford-Coins um deine Familie zu verbessern", 3000);
                    return false;
                }
                UpdateData.RedBucks(player, -Main.PricesSettings.UpdateTypeOrganization, msg:"Familie verbessern");
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Organisation Typ geändert", 3000);
                return true;
            }
            catch (Exception e)
            {
        ...
        public static bool SchemeBuy(ExtPlayer player, string scheme, int price = -1)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Dieses System wurde bereits in Ihre Familie eingekauft", 3000);
                    return false;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast einen Bauplan für die Familie gekauft", 3000);
        ...
        public static bool UpgradeOrganization(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie dürfen sich nicht in einem Fahrzeug befinden während Sie ein Upgrade kaufen", 6000);
                    return false;
                }
                if (upgraded == 0)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du hast nicht genug Geld um deine Familie zu verbessern", 3000);
                        return false;
                    }
                ...
                else if (upgraded == 1)
                {
                    ...
                    UpdateData.RedBucks(player, -Main.PricesSettings.SecondOrgPrice, msg:"Familie verbessern");
                ...
                if (organizationData.Stock) 
                    ...
                    organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Lager {organizationData.Name}\n~r~nicht aktiv"), StockPositions[organizationData.OfficeUpgrade], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hst die Familie verbessert", 3000);
                GameLog.Money($"player({player.GetUUID()})", $"server", Main.PricesSettings.StockPrice, $"OrgStockBuy({organizationData.Id})");
                return true;
            }
            catch (Exception e)
        ...
        public static void CreateOrganization(ExtPlayer player, bool isCrime, string orgName)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Block10Min), 4500);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Verwaltung nicht verfügbar", 3000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Man darf keine Vewarnungen haben um eine Familie zu gründen", 3000);
                    Trigger.ClientEvent(player, "client.org.create.close");
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MaxRankNameLenght), 4500);
                    return;
                }
                ...
                if (Main.stringGlobalBlock.Any(c => testmsg.Contains(c)))
                {
                    ...
                    Trigger.SendToAdmins(3, "!{#636363}[A] " + LangFunc.GetText(LangType.De, DataName.AdminAlertFTableNews, player.Name, player.Value, orgName));
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Wir bedauern aber eine Familie mit den Namen {orgName} existiert bereits", 3000);
                    return;
                }
                if (!MoneySystem.Wallet.Change(player, -Main.PricesSettings.CreateOrgPrice))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du hst nicht genug Geld um eine Familie zu gründen", 3000);
                    Trigger.ClientEvent(player, "client.org.create.close");
                    return;
                }
                    ...
                    try
                    {
                        ...
                        Trigger.SetMainTask(() =>
                        {
                            try
                            {
                                ...
                                CustomColShape.CreateCylinderColShape(new Vector3(-1396.274, -480.7186, 57.100), 2f, 2f, organizationData.GetDimension(), ColShapeEnums.Organizations, 4);
                                organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Lager {orgName}\n~r~nicht aktiv"), StockPositions[0], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
                                ...
                                SendPlayer(player);
                                NAPI.Chat.SendChatMessageToAll("!{#50C878}" + $"Im Staat wurde eine neue Familie gegründet - {orgName}!");
                                
                                EventSys.SendCoolMsg(player,"Familie", "Familie gründen", $"Herzlichen Glückwunsch zur Gründung Ihrer eigenen Familie - {orgName}!", "", 10000);
        ...
        public static void CraftBodyArmor(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Nicht für deine Familie verfügbar", 5000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du hast eine Schutzweste mit 100% hergestellt", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void OpenStock(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Familienlager geschlossen.", 3000);
                    return;
                }
                OpenOrgStock(player);
            }
                ...
                switch (action)
                {
                    case "put_stock":
                        ...
                        if (item == "mats")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMats, $"lagert Mats ins Lager (x{amount})");
                        }
                        else if (item == "drugs")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeDrugs, $"lagert Drogen ins Lager (x{amount})");
                        }
                        else if (item == "money")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMoney, $"lagert Geld ins Lager (${MoneySystem.Wallet.Format(amount)})");
                        }
                        else if (item == "medkits")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMedkits, $"lagert Verbandskasten ins Lager (x{amount})");
                        }
                        //GameLog.Stock(cdata.FractionID, cdata.UUID, player.Name, item, amount, "in");
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Im Lager verbleiben {stockContains + amount}, Du hast {playerHave - amount}", 5000);
                        break;
                    case "take_stock":
                        ...
                        if (item == "mats")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMats, $"nimmt Mats von Lager (x{amount})");
                        }
                        else if (item == "drugs")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeDrugs, $"nimmt Drogen von Lager (x{amount})");
                        }
                        else if (item == "money")
                        {
                            var lastMulti = organizationData.MoneyMultiplier();
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMoney, $"nimmt Geld von Lager (${MoneySystem.Wallet.Format(amount)})");
                        }
                        else if (item == "medkits")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMedkits, $"nimmt Verbandskasten von Lager (x{amount})");
                        }
                        //GameLog.Stock(cdata.FractionID, cdata.UUID, player.Name, item, amount, "out");
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Im Lager verbleiben {stockContains - amount}, Du hast {playerHave + amount}", 3000);
                        break;
                    default:
        ...
        public static void Open(ExtPlayer player, int index)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du schleppst jemanden mit", 3000);
                return;
            }
            ...
            try
            {
                ...
                switch (intid)
                {
                    case 1:
                        ...
                        Trigger.ClientEvent(player, "openDialog", "CONFIRM_BUY_ORGBODYARMOUR", "Möchtest du wirklich eine Schutzweste mit 100% herstellen? Kosten: 400 Mats");
                        return;
                    case 2:
                        OpenStock(player);
                        return;
            ...
            try
            {
                ...
                switch (listItem)
                {
                    case "createorg":
                        ...
                        frameList.Header = LangFunc.GetText(LangType.De, DataName.Famka);
                        frameList.Callback = callback_organizations1;

                        frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.OfficePreview), "ofprev"));

                        //frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.OfficeUpg), "ofprevinfo"));

                        frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.GaragePreview), "garprev"));

                        //frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.GarageUpg), "garprevinfo"));

                        frameList.List.Add(new ListData(LangFunc.GetText(LangType.De,DataName.CreatePrice, Main.PricesSettings.CreateOrgPrice), "createorg"));
            ...
            try
            {
                ...
                switch (listItem)
                {
                    ...
                    case "createorg":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du bist bereits in einer Familie", 5000);
                            return;
                        }
                        Trigger.ClientEvent(player, "openDialog", "CreateOrg", $"Sind Sie sicher, dass Sie Ihre eigene Familie gründen wollen? Der Name der Familie ist Ihr Nachname, wenn Sie eine Familie gründen. Preis: {Main.PricesSettings.CreateOrgPrice}$ Du bekommst eine Garage mit 5 Plätzen und ein Office. Familien Chat: (/fc /fcb).");
                        break;
                }
            }
            catch (Exception e)
        ...
        public static void organizationChat(ExtPlayer player, string message)
        {
            try
            {
                ...
                string status = organizationData.Ranks.ContainsKey(memberOrganizationData.Rank) ? $"{organizationData.Ranks[memberOrganizationData.Rank].Name} " : "nicht vorhanden";
