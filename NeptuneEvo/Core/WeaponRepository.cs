    ...
    class WeaponRepository : Script
    {
        ...
        public static void TakeWeapon(ExtPlayer player) // Пока что думаем как реализовать instant переменную.
        {
            try
            {
                ...
                if (ItemInfo.functionType == newItemType.Weapons || ItemInfo.functionType == newItemType.MeleeWeapons)
                {
                    ...
                    Commands.RPChat("sme", player, $"nimmt" + (characterData.Gender ? "" : "а") + $" {Chars.Repository.ItemsInfo[Item].Name}");
                ...
                else
                {
                    ...
                    if (isUseItem == 1) Commands.RPChat("sme", player, $"nimmt" + (characterData.Gender ? "" : "а") + $" {Chars.Repository.ItemsInfo[Item].Name}");
                }
            }
            catch (Exception e)
            {
        ...
        public static void PlayerKickAntiCheat(ExtPlayer player, byte reason, bool type)
        {
            try
            {
                ...
                if (!type)
                {
                    Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.WasKickedAC, player.Name, player.Value, reas));
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.Center, LangFunc.GetText(LangType.De, DataName.YouWasKickedAc), 30000);
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.Center, LangFunc.GetText(LangType.De, DataName.AcIfUsure), 30000);
                ...
                else
                {
                    ...
                    Trigger.SendToAdmins(1,"!{#DF5353}" + LangFunc.GetText(LangType.De, DataName.AutoACBan2, player.Name, reas));
                   ...
                   // Ban.Online(player, until, true, $"Cheats(0{reas})", "AntiCheat");
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.Center, LangFunc.GetText(LangType.De, DataName.ReasonCheats), 30000);
        ...
        public static void RemoteEvent_changeWeapon(ExtPlayer player, int key)
        {
            try
            {
                ...
                if (key == 4)
                {
                    ...
                    if (Item.ItemId == ItemId.Mask) Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouPutOnMask), 6000);
                    else Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouPutOffMask), 6000);
                ...
                if (ItemInfo.functionType == newItemType.Weapons || ItemInfo.functionType == newItemType.MeleeWeapons)
                {
                    /*if (player.IsInVehicle)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantDoWeaponOnVeh), 6000);
                            return;
                        }
                        else if (Item.ItemId == ItemId.Revolver || Item.ItemId == ItemId.RevolverMk2 || Item.ItemId == ItemId.NavyRevolver)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantDoRevolverCar), 6000);
                            return;
                        }
                    }*/
                    if (characterData.LVL == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantDoTimeWeapon), 6000);
                        return;
                    }
                    if (characterData.ArrestTime >= 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantDoKPZ), 6000);
                        return;
                    }
