    ...
    class Commands : Script
    {
        ...
        public static void CMD_findPoint(ExtPlayer player)
        {
            ...
            if (!vehicleLocalData.DeliveryData.JStage) // Машину еще не взяли, нужна точка до машины 
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucMarkerInstallSbyt), 5000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucMarkerInstallSdacha), 5000);
        }
        ...
        public static void CMD_ammo(ExtPlayer player, int ID, int type, int amount = 1)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindPlayerWithId), 6000);
                    return;
                }
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) gibt Munition ({type}|{amount}) {target.Name} ({target.Value})");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_newVehicleNumber(ExtPlayer player, string oldNum, string newNum)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NumberExists), 6000);
                    return;
                }
                Regex rg = new Regex(@"^[a-z0-9]+$", RegexOptions.IgnoreCase);
                if (!rg.IsMatch(newNum))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.IncorrectInputNewNumber), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NewvNum, oldNum, newNum), 6000);
        ...
        public static void CMD_giveredsall(ExtPlayer player, int amount)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du kannst minimum 1 RC und maximum 500 RC verteilen", 3000);
                    return;
                }
                ...
                foreach (ExtPlayer foreachPlayer in NeptuneEvo.Character.Repository.GetPlayers())
                {
                    ...
                    UpdateData.RedBucks(foreachPlayer, correctValue, msg: "Verschickt RC");
                    Players.Phone.Messages.Repository.AddSystemMessage(foreachPlayer, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.RbIncome, amount, player.Name.Replace('_', ' ')), DateTime.Now);
                }

                NAPI.Chat.SendChatMessageToAll($"{CommandsAccess.AdminPrefixChat}{player.Name.Replace('_', ' ')} verteilt an alle Spieler {amount} Rockford-Coins.");
            ...
            try
            {
                ...
                if (targetCharacterData.BizIDs.Count > 0)
                {
                    try
                    {
                        ...
                        Trigger.SendChatMessage(player, $"Spieler besitzt {BusinessManager.BusinessTypeNames[biz.Type]} (ID:{biz.ID})");
                    }
                ...
                if (house != null)
                {
                    ...
                    {
                        Trigger.SendChatMessage(player, $"Spieler besitzt (ID{house.ID}) im Wert von ${house.Price} Klasse: '{HouseManager.HouseTypeList[house.Type].Name}'");
                        ...
                        foreach (string number in vehiclesNumber)
                        {
                            ...
                            Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.PlayerHaveCar, vehicleData.Model, number));
                        }
                    }
                    else
                    {
                        Trigger.SendChatMessage(player, $"Spieler zieht ins  (ID{house.ID}) zum {house.Owner} im Wert von ${house.Price} Klasse: '{HouseManager.HouseTypeList[house.Type].Name}'");
                ...
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.PersonNoHome));
                    var vehiclesNumber = VehicleManager.GetVehiclesCarNumberToPlayer(target.Name);
                    foreach (string number in vehiclesNumber)
                    {
                        var vehicleData = VehicleManager.GetVehicleToNumber(number);
        ...
        public static void CMD_checkId(ExtPlayer player, string target)
        {
            try
            {
                ...
                if (int.TryParse(target, out id))
                {
                    ...
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.CantFindPlayerWithId));
                }
                else
                {
                    int players = 0;
                    if (target.Length < 3)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Name3Symb), 6000);
                        return;
                    }
                    foreach (ExtPlayer foreachPlayer in NeptuneEvo.Character.Repository.GetPlayers())
                    {
                    ...
                    if (players == 0) Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.CantFindMan));
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_setDim(ExtPlayer player, int id, int dim)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerOnTestDrive), 6000);
                    return;
                }
                if (targetCharacterData.InsideHouseID != -1 && targetCharacterData.AdminLVL == 0) return;
                Trigger.Dimension(target, (uint)dim);
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) stellt Dimension({dim}) {target.Name} ({target.Value})");
        ...
        public static void CMD_checkDim(ExtPlayer player, int id)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerDimIs, UpdateData.GetPlayerDimension(target).ToString()), 4000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_setBizMafia(ExtPlayer player, int mafia)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouNotOnBiz), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MafiaOwnsBiz, mafia, biz.ID), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_newsimcard(ExtPlayer player, int id, int newnumber)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NewvNum, target.Name, newnumber), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_takeOffBusiness(ExtPlayer admin, int bizid, bool byaclear = false)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AdminRejectBiz), 6000);
            ...
            Label:
                ...
                Bank.Accounts.TryGetValue(biz.BankID, out var bizBalance);
                {
                    if (bizBalance != null)
                    {
                ...
                string logText = "";
                if (player == null) logText = $"player(`Account does not exist.` Name: {owner})";
                else logText = $"player({Main.PlayerUUIDs[owner]})";
                GameLog.Money($"server", logText, Convert.ToInt32(biz.SellPrice * 0.8), $"takeoffBiz({biz.ID})");
                Notify.Send(admin, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouRejectBiz, owner), 6000);
                if (!byaclear)
        ...
        public static void CMD_MoneyMultiplier(ExtPlayer player, int multi)
        {
            try
            {
                ...
                if (multi < 1 || multi > 5)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Es kann nur von 1 bis 5 eingestellt werden", 3000);
                    return;
                }
                ...
                */
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"MoneyMultiplier geändert in {multi}", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_expmultiplier(ExtPlayer player, int multi)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"ExpMultiplier geändert in {multi}", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_offlineDelFraction(ExtPlayer player, string name)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {name} aus der Fraktion entlassen", 3000);
                Manager.RemoveFractionMemberData(name);
                GameLog.Admin($"{player.Name}", $"delfrac", $"{name}");
                Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) hat {name} aus der Fraktion entlassen");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"{name} wurde aus der Fraktion entlassen", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_removeObject(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Der nächste Gegenstand den Sie nehmen wird gebannt!", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_unwarn(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Spieler hat keine verwarnungen", 3000);
                    return;
                }
                ...
                NAPI.Chat.SendChatMessageToAll("!{#DF5353}" + $"Administrator  {player.Name} hat von {target.Name} eine Verwarnung entfernt ({target.Value}).");

                //Chars.Repository.PlayerStats(target);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast von {target.Name} eine Verwarnung entfernt! Es verbleiben noch {targetCharacterData.Warns} Verwarnungen", 9000);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"Von dir wurde eine Verwarnung entfernt! Es verbleiben noch {targetCharacterData.Warns} Verwarnungen", 9000);
        ...
        public static void CMD_offunwarn(ExtPlayer player, string targetName)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Spieler war Online, deswegen wurde der offunwarn auf unwarn geändert", 3000);
                    return;
                }
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        if (warninfo == null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Fehler von aufheben einer Verwarnung. Bitte teile es einen Admin mit", 3000);
                            return;
                        }
                        if (warns <= 0)
                        {
                        ...
                        Trigger.SetMainTask(() =>
                        {
                            NAPI.Chat.SendChatMessageToAll("!{#DF5353}" + $"Administrator {sessionData.Name} hat eine Verwarnung vom {targetName} entfernt! Offline");
                            GameLog.Admin($"{sessionData.Name}", $"offUnwarn", $"{targetName}");
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du eine eine Verwarnung von {targetName} entfernt! Es verbleiben {warns} Verwarnungen", 3000);
                        });
        ...
        public static void CMD_respawnCars(ExtPlayer player)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) schickte alle Fahrzeuge zum spawnen ({countToSpawn} Stk.)");
                ...
                if (vehicleLocalData != null)
                {
                    switch (vehicleLocalData.Access)
                    {
                        ...
                        case VehicleAccess.DeliveryBike:
                            ...
                            return;
                        default:
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Dieses Fahrzeug kann nicht auf einen Spawnpunkt geschickt werden", 3000);
                            return;
                    }
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) schickt das Fahrzeug zum Spawn ({vehicle.Value})");
            ...
            try
            {
                ...
                foreach (ExtVehicle veh in vehs)
                {
                    ...
                    if (vehicleLocalData != null)
                    {
                        ...
                        Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) schickt das Fahrzeug zum Spawn ({veh.Value})");
        ...
        */
        ...
        public static void CMD_GetMyPromo(ExtPlayer player)
        {
            try
            {
                ...
                if (pcdata == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Promo-Code, welcher auf deinen Charakter benutzt wurde, wurde nicht gefunden", 6000);
                    return;
                }
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Dein Promo-Code wurde {pcdata.UsedTimes} benutzt.", 10000);
                switch (pcdata.RewardVipLvl)
                {
                    case 1:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Bonus: Silver VIP für {pcdata.RewardVipDays} Tage, {pcdata.RewardMoney}$ und {pcdata.RewardItems.Count} Gegenstände.", 10000);
                        break;
                    case 2:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Bonus: Gold VIP für {pcdata.RewardVipDays} Tage, {pcdata.RewardMoney}$ и {pcdata.RewardItems.Count} Gegenstände.", 10000);
                        break;
                    case 3:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Bonus: Platinum VIP für {pcdata.RewardVipDays} Tage, {pcdata.RewardMoney}$ и {pcdata.RewardItems.Count} Gegenstände.", 10000);
                        break;
                    case 4:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Bonus: Diamond VIP für {pcdata.RewardVipDays} Tage, {pcdata.RewardMoney}$ и {pcdata.RewardItems.Count} Gegenstände.", 10000);
                        break;
                    default:
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Bonus: {pcdata.RewardMoney}$ und {pcdata.RewardItems.Count} Gegenstände.", 10000);
                        break;
                }
                if (pcdata.DonatePercent != 0)
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Unter anderem bekommen Sie: {pcdata.DonatePercent * 100}% vom Donation Rockford-Coins auf den Account {pcdata.DonateLogin} ,wenn ein Spieler mit Ihrem Promo-Code sein Konto auffüllt (Beispiel: bei 1% für die Auffüllung durch einen Spieler von 100 RB erhalten Sie 1 RB).", 15000);
                    if (pcdata.DonateReceivedByStreamer != 0) Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie bekommen insgesamt {pcdata.DonateReceivedByStreamer} Rockford-Coins für andere Spieler.", 15000);
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Promo-Code erfolgreich neu geladen!", 3000);
                    });
                });
            }
            catch (Exception e)
        ...
        public static void CMD_BonuscmdSync(ExtPlayer player)
        {
            try
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Bonus-Code erfolgreich neu geladen!", 3000);
                });
            }
            catch (Exception e)
            {
        ...
        public static void CMD_EconomySync(ExtPlayer player)
        {
            try
            {
                ...
                Economy.Init();
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben die Server Wirtschaft erfolgreich aus der Datenbank neu geladen.", 5000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_FracCmdSync(ExtPlayer player, string cmd, byte fractionId, byte access)
        {
            try
            {
                ...
                /*if (Configs.FractionCommands[frac].ContainsKey(cmd))
                {
                    ...
                    MySQL.Query(mcmd);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben eine neue Berechtigung ({access}) на команду {cmd}", 3000);
                }
                else*/
                if (Configs.FractionWeapons[fractionId].ContainsKey(cmd))
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben eine neue Berechtigung ({access}) für Waffen/Muniton festgelegt {cmd}", 3000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Kein Zugriff in dieser Fraktion gefunden", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_setTerritoryColor(ExtPlayer player, int gangid)
        {
            try
            {
                ...
                if (shapeData == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie befinden sich in keiner der Gebiete", 3000);
                    return;
                }
                int terrid = shapeData.Index;
                if (!GangsCapture.gangPointsColor.ContainsKey(gangid))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Eine Gang mit dieser ID existiert nicht", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Das Gebiet {terrid} wird von {Fractions.Manager.FractionNames[gangid]} kontrolliert", 6000);
        ...
        public static void CMD_checkwanted(ExtPlayer player, int id)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sterne Anzahl - {stars}", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_fixcar(ExtPlayer player)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) repariert Fahrzeug ({vehicle.Value})");
        ...
        public static void CMD_fixcarID(ExtPlayer player, int id)
        {
            try
            {
                ...
                foreach (ExtVehicle veh in vehs)
                {
                    ...
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) repariert Fahrzeug mit der ID ({veh.Value})");
        ...
        public static void CMD_showPlayerOffStats(ExtPlayer player, string Name)
        {
            try
            {
                ...
                if (target != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Ein Spieler mit diesem Namen war Online, deswegen wurde der offstats auf stats geändert!", 6000);
                    return;
                }
                Trigger.SetTask(() =>
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"Ein Spieler mit den Namen: {Name} wurde im System nicht gefunden", 3000);
                        return;
                    }
                    ...
                    charData.Add(weddingName.Length > 5 ? weddingName : "ledig");//14
                    ...
                    Admin.AdminLog(characterData.AdminLVL, $"{sessionData.Name} ({sessionData.Value}) schaut offline Statistik {Name}");
                });
        ...
        public static void CMD_showPlayerStats(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= 6)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindPlayerWithId), 6000);
                        Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{player.Name} ({player.Value}) versucht deine Statistik zu sehen (/stats)", 3000);
                        return;
                    }
                }
                ...
                Trigger.ClientEvent(player, "client.inventory.Open");
                sessionData.LookingStats = true;
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) schaut die Statistik {target.Name} ({target.Value})");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_AllAdmins(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "=== ADMINS ONLINE ===");
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) überprüft die Onlineliste von Admins");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_AllMedias(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "=== MEDIA ONLINE ===");
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) kontrolliert die Onlineliste");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_Clear(ExtPlayer player, int id)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) entfernt die Fahndung von: {target.Name} ({target.Value})");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du entfernst die Fahndung von: {target.Name} ({target.Value})", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_Agl(ExtPlayer player, int id)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) stellt Lizenz an {target.Name} aus ({target.Value})");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sie stellen eine Lizenz an {target.Name} aus ({target.Value})", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_Clear Exception: {e.ToString()}");
        ...
        public static void CMD_additem(ExtPlayer player, string Name, int Value, string Data)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, $"~r~Sie geben sich {Chars.Repository.ItemsInfo[ItemId].Name}!");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_nalog(ExtPlayer player)
        {
            try
            {
                ...
                if (toggled)
                    Trigger.SendChatMessage(player, $"Steuersystem eingeschaltet!");
                else
                    Trigger.SendChatMessage(player, $"Steuersystem ausgeschaltet!");
            }
            catch (Exception e)
            {
                Log.Write($"CMD_goadditem Exception: {e.ToString()}");
        ...
        public static void CMD_setprod(ExtPlayer player, int id)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "~r~Die haben das Lager vollständig gefüllt!");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_changeHousePrice(ExtPlayer player, int newPrice)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du musst dich im Marker vom Haus befinden", 3000);
                    return;
                }
        ...
        public static void CMD_changeStock(ExtPlayer player, int fractionId, string item, int amount, bool add = false)
        {
            try
            {
                ...
                if (fractionData == null)
                {
                    Trigger.SendChatMessage(player, "~r~Ein Lager ist in dieser Fraktion nicht vorhanden");
                    return;
                }
                switch (item)
                {
                    ...
                    case "money":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Keine Rechte", 3000);
                            return;
                        }
                        ...
                        break;
                    default:
                        Trigger.SendChatMessage(player, "~r~mats - Mats");
                        Trigger.SendChatMessage(player, "~r~drugs - Drogen");
                        Trigger.SendChatMessage(player, "~r~medkits - Verbandskasten");
                        Trigger.SendChatMessage(player, "~r~money - Geld");
                        return;
                }
                fractionData.UpdateLabel();
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Lager erfolgreich geändert", 1500);
        ...
        public static void CMD_tpCoord(ExtPlayer player, string cords)
        {
            try
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        player.SendChatMessage("Fehler! Geben Sie die richtigen Koordinaten an");
                        return;
                    }
                }
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) teleport zu Koordinaten {resultCords[0]} {resultCords[1]} {resultCords[2]}");
        ...
        public static void CMD_SendToCreator(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (characterData.AdminLVL >= 6 || Admin.SendCreatorQueue.ContainsKey(target))
                {
                    ...
                    {
                        Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) schickt zum Creator {target.Name} ({target.Value})");
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Nur ein anderer Administrator kann das bestätigen!", 3000);
                            return;
                        }
                        Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) bestätigt die Anfrage zur Änderung {Admin.SendCreatorQueue[target].Item1}");
                        Admin.AdminsLog(Admin.SendCreatorQueue[target].Item2, $"{Admin.SendCreatorQueue[target].Item1} schickt zum Creator {target.Name} ({target.Value})");
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Sie haben die Anfrage bestätigt!", 3000);
                    }
                ...
                else
                {
                    ...
                    Trigger.SendToAdmins(2, $"{ChatColors.StrongOrange}[A] Anfrage von {player.Name} ({player.Value}) auf Charakteränderung {target.Name}. Zum bestätigen: /sendcreator {id}");
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    if (vehicleLocalData.Access == VehicleAccess.Personal)
                    {
                        ...
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Fahrzeug nach dem Respawn verfügbar", 3000);
                    }
                }
            }
            catch (Exception e)
        ...
        public static void CMD_setVehiclePetrol(ExtPlayer player, int fuel)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Es passen maximal 200 Liter rein!", 3000);
                    return;
                }
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) tankt das Fahrzeug ({player.Vehicle.Value},{fuel}liter)");
                }
            }
            catch (Exception e)
            {
        ...
        */
        ...
        public static void CMD_changeNamebyId(ExtPlayer player, int id, string newName)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Falsche Eingabe für neuen Namen, es fehlt das erforderliche Symbol!", 3000);
                    return;
                }
                if (Main.PlayerNames.Values.Contains(newName))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NameExists), 6000);
                    return;
                }
                string[] split = newName.Split("_");
                if (split[0].Length < 3 || split[1].Length < 3)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Name oder Vorname zu kurz!", 3000);
                    return;
                }
                Regex rg = new Regex(@"^[a-z]+$", RegexOptions.IgnoreCase);
                if (!rg.IsMatch(split[0]))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Vorname ist falsch!", 3000);
                    return;
                }
                if (!rg.IsMatch(split[1]))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Nachname ist falsch!", 3000);
                    return;
                }
                ...
                if (characterData.AdminLVL >= 5 || (Admin.ChangeNameQueue.ContainsKey(curient) && Admin.ChangeNameQueue[curient].Item1 == newName))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie können den Benutzernamen des Administrators mit höherer Ebene nicht ändern!", 3000);
                        return;
                    }
                    if (characterData.AdminLVL >= 5)
                    {
                        Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) geändert von{curient} auf {newName}");
                        GameLog.Admin($"{player.Name}", $"changeName({newName})", $"{curient}");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Name geändert!", 3000);
                    }
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Nur ein anderer Administrator kann bestätigen", 3000);
                            return;
                        }
                        Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) Ein anderer Administrator hat die Anfrage zur Änderung bestätigt {Admin.ChangeNameQueue[curient].Item2}");
                        Admin.AdminsLog(Admin.ChangeNameQueue[curient].Item3, $"{Admin.ChangeNameQueue[curient].Item2} geändert von {curient} auf {newName}");
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Sie haben die Anfrage zur Änderung des Namens bestätigt!", 3000);
                    }
                ...
                else
                {
                    ...
                    {
                        Admin.ChangeNameQueue.Remove(curient);
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Der alte Antrag auf Änderung des Namens für diesen Charakter wurde gelöscht", 3000);
                    }
                    Admin.ChangeNameQueue.Add(curient, (newName, player.Name, characterData.AdminLVL));
                    Trigger.SendToAdmins(2, $"{ChatColors.StrongOrange}[A] Anfrage von {player.Name} ({player.Value}) auf Name änderung {curient} -> {newName}. Zum bestätigen: /setname {curient} {newName}");
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_changeName(ExtPlayer player, string curient, string newName)
        {
            try
            {
                ...
                if (!newName.Contains('_') || newName.Contains(' '))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Falsche Eingabe des neuen Namens, das benötigte Zeichen fehlt!", 3000);
                    return;
                }
                if (Main.PlayerNames.Values.Contains(newName))
                {
                ...
                if (split[0].Length < 3 || split[1].Length < 3)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Falsche Eingabe des neuen Vorname!", 3000);
                    return;
                }
                Regex rg = new Regex(@"^[a-z]+$", RegexOptions.IgnoreCase);
                if (!rg.IsMatch(split[0]))
                ...
                if (!rg.IsMatch(split[1]))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Falsche Eingabe des neuen Nachname!", 3000);
                    return;
                }
                if (characterData.AdminLVL >= 6 || (Admin.ChangeNameQueue.ContainsKey(curient) && Admin.ChangeNameQueue[curient].Item1 == newName))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Es ist nicht möglich, den Benutzernamen des Administrators zu ändern, der höheren Rangs ist!", 3000);
                        return;
                    }
                    if (characterData.AdminLVL >= 6)
                    {
                        Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ändert name {curient} auf {newName} Offline");
                        GameLog.Admin($"{player.Name}", $"changeName({newName})", $"{curient}");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Nickname geändert", 3000);
                    }
                    else
                    {
                        if (!Admin.ChangeNameQueue.ContainsKey(curient)) return;
                        if (Admin.ChangeNameQueue[curient].Item2 == player.Name)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Es kann nur ein anderer Administrator bestätigen!", 3000);
                            return;
                        }
                        Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) Bestätigen Sie diese Anforderung zur Änderung des Administrators. {Admin.ChangeNameQueue[curient].Item2}");
                        Admin.AdminsLog(Admin.ChangeNameQueue[curient].Item3, $"{Admin.ChangeNameQueue[curient].Item2} ändert Name {curient} auf {newName} Offline");
                        GameLog.Admin($"{player.Name}", $"changeNameAccept", $"{Admin.ChangeNameQueue[curient].Item2}");
                        GameLog.Admin($"{Admin.ChangeNameQueue[curient].Item2}", $"changeName({newName})", $"{curient}");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Sie haben die Anfrage zur Änderung des Namens bestätigt!", 3000);
                    }
                ...
                else
                {
                    ...
                    {
                        Admin.ChangeNameQueue.Remove(curient);
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Der alte Antrag auf Namensänderung für diesen Charakter wurde gelöscht", 3000);
                    }
                    Admin.ChangeNameQueue.Add(curient, (newName, player.Name, characterData.AdminLVL));
                    Trigger.SendToAdmins(2, $"{ChatColors.StrongOrange}[A] Anfrage von {player.Name} ({player.Value}) auf änderung {curient} -> {newName}. zum bestätigen: /setnameoff {curient} {newName}");
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_startMatWars(ExtPlayer player)
        {
            try
            {
                ...
                if (MatsWar.isWar)
                {
                    Trigger.SendChatMessage(player, "~r~Krieg um Mats läuft bereits");
                    return;
                }
                MatsWar.startWar();
                Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} startet" + (characterData.Gender ? "" : "а") + " Krieg um Mats!");
        ...
        public static void CMD_Stopmatwars(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Trigger.SendChatMessage(player, "~r~Krieg um Mats hat noch nicht angefangen");
                    return;
                }
                MatsWar.endWar();
                Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} beendet" + (characterData.Gender ? "" : "а") + " Krieg um Mats");
        ...
        public static void CMD_giveLvl(ExtPlayer player, int id, int lvl)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie geben {target.Name} ({target.Value}) {lvl} Level", 3000);
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) gibt {target.Name} ({target.Value}) {lvl} Level");
        ...
        public static void CMD_replaceHousePrices(ExtPlayer player, int type, int newPrice)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, $"~r~Preis für Haus wurde festgelegt - {newPrice}");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_deleteHouseOwner(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie müssen sich auf dem Haus Marker befinden.", 3000);
                    return;
                }
                ...
                Trigger.SendChatMessage(player, $"~r~Sie haben den Eigentümer der Immobilie erfolgreich gelöscht {house.ID}");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_deleteThisAdminCar(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) löscht Admin Fahrzeug ({vehicle.Value})");
                        VehicleStreaming.DeleteVehicle(vehicle);
        ...
        public static void CMD_delMyCars(ExtPlayer player)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) alle Admin Fahrzeuge gelöscht");
        ...
        public static void ACMD_setfractun(ExtPlayer player, int cat = -1, int id = -1)
        {
            try
            {
                ...
                if (!player.IsInVehicle)
                {
                    Trigger.SendChatMessage(player, "Sie müssen in einem Fahrzeug der Fraktion sitzen, die Sie ändern möchten");
                    return;
                }
                var vehicle = (ExtVehicle)player.Vehicle;
                var vehicleLocalData = vehicle.GetVehicleLocalData();
                if (vehicleLocalData != null)
                {
                    if (vehicleLocalData.Access == VehicleAccess.Fraction)
                    {
                        ...
                        VehicleManager.FracApplyCustomization(vehicle, fractionData.Id);
                        Trigger.SendChatMessage(player, "Sie haben das Tuning dieses Autos für die Fraktion geändert");
                    }
                    else Trigger.SendChatMessage(player, "Sie müssen im Auto der Fraktion sitzen, die Sie ändern möchten");
                }
            }
            catch (Exception e)
            {
        ...
        public static void ACMD_setfracveh(ExtPlayer player, string vehname, int rank, int c1, int c2)
        {
            try
            {
                ...
                if (!player.IsInVehicle)
                {
                    Trigger.SendChatMessage(player, "Sie müssen im Auto der Fraktion sitzen, die Sie ändern möchten");
                    return;
                }
            ...
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    if (vehicleLocalData.Access == VehicleAccess.Fraction)
                    {
                        ...
                        if (isMessage) Trigger.SendChatMessage(player, "Sie haben die Informationen dieses Fahrzeugs für die Fraktion geändert");
                    }
                    else if (isMessage) Trigger.SendChatMessage(player, "Sie müssen im Auto der Fraktion sitzen, die Sie ändern möchten");
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_stopServer(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openDialog", "STOP_SERVER", $"Sind Sie sicher, dass Sie den Server neustarten möchten?");
            }
            catch (Exception e)
            {
                Log.Write($"CMD_stopServer Exception: {e.ToString()}");
        ...
        public static void CMD_SaveServer(ExtPlayer player)
        {
            try
            {
                ...
                Admin.SaveServer();
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Alle Daten wurden gespeichert", 1000);
        ...
        public static void CMD_inv(ExtPlayer player)
        {
            try
            {
                ...
                bool isHasData = NAPI.Data.HasEntityData(player, "OutgoingSyncDisabled");
                
                if (!isHasData)
                {
                    NAPI.Data.SetEntityData(player, "OutgoingSyncDisabled", false);
                }

                if (!NAPI.Data.GetEntityData(player, "OutgoingSyncDisabled"))
                {
                    NAPI.Data.SetEntityData(player, "OutgoingSyncDisabled", true);
                    //player.OutgoingSyncDisabled = true;
                    player.Position = new Vector3(99999f, 9999f, 9999f);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Du bist jetzt unsichbar", 3000);
                }
                else
                {
                    //player.OutgoingSyncDisabled = false;
                    NAPI.Data.SetEntityData(player, "OutgoingSyncDisabled", false);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Du bist wieder sichbar", 3000);
                }

                Trigger.ClientEvent(player, "clientSyncHandle", NAPI.Data.GetEntityData(player, "OutgoingSyncDisabled"));
            }
            catch (Exception e)
            {
        ...
        public static void CMD_Spawn(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (targetCharacterData.InsideHouseID == -1 && target.Dimension == 0 && targetCharacterData.DemorganTime <= 0 && targetCharacterData.ArrestTime <= 0 && !targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath && targetSessionData.SitPos == -1)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Spieler wurde an den Start gespawnt", 3000);
                    //Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"Administrator {player.Name} телепортировал Вас на спавн", 7000);
                    EventSys.SendCoolMsg(target, "Administrator", $"{player.Name}", $"Teleportiert euch zum Anfang Spawn", "", 10000);
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) schickt zum Anfang Spawn {target.Name} ({target.Value})");
                    GameLog.Admin($"{player.Name}", $"spawn", $"{target.Name}");
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Zurzeit nicht verfügbar", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_adminHeal(ExtPlayer player)
        {
            try
            {
                ...
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) erhöht seine HP");
        ...
        public static void CMD_createVehicle(ExtPlayer player, string name, int a, int b)
        {
            try
            {
                ...
                else
                {
                    if (characterData.AdminLVL <= 5)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ein Fahrzeug mit diesen Namen wurde nicht gefunden", 3000);
                        return;
                    }
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) erstellt Fahrzeug({name},{veh.Value})");
            ...
            try
            {
                ...
                if (count >= 1 && count <= 20)
                {
                    ...
                    if (vehHash != 0)
                    {
                        ...
                        Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) erstellt {count} Fahrzeug({name})");
                        GameLog.Admin($"{player.Name}", $"vehsCreate({count} {name})", $"");
                    }
                    else
                    {
                        if (characterData.AdminLVL <= 5) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ein Fahrzeug mit diesen Namen wurde nicht gefunden", 3000);
                        else
                        {
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Es können bis zu 20 Fahrzeuge erstellt werden", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void aclear(ExtPlayer player, string targetName, string reason)
        {
            try
            {
                ...
                if (!Main.PlayerNames.Values.Contains(targetName) || !Main.PlayerUUIDs.ContainsKey(targetName))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ein Spieler mit diesem Namen wurde nicht gefunden", 3000);
                    return;
                }
                ExtPlayer target = (ExtPlayer)NAPI.Player.GetPlayerFromName(targetName);
                if (target.IsCharacterData())
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Es ist nicht möglich, Verwarnungen vom Spieler, der sich im Spiel befindet zu entfernen", 5000);
                    return;
                }

                if (Main.stringGlobalBlock.Any(c => reason.Contains(c)))
                {
                    Trigger.SendToAdmins(1, $"{ChatColors.Red}[A] {sessionData.Name} ({sessionData.Value})  wurde von dem System aufgrund eines Verstoßes gegen die Bestimmungen entfernt: {reason}");
                ...
                Trigger.SetTask(() =>
                {
                    try
                    {
                        ...
                        if (result != null && result.Rows.Count != 0)
                        {
                            ...
                            {
                                Trigger.SendToAdmins(3, "!{#FF0000}" + $"[A] {sessionData.Name} ({sessionData.Value}) versucht zu Clearen {targetName} (offline), der höhere Administratorrechte hat.");
                                return;
                            }
                            ...
                            if (TBiz.Count >= 1 && TBiz[0] >= 1)
                            {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Business von {owner} entfernt", 3000);
                            }
                        }
                        else
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Konnte den Charakter nicht in der Datenbank finden", 3000);
                            return;
                        }
                        // CLEAR HOUSE
                        ...
                        if (result4 != null && result4.Rows.Count != 0)
                        {
                            ...
                            if (house != null)
                            {
                                if (FurnitureManager.HouseFurnitures.ContainsKey(hid))
                                {
                                    ...
                                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sie haben {targetName} aus dem Haus entfernt. Möbel auch", 3000);
                                }
                                house.ClearOwner();
                                Houses.Rieltagency.Repository.OnPayDay(new List<House>()
                                {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du hast das Eigentum von {targetName} enfernt", 4500);
                                //Chars.Repository.PlayerStats(player);
                            }
                        }
                        // CLEAR VEHICLES
                        ...
                        if (vehiclesNumber.Count > 0)
                        {
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Sie entfernen von {targetName} alle Fahrzeuge", 3000);
                        }
                        ...
                        Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{sessionData.Name} Dieser Code hat den Charakter vollständig zurückgesetzt {targetName}. Grund: {reason}");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben von {targetName} alles entfernt! Kleidung, Bargeld, Bankkonto", 3000);
        ...
        public static void CMD_ClearFraction(ExtPlayer player, byte fraction)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.Center, $"Sie haben mit der Säuberung der Fraktion begonnen {Manager.FractionNames[fraction]}!", 3000);
        ...
        public static void CMD_Carnumber(ExtPlayer player, string number)
        {
            try
            {
                ...
                if (number.Length > 8)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Die Anzahl der Buchstaben in dem Nummernschild darf 8 nicht überschreiten", 3000);
                    return;
                }
                var vehicleData = VehicleManager.GetVehicleToNumber(number);
                if (vehicleData != null)
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Kennzeichen: {number} | Model: {vehicleData.Model} | Besitzer: {vehicleData.Holder}", 6000);
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) überprüft den Besitzer des Fahrzeugs {number} ({vehicleData.Holder})");
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Es wurde kein Fahrzeug mit diesem Kennzeichen gefunden", 5000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_setWeather(ExtPlayer player, int weather)
        {
            try
            {
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ändert das Wetter({weather})");
        ...
        public static void CMD_setTime(ExtPlayer player, short hours, short minutes, short seconds)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Falsche Daten, Stunden 0-23, Minuten 0-59, Sekunden 0-59", 3000);
                    return;
                }
                Admin.TimeChanged = false;
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) stellt die Uhrzeit ein ({hours} {minutes} {seconds})");
                ...
                if (hours != -1 && minutes != -1 && seconds != -1)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Spielzeit eingefroren {hours}h. {minutes}m. {seconds}s. Um zurückzukehren, setzen Sie -1 -1 -1", 3000);
        ...
        public static void CMD_DelObjects(ExtPlayer player, string locationName)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openDialog", "DelObjects", $"Möchten Sie löschen {locationName}?");
        ...
        public static void CMD_teleport(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (targetCharacterData.AdminLVL >= 6)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindPlayerWithId), 6000);
                        Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{player.Name} ({player.Value}) hat versucht sich zu Ihnen zu teleportieren (/tp)", 3000);
                        return;
                    }
                }
                Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) teleportiert sich zu {target.Name} ({target.Value})", 1, "#636363");
        ...
        public static void CMD_Metpcar(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    if (veh.Value != id) continue;
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) Teleportiert Fahrzeug zu sich {veh.DisplayName} ({id})", 1, "#636363", hideAdminLevel: 9);
        ...
        public static void CMD_tpTocar(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    if (veh.Value != id) continue;
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) teleportiert sich zum Fahrzeug {veh.DisplayName} ({id})", 1, "#636363", hideAdminLevel: 9);
        ...
        public static void CMD_tpcarnumber(ExtPlayer player, string veh_number)
        {
            try
            {
                ...
                foreach (ExtVehicle veh in vehs)
                {
                    ...
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) teleportiert sich zum Fahrzeug {veh.DisplayName} (ID: {veh.Value}; Number: {veh.NumberPlate})", 1, "#636363", hideAdminLevel: 9);
                    NAPI.Entity.SetEntityPosition(player, veh.Position);
                    Trigger.Dimension(player, UpdateData.GetVehicleDimension(veh));
        ...
        public static void CMD_Setdimcar(ExtPlayer player, int id, int newDimensionNumber)
        {
            try
            {
                ...
                {
                    if (veh.Value != id) continue;
                    Admin.AdminsLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) ändert Dimension vom Fahrzeug {veh.DisplayName} ({id}) auf {newDimensionNumber}.", 1, "#636363", hideAdminLevel: 9);
        ...
        [Command(AdminCommands.Crimeban, "benutze: /crimeban [Spieler ID] [Grund]", GreedyArg = true)]
        public static void BanCrime(ExtPlayer player, int pid, string reason)
        {
            try
            {
                ...
                if (target != null && targetCharacterData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Spieler ist bereits bei Capture und Business Krieg gesperrt", 5000);
                        return;
                    }

                    Trigger.SendPunishment($"~r~{CommandsAccess.AdminPrefix}{player.Name} sperrt den Zugang zu Events wie Capture und Business Krieg {target.Name}({target.Value}) навсегда. Причина: {reason}.", target);
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ein Spieler mit dieser ID wurde nicht gefunden", 5000);
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.Uncrimeban, "benutze: /uncrimeban [Spieler ID]")]
        public static void Uncrimeban(ExtPlayer player, int pid)
        {
            try
            {
                ...
                if (target != null && targetCharacterData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Spieler ist nicht bei Capture und Business Krieg gesperrt", 5000);
                        return;
                    }

                    Trigger.SendToAdmins(1, $"~r~[A] {player.Name} ({player.Value}) entsperrt {target.Name} von Capture und Business Krieg ({target.Value})");
                    Trigger.SendChatMessage(target, $"Administrator {player.Name} entsperrt Sie von Capture und Business Krieg");
        ...
        [Command(AdminCommands.Offcrimeban, "benutze: /offcrimeban [Vor_Nachname] [Grund]", GreedyArg = true)]
                ...
                else
                {
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Spieler ist bereits bei Capture und Business Krieg gesperrt", 3000);
                                return;
                            }

                            Trigger.SendPunishment(
                                $"~r~{CommandsAccess.AdminPrefix}{sessionData.Name} beschränkt den Zugang von {name} bei Capture und Business Krieg. Grund: {reason}.");
        ...
        [Command(AdminCommands.Offuncrimeban, "benutze: /offuncrimeban [Vor_Nachname]")]
                ...
                else
                {
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            if (!character.IsBannedCrime)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Spieler ist nicht bei Capture und Business Krieg gesperrt", 3000);
                                return;
                            }

                            Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.UnblockBizAdmin, CommandsAccess.AdminPrefix, sessionData.Name, name));
        ...
        public static void CMD_flipveh(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    if (veh.Value != id) continue;
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) dreht das Fahrzeug {veh.DisplayName} ({id}) um");
        ...
        public static void CMD_LoadIPL(ExtPlayer player, string ipl)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "IPL wurde geladen: " + ipl);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_UnLoadIPL(ExtPlayer player, string ipl)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "Unload IPL: " + ipl);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_StartEffect(ExtPlayer player, string effect, int dur = 0, bool loop = false)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "Effekt eingeschaltet: " + effect);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_StopEffect(ExtPlayer player, string effect)
        {
            try
            {
                ...
                Trigger.SendChatMessage(player, "Effekt ausgeschaltet: " + effect);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_offlineJailTarget(ExtPlayer player, string targetName, int time, string reason)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Spieler war Online, deswegen wurde offjail auf jail geändert", 3000);
                    return;
                }
                if (time < 5 || time > 10080)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Du kannst nicht mehr als 10080 Minuten Jail geben", 3000);
                    return;
                }
                if (Main.stringGlobalBlock.Any(c => reason.Contains(c)))
                {
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} steckt {targetName} ins Knast! Dauer: {time} {deTimeMsg}. Grund: {reason}");
        ...
        public static void CMD_Offunjail(ExtPlayer player, string targetName)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Spieler war Online, deswegen wurde offunjail auf unjail geändert", 3000);
                    return;
                }
                int targetUuid = Main.PlayerUUIDs[targetName];
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} holt {targetName.Replace('_', ' ')} aus dem Knast raus");
        ...
        public static void CMD_offlineWarnTarget(ExtPlayer player, string targetName, string reason)
        {
            try
            {
                ...
                if (target.IsCharacterData())
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Spieler war Online, deswegen wurde offwarn auf warn geändert", 3000);
                    return;
                }
                if (Main.stringGlobalBlock.Any(c => reason.Contains(c)))
                {
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        {
                            Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {sessionData.Name} ({sessionData.Value}) hat versucht {targetName} zu Bannen (offline)! Dieser hat einen höheren Rang als der Admin");
                            return;
                        }
                        ...
                        if (memberFractionData != null)
                        {
                            Fractions.Table.Logs.Repository.AddOffLogs(memberFractionData.Id, targetName, targetUuid,
                                FractionLogsType.UnInvite, "bekommt eine Verwarnung");
                        ...
                        Trigger.SetMainTask(() =>
                        {
                            ...
                            Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{sessionData.Name} gibt {targetName} eine Verwarnung | {warns}/3. Grund: {reason}");
        ...
        public static void CMD_pritonbanTarget(ExtPlayer player, int id)
        {
            try
            {
                ...
                Trigger.SendPunishment($"{CommandsAccess.AdminPrefix}{player.Name} verhaftet {targetPlayer.Name}({targetPlayer.Id})");
        ...
        public static void CMD_offredbaks(ExtPlayer player, string name, int amount)
        {
            try
            {
                ...
                if (target != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Spieler war Online, deswegen wurde offgivereds auf givereds geändert", 3000);
                    return;
                }
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du gibst {name} {amount} Rockford-Coins", 3000);
        ...
        public static void CMD_ALog(ExtPlayer player)
        {
            try
            {
                ...
                if (adminConfig.ALog)
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Admin Log eingeschaltet", 3000);
                else
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Admin Log ausgeschaltet", 3000);
        ...
        public static void CMD_ELog(ExtPlayer player)
        {
            try
            {
                ...
                if (adminConfig.ELog)
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Error Log eingeschaltet", 3000);
                else
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Error Log ausgeschaltet", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_WinLog(ExtPlayer player)
        {
            try
            {
                ...
                if (adminConfig.WinLog)
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Roulette Win Log eingeschaltet", 3000);
                else
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Roulette Win Log ausgeschaltet", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_NickHistory(ExtPlayer player, int id, bool uuid = false)
        {
            try
            {
                ...
                if (DateTime.Now < sessionData.TimingsData.NextNHistory)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Es ist nicht erlaubt, diesen Befehl öfter als einmal alle 5 Sekunden zu verwenden", 3000);
                    return;
                }
                    ...
                    if (targetSessionData != null && targetCharacterData != null)
                    {
                        Trigger.SetTask(() =>
                        {
                            try
                            {
                                ...
                                if (result == null || result.Rows.Count == 0)
                                {
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Spieler hat noch nie seinen Nicknamen geändert", 3000);
                                    return;
                                }
                                else
                                {
                                    Admin.AdminLog(characterData.AdminLVL, $"{sessionData.Name} ({sessionData.Value}) Überprüft die Namenshistorie {targetSessionData.Name} ({targetSessionData.Value})");
                                    sessionData.TimingsData.NextNHistory = DateTime.Now.AddSeconds(5);
                ...
                else if (uuid)
                {
                    if (Main.PlayerNames.ContainsKey(id))
                    {
                        Trigger.SetTask(() =>
                        {
                            try
                            {
                                ...
                                if (result == null || result.Rows.Count == 0)
                                {
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Der Spieler hat entweder nie seinen Nicknamen geändert oder eine solche Passnummer existiert nicht", 3000);
                                    return;
                                }
                                else
                                {
                                    Admin.AdminLog(characterData.AdminLVL, $"{sessionData.Name} ({sessionData.Value}) überprüft die Änderungshistorie von Namen anhand der UUID {id}");
                                    sessionData.TimingsData.NextNHistory = DateTime.Now.AddSeconds(5);
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoPlayerWithPass), 6000);
                        return;
                    }
                }
            }
        ...
        public static void CMD_setOfflineVip(ExtPlayer player, int rank, ushort days, string login)
        {
            try
            {
                ...
                if (rank > 5 || rank < 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantGiveThatVipLvl), 6000);
                    return;
                }
                login = login.ToLower();
                ...
                if (target != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucGiveVipOff), 6000);
                    return;
                }
                ...
                Trigger.SetTask(async () =>
                {
                    try
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucGiveVip, rank, days, login), 6000);
        ...
        public static void CMD_leaveFraction(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VipForUvalFamily), 5000);
                    return;
                }
        ...
        public static void LeaveFractionOrg(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (index == 0)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LeaderCantUval), 5000);
                        return;
                    }
                    Manager.sendFractionMessage(memberFractionData.Id, "!{#FF8C00}[F] " + $"{player.Name} ({player.Value}) hat aus eigenem Wunsch gekündigt", true);
                    Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.UnInvite, LangFunc.GetText(LangType.De, DataName.SelfUval));

                    player.RemoveFractionMemberData();
                    player.ClearAccessories();
                    Customization.ApplyCharacter(player);

                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouLeaveFraction), 6000);
                }
                else if (index == 1)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OwnerCantLeave), 5000);
                        return;
                    }

                    NeptuneEvo.Organizations.Player.Repository.RemoveOrganizationMemberData(organizationData.Id, player.GetUUID());

                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouLeftFamily), 6000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_ResetPassword(ExtPlayer player, string new_password)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PassRestrcSymbols), 6000);
                    return;
                }
                Accounts.NewPassword.Repository.changePassword(player, new_password);
                Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PassChanged), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_checkPrisonTime(ExtPlayer player)
        {
            try
            {
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                if (characterData.ArrestTime >= 1) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du musst noch {Convert.ToInt32(characterData.ArrestTime / 60.0)} minuten sitzten", 3000);
                else if (characterData.DemorganTime >= 1)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Spezielle Strafe wird in {Convert.ToInt32(characterData.DemorganTime / 60.0)} minuten ablaufen", 5000);
                    if (!characterData.DemorganInfo.Admin.Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du wurdest von {characterData.DemorganInfo.Admin} ins Knast gesteckt! Grund: {characterData.DemorganInfo.Reason}", 10000);
                }
                else if (characterData.Warns >= 1)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Dieser Account hat aktuell {characterData.Warns} Verwarnungen", 5000);
                    if (!characterData.WarnInfo.Admin[0].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Verwarnung (1/3) ausgestellt {characterData.WarnInfo.Admin[0]} Grund: {characterData.WarnInfo.Reason[0]}", 10000);
                    if (characterData.Warns >= 2 && !characterData.WarnInfo.Admin[1].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Verwarnung (2/3) ausgestellt {characterData.WarnInfo.Admin[1]} Grund: {characterData.WarnInfo.Reason[1]}", 10000);
                    if (characterData.Warns >= 3 && !characterData.WarnInfo.Admin[2].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Verwarnung (3/3) ausgestellt {characterData.WarnInfo.Admin[2]} Grund: {characterData.WarnInfo.Reason[2]}", 10000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_pcheckPrisonTime(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (targetCharacterData.Unmute > 0)
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"{target.Name} ist noch {targetCharacterData.Unmute / 60} minuten gemutet", 3000);

                if (targetCharacterData.ArrestTime >= 1)
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"{target.Name} ist noch {Convert.ToInt32(targetCharacterData.ArrestTime / 60.0)} minuten im Knast", 3000);

                if (targetCharacterData.DemorganTime >= 1)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Spezielle Strafe von {target.Name} wird in {Convert.ToInt32(targetCharacterData.DemorganTime / 60.0)} minuten ablaufen", 5000);
                    if (!targetCharacterData.DemorganInfo.Admin.Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Его посадил {targetCharacterData.DemorganInfo.Admin} Grund: {targetCharacterData.DemorganInfo.Reason}", 10000);
                }

                if (targetCharacterData.Warns >= 1)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Dieser Account {target.Name} hat aktuell {targetCharacterData.Warns} Verwarnungen", 5000);
                    if (!targetCharacterData.WarnInfo.Admin[0].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Verwarnung (1/3) ausgestellt {targetCharacterData.WarnInfo.Admin[0]} Grund: {targetCharacterData.WarnInfo.Reason[0]}", 10000);
                    if (targetCharacterData.Warns >= 2 && !targetCharacterData.WarnInfo.Admin[1].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Verwarnung (2/3) ausgestellt {targetCharacterData.WarnInfo.Admin[1]} Grund: {targetCharacterData.WarnInfo.Reason[1]}", 10000);
                    if (targetCharacterData.Warns >= 3 && !targetCharacterData.WarnInfo.Admin[2].Equals("-1")) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Verwarnung (3/3) ausgestellt {targetCharacterData.WarnInfo.Admin[2]} Grund: {targetCharacterData.WarnInfo.Reason[2]}", 10000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_pMutePlayer(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoGaggedId), 6000);
                    return;
                }
        ...
        public static void CMD_gov(ExtPlayer player, string msg)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouMutedMins, characterData.Unmute / 60), 6000);
                    return;
                }
                if (DateTime.Now < sessionData.TimingsData.NextGlobalChat)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GlobalBlocked), 4500);
                    return;
                }
                ...
                if (Main.stringGlobalBlock.Any(c => testmsg.Contains(c)))
                {
                    ...
                    Trigger.SendToAdmins(3, $"{ChatColors.StrongOrange}[A] {player.Name} ({player.Value}) hat versucht in die Staatliche Fraktion zu schreiben: {msg}");
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RestrictedWordsTableNews), 15000);
                    return;
                }

                NAPI.Chat.SendChatMessageToAll($"~y~[Staatliche News] {player.Name}: {msg}");
        ...
        public static void CMD_forget(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HaveNoFriends), 6000);
                    return;
                }

                Trigger.ClientEvent(player, "openDialog", "FORGET_FRIENDS", LangFunc.GetText(LangType.De, DataName.Forget));
            }
            catch (Exception e)
            {
        ...
        public static void CMD_report(ExtPlayer player, string message)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MsgLonger5Shorter150), 6000);
                    return;
                }
                ...
                if (DateTime.Now < nextReport)
                {
                    ...
                    if (ticks >= 1)
                    {
                        ...
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NextReportCooldown, g.Minute, g.Second), 6000);
                        return;
                    }
                }
                if (characterData.Unmute >= 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoReportMute), 6000);
                    return;
                }
                if (ReportSys.Reports.Values.FirstOrDefault(p => p.Author == player.Name && !p.Status) != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouInQueueReport), 5000);
                    return;
                }
        ...
        public static void CMD_ejectTarget(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouNotInCarOnVehPlace), 6000);
                    return;
                }
                if (!target.IsInVehicle || player.Vehicle != target.Vehicle) return;
                VehicleManager.WarpPlayerOutOfVehicle(target);

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouVikinylPlayer, target.Value), 6000);
                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerVikinylYou, player.Value), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void AcceptDice(ExtPlayer player, ExtPlayer target, int price) // player - тот, кому предложили, target - кто предлагал
        {
            try
            {
                ...
                if (characterData == null)
                {
                    if (targetSessionData != null && targetCharacterData != null)
                    {
                        ...
                        Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameCancelledLeft), 6000);
                    }
                    return;
                }
                if (targetCharacterData == null)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameCancelledLeft), 6000);
                    return;
                }
                if (player != targetSessionData.DiceData.Target || target != sessionData.DiceData.Target)
                {
                    targetSessionData.DiceData = new DiceData();
                    sessionData.DiceData = new DiceData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameCancelledSboy), 6000);
                    Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameCancelledSboy), 6000);
                    return;
                }
                ...
                if (!characterData.InCasino || !targetCharacterData.InCasino)
                {
                    targetSessionData.DiceData = new DiceData();
                    sessionData.DiceData = new DiceData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameCancelCuzNoCasino), 6000);
                    Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameCancelCuzNoCasino), 6000);
                    return;
                }
                if (sessionData.IsCasinoGame != null || targetSessionData.IsCasinoGame != null)
                {
                    targetSessionData.DiceData = new DiceData();
                    sessionData.DiceData = new DiceData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameCancelled), 6000);
                    Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameCancelled), 6000);
                    return;
                }
                int number1 = Main.rnd.Next(2, 13); // Выпавший номер для target
                int number2 = Main.rnd.Next(2, 13); // Выпавший номер для player
                RPChat("sme", target, LangFunc.GetText(LangType.De, DataName.SmeDice, number1));
                RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.SmeDice, number2));
                if (number1 == number2)
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SameNumbers), 4000);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SameNumbers), 4000);
                ...
                else
                {
                    if (price == sessionData.DiceData.Money && price == targetSessionData.DiceData.Money)
                    {
                        if (number1 > number2) // Выиграл target
                        {
                            ...
                            Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceWin, MoneySystem.Wallet.Format(price)), 5000);
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceLose, MoneySystem.Wallet.Format(price)), 5000);
                        }
                        else if (number2 > number1) // Выиграл player
                        {
                            ...
                            Wallet.Change(player, price);
                            Wallet.Change(target, -price);
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceWin, MoneySystem.Wallet.Format(price)), 5000);
                            Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceLose, MoneySystem.Wallet.Format(price)), 5000);
                        }
                        targetSessionData.DiceData = new DiceData();
                        sessionData.DiceData = new DiceData();
                    }
            ...
            catch (Exception e)
            {
                ...
                {
                    sessionData.DiceData = new DiceData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SystemErrorGame), 6000);
                }
                if (target.IsCharacterData())
                {
                    var targetSessionData = target.GetSessionData();
                    if (targetSessionData == null) return;
                    targetSessionData.DiceData = new DiceData();
                    Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SystemErrorGame), 6000);
                }
            }
        }
        ...
        public static void callDice(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceMinMax, Main.MinDice, Main.MaxDice), 6000);
                        return;
                    }
                    if (characterData.LVL < 5)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Function5lvl), 6000);
                        return;
                    }
                    if (targetCharacterData.LVL < 5)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerCantUseThis), 6000);
                        return;
                    }
                    if (!characterData.InCasino || !targetCharacterData.InCasino)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceCasinoOnly), 6000);
                        return;
                    }
                    if (sessionData.IsCasinoGame != null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceErrorCasino, sessionData.IsCasinoGame), 6000);
                        return;
                    }
                    if (targetSessionData.IsCasinoGame != null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerDiceErrorCasino, targetSessionData.IsCasinoGame), 6000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceAlreadyPlayer), 6000);
                        return;
                    }
                    if (sessionData.DiceData.Target != null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceAlreadyYou), 6000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DiceOffer, target.Value, MoneySystem.Wallet.Format(price)), 6000);
                    Trigger.ClientEvent(target, "openDialog", "DICE_PLAY", LangFunc.GetText(LangType.De, DataName.DiceOffers, player.Value, MoneySystem.Wallet.Format(price)));
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoDice), 6000);
                    return;
                }
            }
            catch (Exception e)
        ...
        public static void CMD_go(ExtPlayer player, int sender_id)
        {
            try
            {
                ...
                if (memberFractionData.Id >= 6 && memberFractionData.Id <= 9 || memberFractionData.Id == (int)Fractions.Models.Fractions.ARMY || memberFractionData.Id == (int)Fractions.Models.Fractions.LSNEWS || memberFractionData.Id == (int)Fractions.Models.Fractions.SHERIFF)
                {
                    ...
                    string numberInfo = characterData.Sim == -1 ? "" : $" Telefonnummer für Kontakt: {characterData.Sim}";

                    Manager.sendFractionMessage(memberFractionData.Id, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.GovReactToCall, player.Name, player.Value), true);
                    Trigger.SendChatMessage(target, LangFunc.GetText(LangType.De, DataName.GovSucReactToCall, player.Name, player.Value) + numberInfo);
        ...
        public static void CMD_inCar2(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (vehicle == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoCarsNear), 6000);
                    return;
                }
                ExtPlayer target = Main.GetPlayerByID(id);
                var targetSessionData = target.GetSessionData();
        ...
        public static void CMD_code(ExtPlayer player, string code)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Need3Lvl), 6000);
                    return;
                }

                if (code.Length > 32)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CodeTooLong), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CodeExists), 6000);
                    return;
                }

                sessionData.TempNewRefCode = code;
                Trigger.ClientEvent(player, "openDialog", "CREATE_REF_CODE", LangFunc.GetText(LangType.De, DataName.CodeCreateYes));
            }
            catch (Exception e)
            {
        ...
        public static void CreateRefCode(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucRefCreated), 5000);

                GameLog.AddInfo($"(ref_code) player({characterData.UUID}) {code}");

                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.RefCreate, code), DateTime.Now);

                UpdateData.RedBucks(player, -228, msg: $"Referral Code erstellung {code}");
                ...
                string message = "!{#636363}[A] " + $"{player.Name} ({player.Value}) hat einen Referral Code erstellt: " + code;
        ...
        public static void CMD_action(ExtPlayer player, string msg)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ActionIsNear), 6000);
                    return;
                }

                if (msg.Length > 60)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MsgTooLong), 6000);
                    return;
                }
                ...
                string message = "!{#636363}[A] " + $"{player.Name} ({player.Value}) schrieb in Action: " + msg;
                Trigger.SendToAdmins(1, message);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_delaction(ExtPlayer player)
        {
            try
            {
                ...
                if (ActionLabels.ContainsKey(player.Value))
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ActionDeleted), 6000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ActionNotCreated), 6000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_adelactionall(ExtPlayer player)
        {
            try
            {
                ...
                if (ActionLabels.Count >= 1)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouDeletedActions), 6000);
                    Admin.AdminLog(characterData.AdminLVL, LangFunc.GetText(LangType.De, DataName.AdminYouDeletedActions, player.Name, player.Value));
        ...
        public static void CMD_adelaction(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (ActionLabels.ContainsKey(index))
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AdminDeleteAction), 6000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void CMD_chatRoll(ExtPlayer player, int first, int second)
        {
            try
            {
                ...
                if (first < 0 || second < 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Der minimale Wert ist 0.", 3000);
                    return;
                }
                if (first >= 65536 || second >= 65536)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Das maximale Wert der Zahlen ist 65535", 3000);
                    return;
                }
                if (second < first)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Die zweite Zahl sollte größer als die erste sein", 3000);
                    return;
                }
        ...
        private static void Try(ExtPlayer sender, string message)
        {
            try
            {
                ...
                if (characterData.Unmute > 0)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouMutedMins, characterData.Unmute / 60), 6000);
                    return;
                }
                ...
                switch (result)
                {
                    ...
                    case 2:
                        ...
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "try", "!{#E066FF}{name} " + message + " | !{#277C6B}" + " erfolgreich", new int[] { sender.Value });
                    ...
                    default:
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f,
                                     UpdateData.GetPlayerDimension(sender)))
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "try", "!{#E066FF}{name} " + message + " | !{#FF0707}" + " fehlgeschlagen", new int[] { sender.Value });
        ...
        private static void Roll(ExtPlayer sender, int first, int second)
        {
            try
            {
                ...
                foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, UpdateData.GetPlayerDimension(sender)))
                    Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "roll", "!{#FF8C00}{name} würfelt " + result + " | !{#e69500}" + first + " - " + second, new int[] { sender.Value });
            }
            catch (Exception e)
            {
                ...
                switch (cmd)
                {
                    ...
                    case "todo":
                        if (message.IndexOf('*') >= 0)
                        {
                            ...
                            if (msg.Length >= 1 && action.Length >= 1)
                            {
                                ...
                                foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, UpdateData.GetPlayerDimension(sender)))
                                    Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "todo", msg + ",!{#E066FF} - sagt" + genderCh + " {name}, " + action, names);
                            }
                        }
                        return;
                    case "do":
                    ...
                    case "s":
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 30f,
                                     UpdateData.GetPlayerDimension(sender)))
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "s", "{name} schreit: " + message, names);
                    ...
                    case "t":
                        ...
                        foreach (ExtPlayer foreachPlayer in NeptuneEvo.Character.Repository.GetPlayers())
                        {
                            ...
                            {
                                if (foreachSessionData.WorkData.OnWork && foreachPlayer.IsInVehicle) Trigger.SendChatMessage(foreachPlayer, $"~y~[T] [{sender.Name}]: {message}");
                            }
                        }
                        return;
                    case "w":
                        ...
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, UpdateData.GetPlayerDimension(sender)))
                        {
                            var foreachCharacterData = foreachPlayer.GetCharacterData();
                            if (foreachCharacterData == null) continue;
                            if (foreachPlayer.IsCharacterData() && foreachCharacterData.AdminLVL >= 1) Trigger.SendChatMessage(foreachPlayer, "!{#C5C7C7}" + $"[flüstert] {sender.Name} ({sender.Value}): {message}");
                            else if (foreachPlayer != sender && foreachPlayer != target) Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "w", "!{#C5C7C7}{name} flüstert {name}", names);
                        }
                        Trigger.SendChatMessage(sender, "!{#C5C7C7}" + $"[flüstert] {sender.Name.Replace('_', ' ')} ({sender.Value}): {message}");
                        ...
                        if (targetFriends.ContainsKey(sender.Name))
                        {
                            ...
                            {
                                Trigger.SendChatMessage(target, "!{#C5C7C7}" + $"[flüstert] {sender.Name.Split('_')[0]} ({sender.Value}): {message}");
                            }
                            else
                            {
                                Trigger.SendChatMessage(target, "!{#C5C7C7}" + $"[flüstert] {sender.Name.Replace('_', ' ')} ({sender.Value}): {message}");
                            }
                        }
                        else Trigger.SendChatMessage(target, "!{#C5C7C7}" + $"[flüstert] " + (characterData.Gender ? "Unbekannter" : "Unbekannte") + $" ({sender.Value}): {message}");
