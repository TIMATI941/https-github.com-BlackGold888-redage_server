...
namespace NeptuneEvo.Core
{
    class CarRoom : Script
    {
        ...
        private static float TestDriveRotation = 0.031053063f;
        private static Vector3[] TestDrivePositions = new Vector3[1] //TESTDRIVE //PROBEFAHRT
        {
            new Vector3(-346.61343, -1359.156, 31.88962),
        ...
        public static void enterCarroom(ExtPlayer player)
        {
            try
            {
                ...
                characterData.ExteriorPos = new Vector3(player.Position.X, player.Position.Y, player.Position.Z);
        ...
        private static IReadOnlyDictionary<string, Color> carColors = new Dictionary<string, Color>
        {
            ...
            { "Hellblau", new Color(0, 205, 255) },
            { "Blau", new Color(0, 0, 230) },
            { "Violett", new Color(190, 60, 165) },
        };
        ...
        public static void OpenCarromMenu(ExtPlayer player, Business biz)
        {
            try
            {
                ...
                if (biztype == 15) biztype = 0; //TODO: Luxure autoshop car list undefined
                else biztype -= 2;
        ...
        public static void RemoteEvent_carroomBuy(ExtPlayer player, string vName, string color, int id)
        {
            try
            {
                ...
                if (id == 1) // Для себя
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie können nicht noch mehr Fahrzeuge haben", 4500);
                    ...
                    if (prod == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Dieses Fahrzeug ist nicht auf Lager", 4500);
                        RemoteEvent_carroomCancel(player);
                        return;
                    }
                    ...
                    if (house != null)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie haben die maximale Anzahl von Fahrzeugen die Ihr Wohnsitz unterstützt.", 4500);
                            RemoteEvent_carroomCancel(player);
                            return;
                        }
                    }
                    if (biz.IsOwner())
                    {
                        if (biz.Type == 15)
                        {
                            if (prod.Lefts != 0)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Dieses Fahrzeug ist nicht auf Lager", 3000);
                                    RemoteEvent_carroomCancel(player);
                                    return;
                                }
                    ...
                    Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.YouBuyCarV2, vName, prod.Price), DateTime.Now);
                }
                else if (id == 2) // В организацию
                {
                    if (!player.IsOrganizationAccess(RankToAccess.OrgBuyCars))
                    {
                        RemoteEvent_carroomCancel(player); return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Ihre Famillie hat die maximale Anzahl an Fahrzeugen erreicht.", 6000);
                        if (upgraded <= 1) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie können die Anzahl der Fahrzeuge erhöhen, indem Sie ein Familien-Upgrade kaufen.!", 6000);
                        RemoteEvent_carroomCancel(player);
                        return;
                    }
                    ...
                    Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.YouBuyCarV, vName, vNumber, price), DateTime.Now);
                    Organizations.Table.Logs.Repository.AddLogs(player, OrganizationLogsType.BuyCar, $"kauft {vName} ({vNumber})");
                }
        ...
        private static void OnAutoroomBuy(ExtPlayer player, string vName, string color, int id, bool isDonate)
        {
            try
            {
                ...
                if (id == 1) // Для себя
                {
                    if (!VehicleModel.AirAutoRoom.isAirCar(vName))
                    {
                        ...
                        if (vehiclesCount >= Houses.GarageManager.MaxGarageCars)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                $"Sie können nicht noch mehr Fahrzeuge haben", 3000);
                            RemoteEvent_carroomCancel(player);
                            return;
                        }
                        ...
                        if (house != null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    $"Sie haben die maximale Anzahl von Fahrzeugen die Ihr Wohnsitz unterstützt.",
                                    3000);
                                RemoteEvent_carroomCancel(player);
                                return;
                            }
                    ...
                    Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.YouBuyCarV3, vName), DateTime.Now);
                }
                else if (id == 2) // В организацию
                {
                    ...
                    if (VehicleModel.AirAutoRoom.isAirCar(vName))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie können dieses Fahrzeug nicht für die Organisation kaufen.", 4500);
                        return;
                    }
                    ...
                    if (organizationData.Vehicles.Count >= maxcars)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Ihre Familie hat bereits die maximale Anzahl an Fahrzeugen.", 6000);
                        if (upgraded <= 1) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie können die Anzahl der Fahrzeuge erhöhen, indem Sie ein Upgrade für die Familie kaufen!", 6000);
                        RemoteEvent_carroomCancel(player);
                        return;
                    }
                    ...
                    Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.YouBuyCarV, vName, vNumber, vehiclePrice), DateTime.Now);
                    Organizations.Table.Logs.Repository.AddLogs(player, OrganizationLogsType.BuyCar, $"kauft {vName} ({vNumber})");
                }
                ...
                if (!isDonate)
                    ...
                    UpdateData.RedBucks(player, -vehiclePrice, msg: LangFunc.GetText(LangType.De, DataName.PremCarBuy, vName, vehiclePrice));
                //Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.PremCarBuy, vName), DateTime.Now.AddSeconds(10));
                RemoteEvent_carroomCancel(player);
            }
            catch (Exception e)
            {
        ...
        public static void RemoteEvent_testDrive(ExtPlayer player, string vName, string color, byte type)
        {
            try
            {
                ...
                else
                {
                    ...
                    positionData.Item2.Z = -TestDriveRotation;
                ...
                NAPI.Task.Run(() =>
                {
                    ...
                    {
                        if (!player.IsCharacterData()) return;
                        if (veh == null || !veh.Exists) return; //!sessionData.TestDriveVehicle ||
        ...
        public static void timer_exitVehicle(ExtPlayer player, bool quit = false)
        {
            if (OnExitTestDrive (player) && !quit)
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Die Zeit für eine Probefahrt ist abgelaufen.", 4500);
        }
        ...
        public void onPlayerExitVehicleHandler(ExtPlayer player, ExtVehicle vehicle)
        {
            if (OnExitTestDrive (player))
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Probefahrt beendet! Sie haben das Fahrzeug verlassen", 4500);
                        ...
                        if (sessionData.TempBizID != -1 && BusinessManager.BizList.ContainsKey(sessionData.TempBizID))
                        {
                            ...
                            if (!isDeath)
                                ...
                                NAPI.Player.SpawnPlayer(player,
                                    new Vector3(biz.EnterPoint.X, biz.EnterPoint.Y, biz.EnterPoint.Z + 1.5)); //SPAWN POS NACH PROBEFAHRT 
                        }
                    }
