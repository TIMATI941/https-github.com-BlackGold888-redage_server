    ...
    class SafeZones : Script
    {
        ...
        public static void CMD_SZState(ExtPlayer player, int sz)
        {
            try
            {
                ...
                if (!state)
                    Trigger.SendToAdmins(2, $"{Chars.Models.ChatColors.StrongOrange}[A] {player.Name} ({player.Value}) Angriff in der SafeZone ausgeschaltet #{sz}");
                else 
                    Trigger.SendToAdmins(2, $"{Chars.Models.ChatColors.StrongOrange}[A] {player.Name} ({player.Value}) Angriff in der SafeZone eingeschaltet #{sz}");
