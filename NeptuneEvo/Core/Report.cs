    ...
    class ReportSys : Script
    {
                    ...
                    if (someone == null)
                    {
                        foreach (ExtPlayer foreachPlayer in Main.AllAdminsOnline)
                        {
                            try
                            {
                                ...
                                string authorName;
                                if (authorCharacterData == null) authorName = $"{Author}[Verlassen]";
        ...
        public static void ReportTake(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du hast bereits einen Report offen", 3000);
                    return;
                }
        ...
        public static void ReportFunc(ExtPlayer player, int ID, string funcName)
        {
            try
            {
                ...
                if (!target.IsCharacterData()) Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Spieler wurde nicht gefunden", 3000);
                else
                {
                    switch (funcName)
                    {
        ...
        public static void ReportSend(ExtPlayer player, int ID, string answer)
        {
            try
            {
                ...
                {
                    Trigger.SendChatMessage(player, "Diese Beschwerde kann man nicht mehr ändern.");
                    Remove(ID, player);
                }
            }
            catch (Exception e)
        ...
        public static void AddReport(ExtPlayer player, string question, string name)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Deine Frage enhählt mehr als 150 Zeichen. Bitte versuche dich kurz zu fassen!", 9000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Dein Ticket {Reports.Count + 1} in Warteschlange", 5000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Dein Ticket: {question}", 5000);

                if (Main.AllAdminsOnline.Count <= 2) 
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, "Derzeit kann die Verwaltung etwas länger als üblich antworten. Wir entschuldigen uns für die mögliche Wartezeit und danken für Ihr Verständnis", 8000);
                            ...
                            try
                            {
                                ...
                                if (Reports.Count >= 1)
                                {
                                    foreach (ExtPlayer foreachPlayer in Main.AllAdminsOnline)
                                    {
                                        ...
                                        NAPI.Notification.SendNotificationToPlayer(foreachPlayer, $"Im System ~r~{Reports.Count} ~s~Reports!", true);
        ...
        private static void AddAnswer(ExtPlayer player, int repID, string response)
        {
            try
            {
                ...
                if (response.Length >= 300)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Deine Antwort ist zu lang (über 300 Zeichen).", 3000);
                    return;
                }
                ...
                {
                    Trigger.SendChatMessage(target, $"~o~Dein Ticket: {Reports[repID].Question}");
                    ...
                    EventSys.SendCoolMsg(target,"Antwort auf Ticket", player.Name, $"{response}", "", 10000);
