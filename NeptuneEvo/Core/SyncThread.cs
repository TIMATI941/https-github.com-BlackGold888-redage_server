        ...
        public static void FClearBackground(ExtPlayer player, int fracId)
        {
            try
            {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter, $"Administrator {player.Name} hat die Fraktion aufgeräumt, in der Du dich befindest", 6000);
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {Manager.FractionNames[fracId]} aufgeräumt", 3000);

                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) räumt die Fraktion {Manager.FractionNames[fracId]} auf");
            }
            catch (Exception e)
            {
