    ...
    class AdminSP : Script
    {
        ...
        public static void Spectate(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Trigger.SendChatMessage(player, "Spieler unter der ID " + id + " existiert nicht");
                    return;
                }
                ...
                if (!targetSessionData.AdminData.SpMode)
                {
                    if (targetCharacterData.AdminLVL >= 6)
                    {
                        ...
                        {
                            Trigger.SendChatMessage(player, "Spieler unter der ID " + id + " existiert nicht");
                            Notify.Send(target, NotifyType.Alert, NotifyPosition.BottomCenter, $"{player.Name} ({player.Value}) hat versucht Sie zu beobachten (/sp)", 3000);
                            return;
                        }
                    }
                    ...
                    Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) startet verfolgung {target.Name} ({target.Value})");
                    adminData.SpMode = true;
                    adminData.SPPlayer = target.Value;
                    Trigger.ClientEvent(player, "spmode", target, true);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie beobachten {target.Name} [ID: {target.Value}].", 1000);
                }
            }
            catch (Exception e)
            {
                ...
                if (adminData.SpMode)
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                            ...
                            Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) hat den Beobachtermodus verlassen");
                            Trigger.SendChatMessage(player, "Sie haben den Beobachtermodus verlassen");
                        }
                        catch (Exception e)
                        {
