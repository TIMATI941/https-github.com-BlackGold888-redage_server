...
namespace NeptuneEvo.Core
{
    class DrivingSchool : Script
    {
        ...
        private static Vector3 enterSchool = new Vector3(-711.75775, -1307.295, 5.1129394);
        ...
        public void onResourceStart()
        {
            try
            {
               ...
               //Main.CreateBlip(new Main.BlipData(498, "Автошкола", enterSchool, 2, true));
                PedSystem.Repository.CreateQuest("g_m_y_salvagoon_02", enterSchool, -60.60183f, title: "~y~NPC~w~ Officer Peter\nFahrprüfer", colShapeEnums: ColShapeEnums.DriveSchool);
        ...
        public void Event_OnPlayerExitVehicle(ExtPlayer player, ExtVehicle vehicle)
        {
            try
            {
                ...
                if (sessionData.DSchoolData.Vehicle == vehicle)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FailExamIfNotSit), 7000);
                    return;
                }
            }
            catch (Exception e)
        ...
        private void timer_exitVehicle(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FailExam), 5000);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (index)
                {
                    case 0:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucBuyMotoLic), 6000);
                    ...
                    case 1:
                        ...
                        //fractionData.Money += Main.LicPrices[3];
                        //GameLog.Money($"player({characterData.UUID})", $"frac(6)", Main.LicPrices[3], $"buyLic");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucBuyVehLic), 6000);
                        //Repository.PlayerStats(player);
                        return;
                    case 2:
                        ...
                        //fractionData.Money += Main.LicPrices[3];
                        //GameLog.Money($"player({characterData.UUID})", $"frac(6)", Main.LicPrices[3], $"buyLic");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucBuyGruzLic), 6000);
                        //Repository.PlayerStats(player);
                        return;
                    /*case 0:
                        try
                    ...
                    case 3:
                        ...
                        fractionData.Money += Main.LicPrices[3];
                        GameLog.Money($"player({characterData.UUID})", $"frac(6)", Main.LicPrices[3], $"buyLic");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucBuySeaLic), 6000);
                        //Repository.PlayerStats(player);
                        return;
                    case 4:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LicMustBe20), 6000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucBuyHeliLic), 6000);
                        //Repository.PlayerStats(player);
                        return;
                    case 5:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucBuyPlaneLic), 6000);
                        //Repository.PlayerStats(player);
                        return;
                    default:
        ...
        public static void OnDriveSchool(ExtPlayer player)
        {
            try
            {
                ...
                frameList.Header = "Lizensen";
                frameList.Callback = callback_driveschool;
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.MotoLic, Main.LicPrices[0]), 0));
                
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.LegLic, Main.LicPrices[1]), 1));
                
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.GruzLic, Main.LicPrices[2]), 2));
                
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.VodLic, Main.LicPrices[3]), 3));
                
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.VertLic, Main.LicPrices[4]), 4));
                
                frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.SamLic, Main.LicPrices[5]), 5));
