    ...
    class Selecting : Script
    {
        ...
        public static void objectSelected(ExtPlayer player, string entityidString)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TooFar), 1500);
                    return;
                }
                ...
                switch (objType)
                {
                    ...
                    case "BurglarProofSafe":
                        ...
                        if (!house.Owner.Equals(player.Name) && (!house.Roommates.ContainsKey(player.Name) || !house.Roommates[player.Name].isFurniture) && characterData.AdminLVL < 6)
                        {
                            ...
                            if (count == 0)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LockpickNeedForHijackMebel), 6000);
                                return;
                            }
                            else
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SafeCantHijacked), 5000);
                                    return;
                                }
                                        ...
                                        case FractionsType.Bikers: // Bikers
                                            if (memberFractionData.Rank < 3)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                                    LangFunc.GetText(LangType.De, DataName.HijackFrom3Rank), 6000);
                                                return;
                                            }
                                ...
                                if (house.ItemsGot <= 0)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MebelEmpty), 6000);
                                    return;
                                }
                                if (sessionData.IsHicjacking) return;
                                sessionData.IsHicjacking = true;
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.StartHijackMebel), 5000);
                                ...
                                NAPI.Task.Run(() =>
                                {
                                    try
                                    {
                                        ...
                                        {
                                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MebelTooFar), 6000);
                                            return;
                                        }
                                        ...
                                        if (house == null)
                                        {
                                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouNotInHouse), 6000);
                                            return;
                                        }
                                        if (house.ItemsGot <= 0)
                                        {
                                        ...
                                        switch (objType)
                                        {
                                            case "WeaponSafe": // Сейф с оружиями
                                                ...
                                                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LockpickBroken), 6000);
                                                break;
                                            ...
                                            case "ClothesSafe": // Мебель со шмотьём
                                                ...
                                                break;
                                            default:
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LickpickError), 6000);
                                                return;
                                        }
                                    }
                                    catch (Exception e)
        ...
        public static void HouseHiJackItem(ExtPlayer player, House house, byte typesafe, string id)
        {
            try
            {
                ...
                if (Chars.Repository.ItemsData.ContainsKey(locationName))
                {
                    ...
                    switch (typesafe)
                    {
                        case 0:
                            ...
                            if (item != null) Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SuccHijackWeaponSafeW), 5000);
                            else Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SuccHijackWeaponSafe), 5000);
                            break;
                        case 1:
                            GameLog.Money($"system", $"player({characterData.UUID})", 900, $"hijackSubjectSafe({house.ID})");
                            MoneySystem.Wallet.Change(player, 900);
                            if (item != null) Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SuccHijackItemSafeW), 5000);
                            else Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SuccHijackItemSafe), 5000);
                            break;
                        case 2:
                            GameLog.Money($"system", $"player({characterData.UUID})", 600, $"hijackClothesSafe({house.ID})");
                            MoneySystem.Wallet.Change(player, 600);
                            if (item != null) Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SuccHijackClothesSafeW), 5000);
                            else Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SuccHijackClothesSafe), 5000);
                            break;
                        default:
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HijackError, house.ID), 5000);
                            return;
                    }
                    if (item != null)
                    {
                ...
                else
                {
                    ...
                    MoneySystem.Wallet.Change(player, 600);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HijackEmptyMebel), 6000);
                    return;
                }
            }
            catch (Exception e)
        ...
        public static void BeltSelected(ExtPlayer player, bool toggle)
        {
            try
            {
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                if (!toggle) Commands.RPChat("sme", player, $"legt" + (characterData.Gender ? "" : "e") + " den Sicherheitsgurt an");
                else Commands.RPChat("sme", player, $"legt" + (characterData.Gender ? "" : "e") + " den Sicherheitsgurt ab");
        ...
        public static void vehicleSelected(ExtPlayer player, ExtVehicle vehicle, int index)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarTooFar), 6000);
                    return;
                }
                bool access = true;
                switch (index)
                {
                    case 0:
                        ...
                        if (VehicleStreaming.GetDoorState(vehicle, DoorId.DoorHood) == DoorState.DoorClosed)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoCapotClosedCar), 6000);
                                return;
                            }
                    ...
                    case 1: 
                        ...
                        {
                            Commands.RPChat("sme", player, $"schließt" + (characterData.Gender ? "" : "e") + " Kofferraum");
                        ...
                        else
                        {
                            ...
                            if (vehicleLocalData1 != null)
                            {
                                ...
                                else if (vehicleLocalData1.Access == VehicleAccess.Fraction && vehicleLocalData1.Fraction != fracId)
                                {
                                    ...
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoBagazhnikCar), 6000);
                                        return;
                                    }
                                }
                                else if (vehicleLocalData1.Access == VehicleAccess.Organization)
                                {
                                    ...
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoAccessToVeh), 6000);
                                        return;
                                    }
                                }
                                ...
                                Commands.RPChat("sme", player, $"öffnet" + (characterData.Gender ? "" : "а") + " Kofferraum");
                            }
                        }
                        return;
                    case 2:
                    ...
                    case 3:
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehNoInventory), 6000);
                                return;
                            }
                            if (characterData.AdminLVL == 0 && VehicleStreaming.GetDoorState(vehicle, DoorId.DoorTrunk) == DoorState.DoorClosed)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoBagazhnikCarClosed), 6000);
                                return;
                            }
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OtherUsingBagazh), 6000);
                                return;
                            }
                    ...
                    case 6:
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            if (memberFractionData != null)
                            {
                                switch (Manager.FractionTypes[memberFractionData.Id])
                                {
                                    ...
                                    case FractionsType.Bikers: // Bikers
                                        ...
                                        {
                                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                                LangFunc.GetText(LangType.De, DataName.HijackBagazh3Rank), 6000);
                                            return;
                                        }
                            ...
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HijackSafeZone), 6000);
                                return;
                            }
                            ...
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehEmptyBagazh), 6000);
                                return;
                            }
                            ...
                            if (count == 0)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HijackBagazhWithLockpickOnly), 7000);
                                return;
                            }
                            ...
                            Commands.RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.breakingbagazhnik));
                            Trigger.ClientEvent(player, "fullblockMove", true);
                            Trigger.ClientEvent(player, "freeze", true);
                            NAPI.Task.Run(() =>
                            {
                                try
                                {
                                    ...
                                    if (random >= 85)
                                    {
                                        ...
                                        if (Chars.Repository.ItemsData.ContainsKey(locationName))
                                        {
                                            ...
                                            if (item != null)
                                            {
                                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Fahrzeug erfolgreich geknackt und 600$ entwendet. Motor wurde gestartet", 8000);
                                            ...
                                            else Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BagazhSuccess), 8000);
                                            VehicleStreaming.SetEngineState(vehicle, true);
                                            VehicleStreaming.SetLockStatus(vehicle, false);
                                        ...
                                        {
                                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BugazhSuccessEmpty), 8000);
                                            VehicleStreaming.SetEngineState(vehicle, true);
                                            VehicleStreaming.SetLockStatus(vehicle, false);
                                            player.AddTableScore(TableTaskId.Item28);
                                            return;
                                    ...
                                    else Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LockpickBroken), 6000);
                                }
                                catch (Exception e)
                                {
                    ...
                    case 7:
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarTooFar), 5000);
                                return;
                            }
                            if (sessionData.ActiveWeap.Item != null)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustWrench), 2000);
                                    return;
                                }
                                ...
                                else if (Item.ItemId == ItemId.Wrench)
                                {
                                    ...
                                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.StartRepairing, vehicle.NumberPlate), 5000);
                                    NAPI.Task.Run(() =>
                                    {
                                        try
                                        {
                                            ...
                                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucRepair), 6000);
                                ...
                                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustWrench), 2000);
                            }
                            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustWrench), 2000);
                        }
                        break;
                    case 10:
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SomebodyInBagazh), 6000);
                                return;
                            }
                            else if (VehicleStreaming.GetDoorState(vehicle, DoorId.DoorTrunk) == DoorState.DoorClosed)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BagazhClosed), 6000);
                                return;
                            }
                            ...
                            Commands.RPChat("sme", player, $"klettert" + (characterData.Gender ? "" : "e") + $" in den Kofferraum");
                    ...
                    case 11:
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            if (vehicleLocalData.AttachToPlayer == null)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NobodyInBagazh), 6000);
                                return;
                            }
                            else if (VehicleStreaming.GetDoorState(vehicle, DoorId.DoorTrunk) == DoorState.DoorClosed)
                            {
                            ...
                            Commands.RPChat("sme", player, $"zieht" + (characterData.Gender ? "" : "e") + $" {target.Name} aus dem Kofferraum");
                        }
                        return;
                    case 12:
                        vehicleLocalData = vehicle.GetVehicleLocalData();
                        if (vehicleLocalData != null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Du kannst es nur an deinem Fahrzeug machen", 3000);
                                return;
                            }

                            if (sessionData.ActiveWeap.Item == null || sessionData.ActiveWeap.Index == -1)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du musst ein Kennzeichen in der Hand haben", 2000);
                                return;
                            }
                            ...
                            Trigger.PlayAnimation(player, "mini@safe_cracking", "idle_base", 39);
                            // Trigger.ClientEventInRange(player.Position, 250f, "PlayAnimToKey", player, false, "vzlomsafe");
                            Commands.RPChat("sme", player, "wechselt Kennzeichen");
                            Trigger.ClientEvent(player, "fullblockMove", true);
                            Trigger.ClientEvent(player, "freeze", true);
                            NAPI.Task.Run(() =>
                            {
        ...
        public static void CallPoliceHijack(ExtPlayer player, byte type, string vnumber)
        {
            try
            {
                ...
                if (type == 0)
                {
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.POLICE, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.SignalHouse, sessionData.HouseID), true);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.SHERIFF, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.SignalHouse, sessionData.HouseID), true);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.FIB, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.SignalHouse, sessionData.HouseID), true);
                    
                    Blip.Name = $"Alarmanlage Immobillie {sessionData.HouseID}";
                    Blip.Transparency = 0;
                    Police.PoliceHouseCalls.Add(sessionData.HouseID, Blip);
                    
                    ExtPlayer target = (ExtPlayer) NAPI.Player.GetPlayerFromName(vnumber);
                    if (target.IsCharacterData()) Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHouseSignal), 5000);
                        ...
                        if (house.Roommates.Count > 0)
                        {
                            foreach (ExtPlayer foreachPlayer in Character.Repository.GetPlayers())
                            {
                                ...
                                if (house.Roommates.ContainsKey(foreachPlayer.Name)) 
                                    Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHouseSignal), 5000);
                            }
                        }
                    }
                    catch (Exception e)
                    ...
                    {
                        Police.setPlayerWantedLevel(player, new WantedLevel(5, LangFunc.GetText(LangType.De, DataName.Police), DateTime.Now, LangFunc.GetText(LangType.De, DataName.ReasonHijackHouse)));
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SignalSrabotalWanted), 6000);
                    }
                    else Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SignalSrabotal), 1000);
                }
                else
                {
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.POLICE, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.SignalVeh, vnumber), true);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.SHERIFF, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.SignalVeh, vnumber), true);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.FIB, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.SignalVeh, vnumber), true);
                    
                    Blip.Name = $"Alarmanlage Fahrzeug {vnumber}";
                    Blip.Transparency = 0;
                    ...
                    {
                        Police.setPlayerWantedLevel(player, new WantedLevel(5, LangFunc.GetText(LangType.De, DataName.Police), DateTime.Now, LangFunc.GetText(LangType.De, DataName.ReasonHijackVeh)));
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SignalSrabotalWanted), 6000);
                    }
                    else Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SignalSrabotal), 1000);
                }
        ...
        public static void DetachPlayer(ExtPlayer player)
        {
            try
            {
                ...
                player.ResetSharedData("AttachToVehicle");
                Trigger.ClientEvent(player, "setPocketEnabled", false);
                Commands.RPChat("sme", player, $"steigt" + (characterData.Gender ? "" : "e") + $" aus dem Kofferraum");
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case "givemoney":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.flvltotransact), 6000);
                            return;
                        }
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.De, DataName.GiveMoney), LangFunc.GetText(LangType.De, DataName.SumBucks), 4, "player_givemoney");
                        return;
                    case "handsup":
                        if (player.IsInVehicle) return;
                    ...
                    case "rob":
                        ...
                        return;
                    case "Freilassen": //ORIGINAL case "Отпустить":
                        if (player.IsInVehicle) return;
                        
                        if (targetSessionData.Following == null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerNoFollowing), 6000);
                            return;
                        }
                        if (sessionData.Follower != target)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SomebodyFollowing), 6000);
                            return;
                        }
                    ...
                    case "search":
                        {
                            ...
                            Commands.RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.SearchingTarget), target);
                            ...
                            if (BagsSqlId != 0 && Chars.Repository.ItemsData.ContainsKey($"backpack_{BagsSqlId}") && Chars.Repository.ItemsData[$"backpack_{BagsSqlId}"].ContainsKey("backpack"))
                            {
                                foreach (KeyValuePair<int, InventoryItemData> itemData in Chars.Repository.ItemsData[$"backpack_{BagsSqlId}"]["backpack"])
                                {
                                    ...
                                    if (ItemInfo.functionType == newItemType.Weapons) _Weapons.Add(LangFunc.GetText(LangType.De, DataName.inabag,ItemInfo.Name, itemData.Value.Data));
                                    else _ItemNames.Add(LangFunc.GetText(LangType.De, DataName.inabag, ItemInfo.Name, itemData.Value.Count));
                                }
                            }
                    ...
                    case "Посмотреть паспорт":
                        {
                            ...
                            Commands.RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.LooksPassport), target);
                            
                            string fraction = (targetFracId > (int)Fractions.Models.Fractions.None) ? Fractions.Manager.FractionNames[targetFracId] : LangFunc.GetText(LangType.De, DataName.No);
                            if ((targetFracId >= (int)Fractions.Models.Fractions.FAMILY && targetFracId <= (int)Fractions.Models.Fractions.BLOOD) || (targetFracId >= (int)Fractions.Models.Fractions.LCN && targetFracId <= (int)Fractions.Models.Fractions.ARMENIAN)) fraction = LangFunc.GetText(LangType.De, DataName.No);
                            string work = (targetCharacterData.WorkID > 0) ? Jobs.WorkManager.JobStats[targetCharacterData.WorkID - 1] : LangFunc.GetText(LangType.De, DataName.NoWorker);
                            List<object> data2 = new List<object>
                            {
                                ...
                                (targetCharacterData.Gender) ? LangFunc.GetText(LangType.De, DataName.Mans) : LangFunc.GetText(LangType.De, DataName.Womens),
                                fraction,
                                work
                            };
                    ...
                    case "Посмотреть лицензии":
                        {
                            ...
                            Commands.RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.LooksLic), target);
                    ...
                    case "sellkit":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.De, DataName.OfferHealth), LangFunc.GetText(LangType.De, DataName.SumBucks), 4, "player_medkit");
                        return;
                    case "offerheal":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.De, DataName.OfferHeal), LangFunc.GetText(LangType.De, DataName.SumBucks), 4, "player_heal");
                        return;
                    case "heal":
                        if (player.IsInVehicle)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantHealveh), 6000);
                            return;
                        }
                    ...
                    case "sellhouse":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.De, DataName.OfferHouse), LangFunc.GetText(LangType.De, DataName.SumBucks), 8, "player_offerhousesell");
                        return;
                    case "roommate":
                    ...
                    case "offer":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PersonCantTrade), 6000);
                            return;
                        }
                        ...
                        //Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OfferTrade, player.Value), 10000);
                        EventSys.SendCoolMsg(target,"Angebot", "Handel", $"{LangFunc.GetText(LangType.De, DataName.OfferTrade, player.Value)}", "", 10000);
                        EventSys.SendCoolMsg(player,"Angebot", "Handel", $"{LangFunc.GetText(LangType.De, DataName.OfferedTrade, target.Value)}", "", 5000);
                        //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OfferedTrade, target.Value), 6000);
                        return;
                    case "pocket":
                        if (player.IsInVehicle) return;
                    ...
                    case "ticket":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.De, DataName.SendTicket), LangFunc.GetText(LangType.De, DataName.Sum0to1000), 4, "player_ticketsum");
        ...
        public static void carryAnimations(ExtPlayer player, ExtPlayer target, int action)
        {
            try
            {
                ...
                else if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath && !targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath && !sessionData.AntiAnimDown && !targetSessionData.AntiAnimDown)
                {
                    ...
                    if (action == 0) action_name = LangFunc.GetText(LangType.De, DataName.YouActionNaRuki);
                    else if (action == 1) action_name = LangFunc.GetText(LangType.De, DataName.YouActionNaSheyu);
                    else if (action == 2) action_name = LangFunc.GetText(LangType.De, DataName.YouActionNaPlecho);
                    else if (action == 3) action_name = LangFunc.GetText(LangType.De, DataName.YouActionZalozhnik);

                    //Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerWants, player.Value, action_name), 10000);
                    EventSys.SendCoolMsg(target,"Angebot", "Paaranimation", $"{LangFunc.GetText(LangType.De, DataName.PlayerWants, player.Value, action_name)}", "", 12000);

                    if (action == 0) action_name = LangFunc.GetText(LangType.De, DataName.HeActionHands);
                    else if (action == 1) action_name = LangFunc.GetText(LangType.De, DataName.HeActionSheya);
                    else if (action == 2) action_name = LangFunc.GetText(LangType.De, DataName.HeActionPlecho);
                    else if (action == 3) action_name = LangFunc.GetText(LangType.De, DataName.HeActionZalozhnik);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouWant, target.Value, action_name), 5000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantInteractAtThisMoment), 4000);
        ...
        public static void StartCarry(ExtPlayer player, ExtPlayer target, string action)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyAnim), 6000);
                    return;
                }
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantInteractAtThisMoment), 6000);
        ...
        public static void pairedAnimations(ExtPlayer player, ExtPlayer target, string action)
        {
            try
            {
                ...
                string action_name = LangFunc.GetText(LangType.De, DataName.pairanim);
                if (action == "PAIRED_EMBRACE") action_name = LangFunc.GetText(LangType.De, DataName.hug);
                else if (action == "PAIRED_KISS") action_name = LangFunc.GetText(LangType.De, DataName.kiss);
                else if (action == "PAIRED_FIVE") action_name = LangFunc.GetText(LangType.De, DataName.hfive);
                else if (action == "PAIRED_SLAP") action_name = LangFunc.GetText(LangType.De, DataName.giveface);
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath && !targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath && !sessionData.AntiAnimDown && !targetSessionData.AntiAnimDown)
                {
                    ...
                    if (action == "PAIRED_FIVE" || action == "PAIRED_SLAP") EventSys.SendCoolMsg(target,"Angebot", "Paaranimation", $"{LangFunc.GetText(LangType.De, DataName.PlayerWantInteractYou, player.Value, action_name)}", "", 10000);
                        //Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerWantInteractYou, player.Value, action_name), 6000);
                    else  EventSys.SendCoolMsg(target,"Angebot", "Paaranimation", $"{LangFunc.GetText(LangType.De, DataName.YouWantInteract, player.Value, action_name)}", "", 10000);
                        //Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouWantInteract, player.Value, action_name), 6000);

                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouWant, target.Value, action_name), 6000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantInteract, action_name), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void pairedAnimationAccept(ExtPlayer player, string action)
        {
            try
            {
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath && !targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath && !sessionData.AntiAnimDown && !targetSessionData.AntiAnimDown)
                {
                    ...
                    switch (action)
                    {
                        case "PAIRED_EMBRACE":
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.hugsyou, target.Name), 2000);
                            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.youhug, player.Name), 2000);
                            break;
                        ...
                        case "PAIRED_KISS":
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.youkiss, target.Name), 2000);
                            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.kissyou, player.Name), 2000);
                            break;
                        ...
                        case "PAIRED_FIVE":
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.givehf, target.Name), 2000);
                            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.hfget, player.Name), 2000);
                            break;
                        ...
                        case "PAIRED_SLAP":
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.givenface, target.Name), 2000);
                            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.facegiven, player.Name), 2000);
                            break;
                        default:
                            // Not supposed to end up here. 
                            break;
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.cantpairanim), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void TriggerVoiceChange(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                if (sessionData.Muted.Contains(target.Name))
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.yougagleft, target.Value), 2000);
                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.gagleft, player.Value), 2000);
                }
                else
                {
                    if (sessionData.Muted.Count >= 10)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.somanymuted), 10000);
                        return;
                    }
                    if (targetCharacterData.AdminLVL >= 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.nomuteadmin), 6000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouGag, target.Value), 2000);
                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GagYou, player.Value), 2000);
                }
            }
            catch (Exception e)
            {
                ...
                switch (action)
                {
                    ...
                    case 4:
                        ...
                        if (sessionData.Follower != null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouReleasePlayer, target.Value), 6000);
                                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerReleaseYou, player.Value), 6000);
                                FractionCommands.unFollow(player, target);
                            }
                            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SomebodyFollowing), 2000);
                        }
        ...
        public static void playerTransferMoney(ExtPlayer player, string arg)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LowTimeLastTransact), 6000);
                    return;
                }
                sessionData.TimingsData.NextTransfer = DateTime.Now.AddSeconds(15);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerGivesYouMoney, player.Value, amount), 6000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouGivePlayerMoney, target.Value, amount), 6000);
                ...
                Commands.RPChat("sme", player, $"übergibt" + (characterData.Gender ? "" : "а") + $" {MoneySystem.Wallet.Format(amount)}$ " + "{name}", target);
        ...
        public static void playerHealTarget(ExtPlayer player, ExtPlayer target, ItemId itemId = ItemId.HealthKit)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHaveNoApteka, Chars.Repository.ItemsInfo[itemId].Name), 6000);
                    return;
                }
                if (targetSessionData.DeathData.IsDying && !targetSessionData.DeathData.IsReviving)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouStartReanim, target.Value), 6000);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.StartReanimOnYou, player.Value), 6000);
                    ...
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ReanimTooFar), 6000);
                                return;
                            }
                            ...
                            if (player.GetFractionId() != (int)Fractions.Models.Fractions.EMS && characterData.AdminLVL == 0)
                            {
                                ...
                                if (characterData.Licenses[8])
                                {
                                    ...
                                    {
                                        if (targetSessionData.TimersData.DeathTimer == null) Ems.ReviveFunc(target);
                                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FailureReanim), 6000);
                                        return;
                                    }
                                }
                                else
                            ...
                            Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerReanimYou, player.Value), 6000);
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouReanimPlayer, target.Value), 6000);
                ...
                {
                    Chars.Repository.RemoveIndex(player, item.Location, item.Index, 1);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerHealYou, player.Value), 6000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHealPlayer, target.Value), 6000);
                    target.Health = 100;
                }
                return;
            }
        ...
        public static void playerTakeGuns(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerTakeWeaponYou, player.Value), 6000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouTakeWeaponPlayer, target.Value), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void playerTakeIlleagal(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerDoesntIllegalStuff), 6000);
                    return;
                }
                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.IllegalStuffTaken, player.Value), 6000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouTakeIllegalStuff, target.Value), 6000);
            ...
            try
            {
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath)
                {
                    if (!targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath)
                    {
                        ...
                        targetSessionData.RequestData.From = player;
                        targetSessionData.RequestData.Time = DateTime.Now.AddSeconds(10);
                        //Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HandshakeYou, player.Value), 6000);
                        EventSys.SendCoolMsg(target,"Angebot", "Handschlag!", $"{LangFunc.GetText(LangType.De, DataName.HandshakeYou, player.Value)}", "", 10000);
                        //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHandshake, target.Value), 6000);
                        EventSys.SendCoolMsg(player,"Angebot", "Handschlag!", $"{LangFunc.GetText(LangType.De, DataName.YouHandshake, target.Value)}", "", 10000);
                    }
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantHandshake), 6000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantHandshake), 6000);
            }
            catch (Exception e)
