    ...
    class VehicleManager : Script
    {
        ...
        public static void Init()
        {
            try
            {
                ...
                Log.Write($"Vehicles are loaded ({vehiclesData.Count}) | item verschoben - {countSwipe} Stk.", nLog.Type.Success);
            }
            catch (Exception e)
            {
        ...
        public void onPlayerEnterVehicleHandler(ExtPlayer player, ExtVehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                {
                    player.setKick(LangFunc.GetText(LangType.De, DataName.KickVehNoUsing));
                    return;
                }
                if (characterData.DemorganTime >= 1) 
                    player.setKick(LangFunc.GetText(LangType.De, DataName.KickVehWarned));
                ...
                if (player.VehicleSeat == (int)VehicleSeat.Driver)
                {
                    if (!characterData.Achievements[18])
                    {
                        ...
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Helper, LangFunc.GetText(LangType.De, DataName.Podskazka1), DateTime.Now);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int) DefaultNumber.Helper, LangFunc.GetText(LangType.De, DataName.Podskazka2), DateTime.Now);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Helper, LangFunc.GetText(LangType.De, DataName.Podskazka3), DateTime.Now);
                        
                        
                        //Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Podskazka1), 5000);
                        //Notify.SendToKey(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Podskazka2), 10000, 38);
                        //Notify.SendToKey(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Podskazka3), 10000, 11);
                    ...
                    if (vehicleLocalData.Access == VehicleAccess.Fraction)
                    {
                        ...
                        if (vehicleLocalData.Fraction == (int)Fractions.Models.Fractions.ARMY && (vehicle.Model == (uint)VehicleHash.Barracks || vehicle.Model == (uint)VehicleHash.Brickade || vehicle.Model == (uint)VehicleHash.Cargobob))
                        {
                            if ((fracId >= (int)Fractions.Models.Fractions.FAMILY && fracId <= (int)Fractions.Models.Fractions.BLOOD) || (fracId >= (int)Fractions.Models.Fractions.LCN && fracId <= (int)Fractions.Models.Fractions.ARMENIAN) || fracId == (int)Fractions.Models.Fractions.THELOST)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FractionCarFrom10), 6000);
                                    WarpPlayerOutOfVehicle(player);
                                    return;
                                }
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.EngineONB), 6000);
                                return;
                            }
                    ...
                    if (!VehicleModel.AirAutoRoom.isAirCar(Model))
                    {
                        NAPI.Task.Run(() =>
                        {
                            ...
                            if (house != null && Houses.GarageManager.Garages.ContainsKey(house.GarageID))
                            {
                                ...
                                EventSys.SendCoolMsg(player,"Fahrzeug", "Fahrzeugkauf", $"{LangFunc.GetText(LangType.De, DataName.VehSoonGarage)}", "", 12000);
                                //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehSoonGarage), 5000);
                                //Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.VehSoonGarage), DateTime.Now);
                            }
                            else
                                EventSys.SendCoolMsg(player,"Fahrzeug", "Fahrzeugkauf", $"{LangFunc.GetText(LangType.De, DataName.VehWhenHome)}", "", 12000);
                                    // Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehWhenHome), 5000);
                            //Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.VehWhenHome), DateTime.Now);
        ...
        public static void OpenSellCarMenu(ExtPlayer player)
        {
            try
            {
                ...
                frameList.Header = LangFunc.GetText(LangType.De, DataName.sellcar); 
        ...
        private static void callback_sellcar(ExtPlayer player , object listItem) /// Никитос Чини  
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehOnShtrafSell, vehicleData.Model, vehicleData.Number), 6000);
                    return;
                }
                ...
                Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.De, DataName.ProdazhaVeh), LangFunc.GetText(LangType.De, DataName.VvediteCenu), 8, "sellcar");
            }
            catch (Exception e)
            {
                    ...
                    switch (vehicleLocalData.Access)
                    {
                        case VehicleAccess.Hotel:
                            ...
                            if (VehicleStreaming.GetLockState(vehicle))
                            {
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehOpenDoor), 6000);
                            }
                            else
                            {
                                VehicleStreaming.SetLockStatus(vehicle, true);
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehCloseDoor), 6000);
                            }
                            break;
                        ...
                        default:
                            if (characterData.AdminLVL <= 3)
                            {
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehNoLock), 6000);
                                return;
                            }
                            if (VehicleStreaming.GetLockState(vehicle))
                            {
            ...
            try
            {
                ...
                switch (eventName)
                {
                    case "engineCarPressed":
                        ...
                        {
                            Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustBeDriver), 6000);
                            return;
                        }
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            if (vehicleLocalData.Petrol <= 0)
                            {
                                Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoGasVeh), 6000);
                                return;
                            }
                            switch (vehicleLocalData.Access)
                            {
                                case VehicleAccess.Hotel:
                                    ...
                                    {
                                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoKeysFromVeh), 6000);
                                        return;
                                    }
                                    break;
                                ...
                                case VehicleAccess.Fraction:
                                    ...
                                    {
                                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WorkDayNotStarted), 6000);
                                        return;
                                    }
                                    break;
                                case VehicleAccess.Organization:
                                case VehicleAccess.OrganizationGarage:
                                    ...
                                    else if (vehicleLocalData.MinRank > memberOrganizationData.Rank)
                                    {
                                        ...
                                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoAccessToVeh), 6000);
                                        return;
                                    }
                                ...
                                case VehicleAccess.Personal:
                                    ...
                                    {
                                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarDestroyed), 6000);
                                        return;
                                    }
                                    break;
                                case VehicleAccess.Garage:
                            ...
                            {
                                VehicleStreaming.SetEngineState(vehicle, false);
                                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehEngineON), 6000);
                                Commands.RPChat("sme", sender, $"stellt" + (characterData.Gender ? "" : "а") + " den Motor ab");
                            }
                            else
                            {
                                VehicleStreaming.SetEngineState(vehicle, true);
                                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehEngineOFF), 6000);
                                Commands.RPChat("sme", sender, $"startet" + (characterData.Gender ? "" : "а") + " den Motor");
                            }
                        }
                        return;
                    case "lockCarPressed":
                                    ...
                                    if (!VehicleModel.AirAutoRoom.isAirCar(vehicleData.Model))
                                    {
                                        ...
                                        if (owner != null)
                                            Notify.Send(owner, NotifyType.Alert, NotifyPosition.BottomCenter,
                                                LangFunc.GetText(LangType.De, DataName.CarDestroyed), 6000);
