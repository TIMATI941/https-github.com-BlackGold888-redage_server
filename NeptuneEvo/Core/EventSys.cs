...
namespace NeptuneEvo.Core
{
    class EventSys : Script
    {
        ...
        [Command(AdminCommands.Createmp, "benutze: /createmp [Spieler limit] [Zone Radius] [Eventname]", GreedyArg = true)]
            ...
            try
            {
                ...
                if (AdminEvent.EventState == 0)
                {
                    if (eventname.Length < 50)
                    {
                        ...
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.EventStart, Chars.Models.ChatColors.AMP, eventname));
                        if (members != 0) NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.EventMemberLimit, Chars.Models.ChatColors.AMP, members));
                        else NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.LimitMemberNoStart, Chars.Models.ChatColors.AMP));
                        if (AdminEvent.Zone != null) NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.MpRadiusTp, Chars.Models.ChatColors.AMP, radius));
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.TpMpTp, Chars.Models.ChatColors.AMP));
                    }
                    else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.TooLongMpName));
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.OneMpExists));
            }
            catch (Exception e)
            {
        ...
        public static void OutAdminMP(ExtPlayer player)
        {
            ...
            { // Проверяет только после начала мп, когда телепорт закрыт
                ...
                {
                    ExitFromMP(player);
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.LeftMpTer));
                }
            }
        }
            ...
            try
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    if (AdminEvent.EventMembers.Count >= 1)
                    {
                        ...
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.MpNameStarted, Chars.Models.ChatColors.AMP, AdminEvent.Name));
                        ...
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.MpPlayers, Chars.Models.ChatColors.AMP, AdminEvent.EventMembers.Count));
                    }
                    else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.CantStartMpWithout));
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.MpNotStarted));
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.Stopmp, "benutze: /stopmp [Gewinner ID] [Belohnung #1] [ID Gewinner2?] [Belohnung #2?] [ID Gewinner3?] [Belohnung #3?]")] // ne vishlo
        public static void MPReward(ExtPlayer player, int pid, uint reward, int pid2 = -1, uint reward2 = 0, int pid3 = -1, uint reward3 = 0)
        {
            try
            {
                ...
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.OneWinnerMpCant));
                    return;
                }
                if (AdminEvent.EventState == 2)
                {
                    if (reward <= AdminEvent.RewardLimit)
                    {
                        if (AdminEvent.Winner == null)
                        {
                            ...
                            if (target != null)
                            {
                                ...
                                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.PlayerNotFoundMp));
                            }
                            else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.CantFindPlayerWithId));
                        }
                        else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.WinnerBilNaznachen));
                    }
                    else Trigger.SendChatMessage(player, "Die Belohnung darf die angezeigte Grenze nicht überschreiten: " + AdminEvent.RewardLimit + ".");
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.MpNotStarted));
            }
            catch (Exception e)
        ...
        [Command(AdminCommands.Mpveh, "benutze: /mpveh [Modelname] [Farbe] [Farbe] [Anzahl Fahrzeuge]")]
        public static void CreateMPVehs(ExtPlayer player, string model, byte c1, byte c2, byte count)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantUse0Dim), 6000);
                    return;
                }
                ...
                if (AdminEvent.EventState >= 1)
                {
                    if (count >= 1 && count <= 10)
                    {
                        ...
                        {
                            if (characterData.AdminLVL <= 5) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoVehThatName), 6000);
                            else
                            {
                    ...
                    else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.OneTime10vehs));
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.VehOnlyMp));
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.mpskin, "benutze: /mpskin [ID] [SKIN]")]
        public static void GiveMPSkin(ExtPlayer player, int pid, string pedModel)
        {
            try
            {
                ...
                if (target != null)
                {
                    ...
                    {
                        AdminEvent.Admin = player;
                        Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.AdminMpChangedSkin, Chars.Models.ChatColors.AMP, AdminEvent.Name, player.Name, target.Name, pedModel));
                        ...
                        if (pedModel.Equals("-1"))
                        {
                            if (targetSessionData.AdminSkin)
                            {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VisualPlayerReturned), 6000);
                            }
                            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VisualPlayerNotChanged), 6000);
                        }
                        else
                        {
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerIdFoundButNotMp), 6000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindPlayerWithId), 6000);
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.mpskins, "benutze: /mpskins [SKIN] [RADIUS]")]
        public static void GiveMPSkins(ExtPlayer player, string pedModel, int radius)
        {
            try
            {
                ...
                Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.AdminMpChangedSkinRadius, Chars.Models.ChatColors.AMP, AdminEvent.Name, player.Name, pedModel, radius));
            }
            catch (Exception e)
            {
        ...
        public static void MPAdminPos(ExtPlayer player)
        {
            try
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    ...
                    Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.PostavilMetkuVsem, Chars.Models.ChatColors.AMP, AdminEvent.Name, player.Name));
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MetkuCanPostaviTposleMp), 6000);
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.Mpreward, "benutze: /mpreward [Neuer Limit]")]
            ...
            try
            {
                ...
                if (newreward <= 999999)
                {
                    ...
                    try
                    {
                        ...
                        MySQL.Query(cmd);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MpReward, newreward), 6000);
                    }
                    catch (Exception e)
                    {
                ...
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.MpRewardLimit));
            }
            catch (Exception e)
            {
        ...
        public static void CMD_MPGlobal(ExtPlayer player, string text)
        {
            try
            {
                ...
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.MpGlobalrError));
            }
            catch (Exception e)
            {
        ...
        public static void TpToMp(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BlacklistedMp, characterData.BanMPReason), 5000);
                    return;
                }
                    ...
                    if (AdminEvent.EventState == 1)
                    {
                        if (!AdminEvent.EventMembers.ContainsKey(player))
                        {
                            if (AdminEvent.MembersLimit == 0 || AdminEvent.EventMembers.Count < AdminEvent.MembersLimit)
                            {
                                ...
                                Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.YouTpedMp, AdminEvent.Name));
                            }
                            else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.MpListFull));
                        }
                        else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.YouAlreadyMped));
                    }
                    else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.TpClosed));
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.TpUnavaible));
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.Mpkick, "benutze: /mpkick [Spieler ID] [Grund]", GreedyArg = true)]
            ...
            try
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    ...
                    if (target != null)
                    {
                        ...
                        {
                            AdminEvent.Admin = player;
                            Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.AdminKickedMpS, Chars.Models.ChatColors.AMP, AdminEvent.Name, player.Name, target.Name, reason));
                            lock (AdminEvent.EventMembers)
                            {
                                foreach (ExtPlayer foreachPlayer in AdminEvent.EventMembers.Keys)
                                {
                                    if (foreachPlayer == null) continue;
                                    Trigger.SendChatMessage(foreachPlayer, LangFunc.GetText(LangType.De, DataName.AdminKickedMpS, Chars.Models.ChatColors.AMP, AdminEvent.Name, player.Name, target.Name, reason));
                                }
                            }
                            ExitFromMP(target);
                        }
                        else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.PlayerIdFoundButNotMp));
                    }
                    else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.CantFindPlayerWithId));
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.VignatMozhno));
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.Banmp, "benutze: /banmp [Spieler ID] [Grund]", GreedyArg = true)]
        public static void BanMP(ExtPlayer player, int pid, string reason)
        {
            try
            {
                ...
                if (target != null && targetCharacterData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerBlocked), 5000);
                        return;
                    }
                    
                    Trigger.SendPunishment(LangFunc.GetText(LangType.De, DataName.AdminBlockedMpS, CommandsAccess.AdminPrefix, player.Name, target.Name, target.Value, reason), target);
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindPlayerWithId), 5000);
            }
            catch (Exception e)
            {
        ...
        [Command(AdminCommands.Unbanmp, "benutze: /unbanmp [Spieler ID]")]
        public static void UnbanMP(ExtPlayer player, int pid)
        {
            try
            {
                ...
                if (target != null && targetCharacterData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerHasntBlocked), 5000);
                        return;
                    }
                    
                    Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.AdminUnblockedMp, player.Name, player.Value, target.Name, target.Value));
                    Trigger.SendChatMessage(target, LangFunc.GetText(LangType.De, DataName.AdminBlockedMp, player.Name));
        ...
        public static void MPLeave(ExtPlayer player)
        {
            try
            {
                ...
                if (AdminEvent.EventState != 0)
                {
                    ...
                    {
                        ExitFromMP(player);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucLeaveMp), 5000);
                    }
                }
            }
            catch (Exception e)
        ...
        [Command(AdminCommands.Mphp, "benutze: /mphp [Anzahl HP]")]
            ...
            try
            {
                ...
                if (AdminEvent.EventState >= 1)
                {
                    if (health >= 1 && health <= 100)
                    {
                        ...
                        Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.YouFilledHp, health));
                    }
                    else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.HpOneTo100));
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.HpOnlyUnderStart));
            }
            catch (Exception e)
            {
                ...
                if (AdminEvent.EventState != 0)
                {
                    ...
                    if (memcount > 0)
                    {
                        if (memcount <= 15)
                        {
                            ...
                            {
                                if(foreachPlayer == null) continue;
                                Trigger.SendChatMessage(player, "ID: " + foreachPlayer.Value + " | Name: " + foreachPlayer.Name);
                            }
                            Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.PlayersOnMp, memcount));
                        }
                        else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.PlayersOnMp, memcount));
                    }
                    else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.NoMpPlayers));
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.MpNotSozdano));
            }
            catch (Exception e)
            {
        ...
        public static void CloseAdminEvent(ExtPlayer winner, uint reward, int winnerid2, uint reward2, int winnerid3, uint reward3)
        {
            try
            {
                ...
                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.MpEnded, Chars.Models.ChatColors.AMP, AdminEvent.Name));
                ...
                if (winner != AdminEvent.Admin)
                {
                    if (reward != 0)
                    {
                        NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.MpWinner1, Chars.Models.ChatColors.AMP, winner.Name, MoneySystem.Wallet.Format(reward)));
                        MoneySystem.Wallet.Change(winner, (int)reward);
                    }
                    else NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.WinnerMp1, Chars.Models.ChatColors.AMP, winner.Name));
                    ...
                    if (winnerid2 != -1)
                    {
                        ...
                        if(winner2 != null && winner2 != AdminEvent.Admin)
                        {
                            ...
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.MpWinner, Chars.Models.ChatColors.AMP, winnernum, winner2.Name, MoneySystem.Wallet.Format(reward2)));
                                MoneySystem.Wallet.Change(winner2, (int)reward2);
                            }
                            else NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.WinnerMp, Chars.Models.ChatColors.AMP, winnernum, winner2.Name));
                            winnernum++;
                        }
                    }
                    ...
                    if (winnerid3 != -1)
                    {
                        ...
                        if (winner3 != null && winner3 != AdminEvent.Admin)
                        {
                            ...
                            {
                                NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.MpWinner, Chars.Models.ChatColors.AMP, winnernum, winner3.Name, MoneySystem.Wallet.Format(reward3)));
                                MoneySystem.Wallet.Change(winner3, (int)reward3);
                            }
                            else NAPI.Chat.SendChatMessageToAll(LangFunc.GetText(LangType.De, DataName.WinnerMp, Chars.Models.ChatColors.AMP, winnernum, winner3.Name));
                        }
                    }
                }
                if (AdminEvent.EventMembers.Count != 0)
                {
                    lock(AdminEvent.EventMembers)
                    {
                        foreach (ExtPlayer foreachPlayer in AdminEvent.EventMembers.Keys)
                        {
                            ...
                            //Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SpsMp), 6000);
                            SendCoolMsg(foreachPlayer,"Aktion", AdminEvent.Name, $"{LangFunc.GetText(LangType.De, DataName.SpsMp)}", "", 10000);
                        }
                    }
                }
