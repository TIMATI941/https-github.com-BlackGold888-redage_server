    ...
    public class Repository : Script
    {
        ...
        public static void UpdateReward(ExtPlayer player, int id, bool isEnd = false)
        {
            ...
            if (battlePass != null)
            {
                ...
                if (battlePass.Count >= battlePassTask.Count)
                {
                    ...
                    Trigger.ClientEvent(player, "client.battlepass.missionComplite", battlePassTask.Text, LangFunc.GetText(LangType.De, DataName.BPEverydayComplete));
                }
            }
            ...
            if (missionData.Select == id)
            {
                ...
                if (mission != null && !mission.IsDone)
                {
                    ...
                    if (mission.Count >= battlePassTask.Count)
                    {
                        ...
                        Trigger.ClientEvent(player, "client.battlepass.missionComplite", battlePassTask.Text, LangFunc.GetText(LangType.De, DataName.MissionComplete));
                    }
                }
            }
        }
            ...
            try
            {
                ...
                switch (award.Type)
                {
                    ...
                    case BattlePassRewardType.Vip:
                        ...
                        if (isMessage)
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouGetVipDays, Group.GroupNames[award.ItemId], award.Count), 6000);
                        break;
                    ...
                    case BattlePassRewardType.Donate:
                        ...
                        if (isMessage) 
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouWinRb, award.Count), 5000);
                        break;
                }
            }
            catch (Exception e)
        ...
        public void Take(ExtPlayer player, int index, bool isPrem)
        {
            ...
            if (award != null && (!isPrem || battlePassData.IsPremium))
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BpNoLevel), 6000);
                    return;
                }
                
                if (award.Type == BattlePassRewardType.None)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantTakeThisItem), 6000);
                    return;
                }
                
                if ((!isPrem && battlePassData.TookReward.Contains(award.Id)) || (isPrem && battlePassData.TookRewardPremium.Contains(award.Id)))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyTakeAward), 6000);
            ...
            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoActiveAwards), 6000);
        }
        ...
        public void TakeAll(ExtPlayer player)
        {
            ...
            if (isAdd)
            {
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouTakeAllAwards), 6000);
                Trigger.ClientEvent(player, $"client.battlepass.takeSuccess", JsonConvert.SerializeObject(battlePassData.TookReward), JsonConvert.SerializeObject(battlePassData.TookRewardPremium));
            }
            else
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoActiveAwards), 7000);
            }
        }
        ...
        public static void BuyPremium(ExtPlayer player, bool isBuyDonate = false)
        {
            ...
            if (battlePassData.IsPremium)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyHaveBp), 6000);
                return;
            }
            if (DonatePack.IsDonate(player))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PaymetnLoad), 5000);
                return;
            }
            ...
            //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouBuyBp), 6000);
            Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.YouBuyBp), DateTime.Now);
        ...
        public void BuyLvl(ExtPlayer player, int index)
        {
            ...
            if (IsMaxLvl (battlePassData.Lvl))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyMaxLvl), 6000);
                return;
            }

            AddLvl(player, buyLvl.Lvl);
            
            //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BpBuyLvl), 7000);
            Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.BpBuyLvl, buyLvl.Lvl, buyLvl.PriceRB), DateTime.Now);
        ...
        public static void SetMissions(ExtPlayer player, int id)
        {
            ...
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DontCompletePrevios), 6000);
                return;
            }

            if (missionData.Tasks.Any(t => t.Index == id && t.IsReward))
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.QuestCompletedAward), 6000);
                return;
            }
            ...
            if (missionData.Select != id)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouTakeMissonBp, battlePassTask.Text), 6000);
            }
            else
            {
                ...
                if (task != null && task.IsDone && !task.IsReward)
                {
                    ...
                    if (count == 20)
                    {
                        ...
                        Trigger.SendChatMessage(player,LangFunc.GetText(LangType.De, DataName.YouGetBonusBp));
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BpTake), 6000);
                    return;
                }
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouDontCompleteQuest), 6000);
            }
        ...
        public static void OnSkip(ExtPlayer player)
        {
            try
            {
                ...
                if (missionData.Select != -1)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCompleteQuest), 6000);
                ...
                else
                {
                    ...
                    if (count < 20)
                    {
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantRefreshMissons), 6000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RefreshMissions), 6000);
