    ...
    class Repository
    {
        ...
        public static async void Load(ExtPlayer player, DateTime TestSpeedLoad, bool isReg = false)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.Center, LangFunc.GetText(LangType.De, DataName.CharDataGetError), 5000);
                    return;
                }
                ...
                foreach (var charData in accountData.Chars.ToList())
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CharDelete, charData), 6000);
