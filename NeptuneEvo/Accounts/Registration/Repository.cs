    ...
    class Repository
    {
        ...
        public static async Task<RegistrationEnum> Register(ExtPlayer player, string login, string pass_, string email, string promo_, string ga)
        {
            try
            {
                ...
                {
                    Password = Accounts.Repository.GetMD5(pass_),
                    Login = login,
                    Email = email,
                    VipLvl = 0,
                ...
                GameLog.AccountLog(accountData.Login, accountData.HWID, accountData.IP, accountData.SocialClub, $"Die richtige Zeit um zu Starten!");
