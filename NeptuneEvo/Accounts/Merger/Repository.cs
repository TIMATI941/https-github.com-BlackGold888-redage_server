    ...
    public class Repository
    {
        ...
        public static async Task MergerAuntification(ExtPlayer player, string pass_, int serverId = 2)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MergeErrorRestart), 6000);
                    return;
                }
                ...
                pass_ = Accounts.Repository.GetMD5(pass_);
                ...
                UpdateData.RedBucks(player, redBucks, LangFunc.GetText(LangType.De, DataName.Merge));
        ...
        public static async Task<int> MergerToChar(ExtPlayer player, ServerBD db, ServerBD saveDB, int serverId, int MoneyTo, Characters character, int num)
        {
            try
            {
                ...
                if (renamed) Notify.Send(player, NotifyType.Success, NotifyPosition.TopCenter, LangFunc.GetText(LangType.De, DataName.CharSuccMerged, OldFirstname, OldLastname), 10000);
            ...
            try
            {
                ...
                if (orgRank != null)
                {
                    ...
                    if (orgData != null)
                    {
                        ...
                        if (officeUpgrade >= 2)
                        {
                            UpdateData.RedBucks(player, Convert.ToInt32(Main.PricesSettings.SecondOrgPrice / 1), LangFunc.GetText(LangType.De, DataName.CashBackOrgMerge));
                        }
