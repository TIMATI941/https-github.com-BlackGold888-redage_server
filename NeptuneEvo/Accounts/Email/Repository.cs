...
namespace NeptuneEvo.Accounts.Email
{
    public class Repository
    {
        ...
        public static async Task<string> Add(ExtPlayer player, string login, string password, string email, string promo, bool isRegistered = true, int type = 0)
        {
            ...
            hash = Accounts.Repository.GetMD5($"{DateTime.Now.Ticks}_{Main.ServerNumber}_{login}_{email}");
                    ...
                    foreach (var hash in confirms)
                    {
                        ...
                        if (emailVerification != null && emailVerification.Player != null)
                        {
                            if (!emailVerification.IsRegistered) 
                                Notify.Send(emailVerification.Player, NotifyType.Success, NotifyPosition.BottomCenter, "Die Zeit ist abgelaufen, um die E-Mail zu bestätigen. Bitte versuchen Sie es erneut!", 5000);
                            else 
                                Accounts.Registration.Repository.MessageError(emailVerification.Player, "Die Zeit ist abgelaufen, um die E-Mail zu bestätigen. Bitte versuchen Sie es erneut!");
                        }
                    }
                });
            }
