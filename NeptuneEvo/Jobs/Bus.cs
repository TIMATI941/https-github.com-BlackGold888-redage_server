...
namespace NeptuneEvo.Jobs
{
    class Bus : Script
    {
        ...
        {
            "[Urban 1] EMS - Old City Hall - Market - Driving School - NEWS - LSPD",
            "[Urban 2] EMS - Old City Hall - Market - Driving School - NEWS - LSPD",
            "[Urban 3] EMS - Old City Hall - Market - Driving School - NEWS - LSPD",
        };
            ...
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    else
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BusStop), 6000);
                        ...
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(mypos, 30))
                        {
                            ...
                            Trigger.SendChatMessage(foreachPlayer, LangFunc.GetText(LangType.De, DataName.NextBus, BusWaysNames[way]));
                        }
                    }
                }
            }
        ...
        private static void timer_busStop(ExtPlayer player, int way, int check, Vector3 mypos)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCanGoNext), 6000);
                int payment = Convert.ToInt32(Main.BuswaysPayments[way] * Group.GroupPayAdd[accountData.VipLvl] * Main.ServerSettings.MoneyMultiplier);
                ...
                {
                    if (!foreachPlayer.IsCharacterData()) continue;
                    Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BusStartsWay, BusWaysNames[way]), 1000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouRentBus, BusWaysNames[way]), 10000);
        }
        ...
        public static void OnPlayerEnterVehicle(ExtPlayer player, ExtVehicle vehicle, sbyte seatId)
        {
            ...
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BusPayed, Main.BusPay), 6000);
