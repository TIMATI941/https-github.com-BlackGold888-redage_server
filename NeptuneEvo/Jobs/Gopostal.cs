    ...
    class Gopostal : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(LangFunc.GetText(LangType.De, DataName.PostHouse), Coords[0] + new Vector3(0, 0, 0.3), 10F, 0.6F, 0, new Color(0, 180, 0));

                //CustomColShape.CreateCylinderColShape(Coords[1], 3, 2, 0, ColShapeEnums.JobGoPostaCar); // get car
                //NAPI.TextLabel.CreateTextLabel(LangFunc.GetText(LangType.De, DataName.TakeWorkVeh), Coords[1] + new Vector3(0, 0, 0.3), 10F, 0.6F, 0, new Color(0, 180, 0));
        ...
        public static void OnJobGoPosta(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Sie arbeiten nicht als Postbote", 3000);
                    return;
                }
                
                var frameList = new FrameListData(); 
                frameList.Header = LangFunc.GetText(LangType.De, DataName.Warehouse); 
                frameList.Callback = callback_gpStartMenu;

                if (sessionData.WorkData.OnWork)
                {
                    frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.EndJob), "finish"));
                    frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.TakeParcels), "get"));
                    
                }else
                    frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.StartJob), "start"));
            ...
            try
            {
                ...
                switch (listItem)
                {
                    ...
                    case "get":
                        if (characterData.WorkID != (int)JobsId.Postman)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoPostman), 6000);
                            return;
                        }
                        if (!sessionData.WorkData.OnWork)
                        {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouNotRazdaliPosilki, sessionData.WorkData.Packages), 6000);
                            return;
                        }          
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            sessionData.WorkData.Packages = Main.ServerNumber == 0 ? 2 : count;
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PostmanStartJob, count), 6000);
        ...
        public static void GoPostal_onEntityEnterColShape(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LeaveJobVeh), 6000);
                    return;
                }
                if (sessionData.WorkData.Packages == 0 || !Attachments.HasAttachment(player, Attachments.AttachmentsName.Postal))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben kein Paket in der Hand", 3000);
                    return;
                }
                ...
                if (jobLevelInfo.Item1 <= 4)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.JobVehFar), 6000);
                        return;
                    }
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoHozyain), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MailsLeft, sessionData.WorkData.Packages), 8000);
                ...
                else
                {
                    ...
                    Trigger.PlayAnimation(player, "anim@heists@narcotics@trash", "drop_side", -1, false);
                    sessionData.WorkData.Packages = 0;
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoMails), 6000);
