    ...
    class Lumberjack : Script
    {
        ...
        public void onResourceStartHandler()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(836, LangFunc.GetText(LangType.De, DataName.LesniyeResi), new Vector3(-490.60272, -1124.4965, 25.138412), 2, true));
                Main.CreateBlip(new Main.BlipData(836, LangFunc.GetText(LangType.De, DataName.LesniyeResi), new Vector3(3370.0408, 4945.8154, 33.202995), 2, true));
                Main.CreateBlip(new Main.BlipData(836, LangFunc.GetText(LangType.De, DataName.LesniyeResi), new Vector3(-1319.7822, 4444.8164, 23.27308), 2, true));
                Main.CreateBlip(new Main.BlipData(836, LangFunc.GetText(LangType.De, DataName.LesniyeResi), new Vector3(-1988.7891, 2584.667, 3.311179), 2, true));
                Main.CreateBlip(new Main.BlipData(836, LangFunc.GetText(LangType.De, DataName.LesniyeResi), new Vector3(160.4275, 6895.9033, 20.979313), 2, true)); 

                Main.CreateBlip(new Main.BlipData(569, LangFunc.GetText(LangType.De, DataName.SkladLesa), new Vector3(-540.13434, 5380.038, 70.48429), 4, true));                 
                Main.CreateBlip(new Main.BlipData(480, LangFunc.GetText(LangType.De, DataName.SkladDereva), new Vector3(-575.54004, 5350.516, 70.214424), 70, true));
 
                Main.CreateBlip(new Main.BlipData(566, LangFunc.GetText(LangType.De, DataName.OhotnMagaz), new Vector3(-758.8592, -618.1002, 30.2762), 21, true));

                TreeStockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16(LangFunc.GetText(LangType.De, DataName.Hranilishe, TreeStock[0], TreeStock[1], TreeStock[2])), new Vector3(-540.13434, 5380.038, 70.48429), 5F, 0.5F, 0, new Color(255, 255, 255), true, 0);

                PedSystem.Repository.CreateQuest("s_m_y_construct_01", new Vector3(-758.8592, -618.1002, 30.2762), -90.1f, title: LangFunc.GetText(LangType.De, DataName.BearGrylls), colShapeEnums: ColShapeEnums.HuntingShop);
                PedSystem.Repository.CreateQuest("s_m_y_construct_01", new Vector3(-1640.2056, -802.55475, 10.231294), 58.71f, title: LangFunc.GetText(LangType.De, DataName.DmitrySkupshik), colShapeEnums: ColShapeEnums.TreesSell);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (index)
                {
                    case 74:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BoughtAxe), 6000);
                    ...
                    case 75:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BoughtPickaxe1), 6000);
                        BattlePass.Repository.UpdateReward(player, 57);
                        return;
                    case 76:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BoughtPickaxe2), 6000);
                        BattlePass.Repository.UpdateReward(player, 57);
                        return;
                    case 77:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BoughtPickaxe3), 6000);
                        BattlePass.Repository.UpdateReward(player, 57);
                        return;
                     case 100:
                         if (UpdateData.CanIChange(player, Main.PricesSettings.InstrumentPrices[4], true) != 255) return;
                         else if (Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.WaterCan, 1, "2250") == -1)
                         {
                             Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoSpaceInventory), 6000);
                             return;
                         }
                         Wallet.Change(player, -Main.PricesSettings.InstrumentPrices[4]);
                         GameLog.Money($"player({characterData.UUID})", $"server", Main.PricesSettings.InstrumentPrices[4], $"buyHuntingShop(100)");
                         Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BoughtWaterCan), 6000);
                         //BattlePass.Repository.UpdateReward(player, 57);
                         return;
                    case 101:
                         if (UpdateData.CanIChange(player, Main.PricesSettings.InstrumentPrices[5], true) != 255) return;
                         else if (Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.Pot, 1, "2250") == -1)
                         {
                             Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoSpaceInventory), 6000);
                             return;
                         }
                         Wallet.Change(player, -Main.PricesSettings.InstrumentPrices[5]);
                         GameLog.Money($"player({characterData.UUID})", $"server", Main.PricesSettings.InstrumentPrices[5], $"buyHuntingShop(101)");
                         Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BoughtPot), 6000);
                         //BattlePass.Repository.UpdateReward(player, 57);
                         return;
                }
            }
        ...
        public static void Lumberjack_HitTree(ExtPlayer player, int TreeIndex)
        {
            try
            {
                ...
                if (TreesData.ContainsKey(TreeIndex))
                {
                    ...
                    if (WorkAxe == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHaveNoAxe), 6000);
                        return;
                    }
                    ...
                    if (treesData.StartedTreeHitPlayerInfo != null)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SomebodyCutting), 6000);
                            return;
                        }
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AxeBroken), 6000);
        ...
        public static void Lumberjack_CutTree(ExtPlayer player, int TreeIndex)
        {
            try
            {
                ...
                if (TreesData.ContainsKey(TreeIndex))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoMoreLogs), 6000);
                    }
                    else
                    {
        ...
        public static void PlayerSellTrees(ExtPlayer player, int stock_item_index, int amount)
        {
            try
            {
                ...
                if (success == true)
                {
                    ...
                    TreeStockLabel.Text = Main.StringToU16(LangFunc.GetText(LangType.De, DataName.Hranilishe,  TreeStock[0], TreeStock[1], TreeStock[2]));
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NotEnoughRes), 6000);
            }
            catch (Exception e)
            {
