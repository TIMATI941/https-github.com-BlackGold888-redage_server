    ...
    class WorkManager : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                //(ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Jobs"), new Vector3(Points[0].X, Points[0].Y, Points[0].Z + 0.5), 10F, 0.3F, 0, new Color(255, 255, 255));
                //(ExtMarker) NAPI.Marker.CreateMarker(1, Points[0] - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1f, new Color(255, 255, 255, 220));

                // blips

                Main.CreateBlip(new Main.BlipData(354, LangFunc.GetText(LangType.De, DataName.Electrostanciya), new Vector3(724.9625, 133.9959, 79.83643), 70, true, 1.2f));
        ...
        {
            LangFunc.GetText(LangType.De, DataName.Electrician),
            LangFunc.GetText(LangType.De, DataName.Postman),
            LangFunc.GetText(LangType.De, DataName.Taximan),
            LangFunc.GetText(LangType.De, DataName.Busdriver),
            LangFunc.GetText(LangType.De, DataName.Gazonman),
            LangFunc.GetText(LangType.De, DataName.Dalnoboy),
            LangFunc.GetText(LangType.De, DataName.Inkassman),
            LangFunc.GetText(LangType.De, DataName.Mechanicman),
        };
        public static SortedList<int, Vector3> Points = new SortedList<int, Vector3>
        {
            ...
            {1, new Vector3(720.61, 153.67567, 80.90749) },  // Electrician job
        ...
        {
            {1, LangFunc.GetText(LangType.De, DataName.Electriciany) },
            {2, LangFunc.GetText(LangType.De, DataName.Postmany) },
            {3, LangFunc.GetText(LangType.De, DataName.Taximany) },
            {4, LangFunc.GetText(LangType.De, DataName.Busdrivery) },
            {5, LangFunc.GetText(LangType.De, DataName.Gazonmany) },
            {6, LangFunc.GetText(LangType.De, DataName.Dalnoboyy) },
            {7, LangFunc.GetText(LangType.De, DataName.Inkassmany) },
            {8, LangFunc.GetText(LangType.De, DataName.Mechanicmany) },
        };
        ...
        public static void Layoff(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData.WorkID != 0)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouLeaveJob), 6000);
                    BattlePass.Repository.UpdateReward(player, 10);
                    UpdateData.Work(player, 0);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouNotJober), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void JobJoin(ExtPlayer player, int job)
        {
            try
            {
                ...
                if (characterData.WorkID != 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BeforeLeaveLastJob), 6000);
                    return;
                }
                if (sessionData.WorkData.OnWork == true)
                {
                ...
                if (characterData.WorkID == job) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouAlreadyJob, JobList[job]), 6000);
                else
                {
                    if (characterData.LVL < JobsMinLvl[job])
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NeedJobsLvl, JobsMinLvl[job]), 6000);
                        return;
                    }
                    if ((job == (int)JobsId.Postman || job == (int)JobsId.Taxi || job == (int)JobsId.CarMechanic) && !characterData.Licenses[1])
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.JobLicB), 10000);
                        return;
                    }
                    if ((job == (int)JobsId.Bus || job == (int)JobsId.Trucker || job == (int)JobsId.CashCollector) && !characterData.Licenses[2])
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.JobLicC), 10000);
                        return;
                    }
                    UpdateData.Work(player, job);

                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouStartedJob, JobList[job]), 8000);
