...
namespace NeptuneEvo.Jobs
{
    class Truckers : Script
    {
        ...
        public void OnResourceStart()
        {
            ...
            {
                Main.CreateBlip(new Main.BlipData(479, LangFunc.GetText(LangType.De, DataName.ZagruzkaTovara), GetProduct[i], 4, true));
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GoToZagruz), 10000);
        ...
        public static void OnGetProductTrucker(ExtPlayer player)
        {
            ...
            if (characterData.WorkID == (int)JobsId.Trucker) 
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Öffne Sie Ihr Telefon um Bestellungen anzunehmen", 10000);
        }
        ...
        public static void playerGotProducts(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DoveziteDo, BusinessManager.BusinessTypeNames[biz.Type]), 6000);
        ...
        public static void OnTrucker(ExtPlayer player, int bizId)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustBeInTruck), 6000);
                    return;
                }
                var uid = sessionData.OrderData.Order;
                ...
                sessionData.OrderData.WayPoint = null;
                if (ow != null) 
                    Notify.Send(ow, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouOrderDeliveried, bizOrder.Name), 6000);
                ...
                sessionData.OrderData.Order = -1;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TrucSuccDelivery), 6000);

                if (sessionData.LastPointTime > DateTime.Now)
                    Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.AnticheatDostavka, player.Name, player.Value));
