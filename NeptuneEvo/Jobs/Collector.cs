    ...
    class Collector : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16(LangFunc.GetText(LangType.De, DataName.TakeMeshki)), TakeMoneyPos + new Vector3(0, 0, 0.3), 30f, 0.4f, 0, new Color(255, 255, 255), true, 0);
            }
            catch (Exception e)
            {
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.InkassStart), 6000);
        ...
        public static void OnTakeMoney(ExtPlayer player)
        {
            try
            {
                ...
                if (sessionData.WorkData.Packages != 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHaveBags, sessionData.WorkData.Packages), 6000);
                    return;
                }
                ...
                Attachments.AddAttachment(player, Attachments.AttachmentsName.MoneyBag);
                SetAtmPoint(player);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GotNewBags), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void InAtm(ExtPlayer player, int Index)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben keine Geldtasche in der Hand", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AtmFull), 6000);
                    return;
                }
                ...
                if (sessionData.WorkData.Packages == 0)
                {
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BackToBaza), 6000);
                ...
                {
                    SetAtmPoint(player);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GoNextAtm), 6000);
                }
