...
namespace NeptuneEvo.Jobs
{
    class Electrician : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                NAPI.TextLabel.CreateTextLabel("~w~Ryan Nelson", new Vector3(720.61, 153.67567, 80.90749), 30f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                if (Main.ServerSettings.IsDeleteProp)
                {
                    ...
                    NAPI.World.DeleteWorldProp(1426534598, new Vector3(440.1373, -999.1079, 42.56145), 30f);
                    NAPI.World.DeleteWorldProp(1426534598, new Vector3(438.006, -976.3385, 42.56371), 30f);
                    NAPI.World.DeleteWorldProp(1426534598, new Vector3(487.0675, -1010.295, 39.7353), 30f);
                    NAPI.World.DeleteWorldProp(977294842, new Vector3(477.9519, -976.1639, 27.41974), 30f);
                    NAPI.World.DeleteWorldProp(977294842, new Vector3(475.1882, -997.1531, 42.69125), 30f);
                    NAPI.World.DeleteWorldProp(977294842, new Vector3(485.8271, -996.1639, 27.41974), 30f);
                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(478.2409, -985.8596, 40.00915), 30f);
                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(481.764, -987.3079, 40.00753), 30f);
                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(485.3017, -985.8329, 40.00811), 30f);
                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(486.7287, -982.3341, 40.008), 30f);
                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(485.2953, -978.8053, 40.0081), 30f);

                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(481.764, -977.2239, 40.00915), 30f);
                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(478.1713,-978.7024, 40.00741), 30f);
                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(476.7524,-982.3341, 40.00814), 30f);
                    NAPI.World.DeleteWorldProp(3259306505, new Vector3(449.3253, -976.1553, 42.70052), 30f);

                    NAPI.World.DeleteWorldProp(1426534598, new Vector3(438.006, -976.3385, 42.56371), 30f);

                    NAPI.World.DeleteWorldProp(1131941737, new Vector3(475.1882, -997.1531, 42.69125), 30f);

                }

                CustomColShape.CreateCylinderColShape(new Vector3(724.9625, 133.9959, 79.83643), 1, 2, 0, ColShapeEnums.Electrician);

                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Drücke\n~r~'Interaktion'"), new Vector3(724.9625, 133.9959, 80.95643), 30f, 0.4f, 0, new Color(255, 255, 255), true, 0);
        ...
        public static void OnElectrician(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoElectrician), 6000);
                    return;
                }
        ...
        public static void StartWork(ExtPlayer player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.StartWorkDay), 6000);
        }
        ...
        public static bool EndWork(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.EndWorkDay), 6000);
        ...
        public void startElectricGame(ExtPlayer player, int shapeId)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                Trigger.ClientEvent(player, "client.electric_minigame_open");
                ...
                player.SetData("electricshapeid", shapeId);
            }
            catch (Exception e)
            {
        ...
        [RemoteEvent("server.electric_minigame_error")]
        public void errorElectricGame(ExtPlayer player)
        {
            try
            {
                player.ResetData("electricshapeid");
        ...
        [RemoteEvent("server.electric_minigame_secc")]
        public void InElectricianPoint(ExtPlayer player)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                int shapeId = player.GetData<int>("electricshapeid");
                if (characterData.WorkID != (int)JobsId.Electrician || !sessionData.WorkData.OnWork || shapeId != sessionData.WorkData.WorkCheck) return;
                if (Checkpoints[shapeId].Position.DistanceTo(player.Position) > 3) return;
