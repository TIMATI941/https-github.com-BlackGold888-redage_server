    ...
    class AutoMechanic : Script
    {
        ...
        public static void mechanicRepair(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YourNotAutoMech), 6000);
                    return;
                }
                if (player == target)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantRepairYourVehicle), 6000);
                    return;
                }
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerMustBeInVeh), 6000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Price5to30), 6000);
                        return;
                    }
                    ...
                    Trigger.ClientEvent(target, "openDialog", "REPAIR_CAR", LangFunc.GetText(LangType.De, DataName.MechanikRepairTo, player.Value, MoneySystem.Wallet.Format(price)));
                    //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MechanikRepairFrom, target.Value, MoneySystem.Wallet.Format(price)), 6000);
                    EventSys.SendCoolMsg(player,"Angebot", "Mechaniker", LangFunc.GetText(LangType.De, DataName.MechanikRepairFrom, target.Value, MoneySystem.Wallet.Format(price)), "", 5000);
        ...
        public static void mechanicPay(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(driver, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerPayedRepair, player.Value), 6000);
                Commands.RPChat("sme", driver, LangFunc.GetText(LangType.De, DataName.RepairVehi));
                driverSessionData.SellItemData = new SellItemData();
                sessionData.SellItemData = new SellItemData();
                ...
                if (qMain.GetQuestsLine(driver, Zdobich.QuestName) == (int)zdobich_quests.Stage11)
                {
                    ...
                    if (driverSessionData.WorkData.PointsCount >= 500)
                    {
                        ...
                        Trigger.SendChatMessage(driver, "!{#fc0}" + LangFunc.GetText(LangType.De, DataName.QuestPartComplete));
                    }
                    else
                    {
                        ...
                        Trigger.SendChatMessage(driver, $"Du verdienst ${driverSessionData.WorkData.PointsCount}. Es verbleiben noch ${500 - driverSessionData.WorkData.PointsCount}.");
                    }
                }
        ...
        public static void buyFuel(ExtPlayer player, int fuel)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustBeMechanikAndWorkCar), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Command3lvl), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustBeAtGas), 6000);
                    return;
                }
                if (fuel <= 0)
                {
                ...
                if (vehicleLocalData == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.UnavaibleForThisVeh), 6000);
                    return;
                }
                if (vehicleLocalData.VehLoadedFuel + fuel > 1000)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FillCanFuel), 6000);
                    return;
                }
                if (!BusinessManager.takeProd(biz.ID, fuel, biz.Products[0].Name, amount))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoFuelAtGasStation), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouFillBack, vehicleLocalData.VehLoadedFuel), 5000);
            }
            catch (Exception e)
            {
        ...
        public static void mechanicFuel(ExtPlayer player, ExtPlayer target, int fuel, int pricePerLitr)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    if (fuel < 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FillErrorLiter), 6000);
                        return;
                    }
                    if (pricePerLitr < 1 || pricePerLitr > 10)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FillPriceError), 6000);
                        return;
                    }
                    ...
                    Trigger.ClientEvent(target, "openDialog", "FUEL_CAR", LangFunc.GetText(LangType.De, DataName.PlayerOfferFill, player.Value, fuel, fuel * pricePerLitr));
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouOfferFill, target.Value, fuel, fuel * pricePerLitr), 7000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void mechanicPayFuel(ExtPlayer player)
        {
            try
            {
                ...
                if (!driver.IsInVehicle)
                {
                    driverSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MechanikMustBeInJobCar), 6000);
                    return;
                }
                ...
                if (vehicleLocalData1 != null && vehicleLocalData2 != null)
                {
                    ...
                    if (vehicleLocalData1.VehLoadedFuel < fuel)
                    {
                        driverSessionData.SellItemData = new SellItemData();
                        sessionData.SellItemData = new SellItemData();
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MechanikNoFuel), 6000);
                        return;
                    }
                    sessionData.WorkData.Player = null;
                    MoneySystem.Wallet.Change(player, -price);
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouPayFillRepairVeh), 6000);
                    Notify.Send(driver, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlayerPayedFillVeh, player.Value), 6000);
                    Commands.RPChat("sme", driver, LangFunc.GetText(LangType.De, DataName.FilledVeh));
