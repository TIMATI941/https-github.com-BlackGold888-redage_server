    ...
    class Miner : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(618, "Mine", new Vector3(124.831116, -397.94046, 41.263977), 4, true));
               ...
               // Main.CreateBlip(new Main.BlipData(618, "Скупщик драгоценностей", new Vector3(1054.20, -1952.84, 32.09), 70, true));
                ...
                MineStockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Lager:\n\nKohle {MineStockOres[0]} stk.\nEisenerz {MineStockOres[1]} stk.\nSchwefelerz {MineStockOres[3]} stk.\nEdelsteine {MineStockOres[2] + MineStockOres[4] + MineStockOres[5]} stk."), new Vector3(-595.855, 2094.609, 131.4356), 5F, 0.5F, 0, new Color(255, 255, 255), true, 0);
                PlantStockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Lager:\n\nKohle {PlantStockOres[0]} stk.\nEisenerz {PlantStockOres[1]} stk.\nGolderz {PlantStockOres[2]} stk.\nEdelsteine {PlantStockOres[4] + PlantStockOres[5]} stk."), new Vector3(1066.399, -1980.138, 31.01464), 5F, 0.5F, 0, new Color(255, 255, 255), true, 0);

                PedSystem.Repository.CreateQuest("s_m_y_construct_01", new Vector3(-1637.993, -800.12006, 10.246064), 57.71f, title: "~y~NPC~w~ Michael\nErzkäufer", colShapeEnums: ColShapeEnums.OresSell);
        ...
        public static void GovMineStockLoad(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PickaxeBroken), 6000);
                    return;
                }

                if (!player.IsInVehicle)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouMustBeInVeh), 6000);
                    return;
                }
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehNoResources), 6000);
                        return;
                    }

                    if (vehicleLocalData.Fraction != (int)Fractions.Models.Fractions.ARMY)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantGruzitVehResc), 6000);
                        return;
                    }
                    ...
                    if (count >= 3200)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.VehMaxResc), 6000);
                        return;
                    }

                    if (MineStockOres[0] == 0 && MineStockOres[1] == 0 && MineStockOres[2] == 0 && MineStockOres[3] == 0 && MineStockOres[4] == 0 && MineStockOres[5] == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WarehouseEmpty), 6000);
                        return;
                    }
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        if (MineStockOres[i] >= 1)
                        {
                            ...
                            if ((MineStockOres[i] + count) < 3200)
                            {
                                ...
                                if (item_type == ItemId.Coal) LoadInfo += LangFunc.GetText(LangType.De, DataName.Puglya, MineStockOres[i]);
                                else if (item_type == ItemId.Iron) LoadInfo += LangFunc.GetText(LangType.De, DataName.Piron, MineStockOres[i]);
                                else if (item_type == ItemId.Sulfur) LoadInfo += LangFunc.GetText(LangType.De, DataName.Psera, MineStockOres[i]);
                                else if (item_type == ItemId.Emerald) LoadInfo += LangFunc.GetText(LangType.De, DataName.Pizumrud, MineStockOres[i]);
                            ...
                            else
                            {
                                ...
                                if (item_type == ItemId.Coal) LoadInfo += LangFunc.GetText(LangType.De, DataName.Puglya, 3200 - count);
                                else if (item_type == ItemId.Iron) LoadInfo += LangFunc.GetText(LangType.De, DataName.Piron, 3200 - count);
                                else if (item_type == ItemId.Sulfur) LoadInfo += LangFunc.GetText(LangType.De, DataName.Psera, 3200 - count);
                                else if (item_type == ItemId.Emerald) LoadInfo += LangFunc.GetText(LangType.De, DataName.Pizumrud, 3200 - count);
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucLoadResToVeh), 6000);
                    SaveMineStocks(1);

                    LoadInfo = LoadInfo.Remove(LoadInfo.Length - 2);
                    Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeOre, LangFunc.GetText(LangType.De, DataName.LoadedFromWarehouse, LoadInfo));
                }
            }
            catch (Exception e)
            {
        ...
        public static void PlayerEnteredMineShape(ExtPlayer player, int zone)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OnlyGosMine), 6000);
                    return;
                }
        ...
        public static void RemoteEvent_PlayerStartedMining(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie verlagern die Ladung", 3000);
                    return;
                }
                
                /*if (DateTime.Now.Hour > 10 && DateTime.Now.Hour != 22 && DateTime.Now.Hour != 23)
                { // todo translate
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OgranVremyaMiner), 6000);
                    return;
                }*/
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath && !sessionData.AntiAnimDown)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHaveNoPickaxe), 6000);
                        return;
                    }*/
                ...
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantStartJobNow), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void RemoteEvent_PlayerMinedGovResources(ExtPlayer player)
        {
            try
            {
                ...
                if (rand > 17 && rand <= 32) // ore_iron (Eisenerz)
                {
                ...
                else if (rand > 1 && rand <= 17) // ore_sulfur (Schwefelerz)
                {
                ...
                else if (rand == 1) // ore_emerald (Smaragd)
                {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucResDobil), 6000);
        ...
        public static void RemoteEvent_PlayerMinedResources(ExtPlayer player)
        {
            try
            {
                ...
                var item_type = ItemId.Coal; // ore_coal (Kohle)

                if (rand > 9 && rand <= 23) // ore_iron (Eisenerz)
                {
                    //item_name = "железную руду";
                    item_type = ItemId.Iron;
                }
                else if (rand > 2 && rand <= 9) // ore_gold (Golderz)
                {
                    //item_name = "золотую руду";
                    item_type = ItemId.Gold;
                }
                else if (rand == 2) // ore_emerald (Smaragd)
                {
                    //item_name = "изумруд";
                    item_type = ItemId.Emerald;
                }
                else if (rand == 1) // ore_ruby (Rubin)
                {
                    //item_name = "рубин";
        ...
        public static void RemoteEvent_PlayerPutGovResources(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SklafFulled), 6000);
                Trigger.ClientEvent(player, "mineJob_updateOreCarryStatus", false);
        ...
        public static void RemoteEvent_PlayerSellOres(ExtPlayer player, int stock_item_index, int amount)
        {
            try
            {
                ...
                if (count < amount) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NotEnoughRes), 6000);
                else
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SellResources, Wallet.Format(totalmoney)), 6000);
                    ...
                    if (qMain.GetQuestsLine(player, Zdobich.QuestName) == (int)zdobich_quests.Stage11)
                    {
                        ...
                        if (sessionData.WorkData.PointsCount >= 500)
                        {
                            ...
                            Trigger.SendChatMessage(player, "!{#fc0}" + LangFunc.GetText(LangType.De, DataName.QuestPartComplete));
                        }
                        else
                        {
                            ...
                            Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.YouEarnedJob, sessionData.WorkData.PointsCount, 500 - sessionData.WorkData.PointsCount));
                        }
                    }
                }
            }
        ...
        public static void SaveMineStocks(int mine_index)
        {
            try
            {
                ...
                {
                    MineStockLabel.Text = Main.StringToU16($"~y~Lagereinrichtung:\n\nKohle {MineStockOres[0]} stk.\nEisenerz {MineStockOres[1]} stk.\nSchwefelerz{MineStockOres[3]} stk.\nEdelsteine {MineStockOres[2] + MineStockOres[4] + MineStockOres[5]} stk.");
                }
                else if (mine_index == 2 && PlantStockLabel != null)
                {
                    PlantStockLabel.Text = Main.StringToU16($"~y~Lagereinrichtung:\n\nKohle {PlantStockOres[0]} stk.\nEisenerz {PlantStockOres[1]} stk.\nGolderz {PlantStockOres[2]} stk.\nEdelsteine {PlantStockOres[4] + PlantStockOres[5]} stk.");
                }
            }
            catch (Exception e)
            {
