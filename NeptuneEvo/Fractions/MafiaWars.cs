    ...
    class MafiaWars : Script
    {
        ...
        public void OnResourceStart()
        {
            try
            {
                ...
                foreach (var position in warPoints)
                {
                    ...
                    warBlips.Add(i, (ExtBlip)NAPI.Blip.CreateBlip(543, position, 1, 40, Main.StringToU16(LangFunc.GetText(LangType.De, DataName.BizWar)), 255, 0, true, 0, 0));
                    i++;
                }
            }
            catch (Exception e)
        ...
        public static void CMD_startBizwar(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BizWarBan, characterData.BanCrimeReason), 5000);
                    return;
                }
                ...
                if (biz.Mafia == fracId)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCantCaptYourBiz), 6000);
                    return;
                }
                
                if (biz.Mafia == -1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BizNotBelongsToMaf), 6000);
                    return;
                }
                
                if (Main.ServerNumber != 0 && (DateTime.Now.Hour < 13 || DateTime.Now.Hour >= 23))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MafiaTimeWar), 6000);
                    return;
                }
                ...
                if (Main.ServerNumber != 0 && DateTime.Now < nextCaptDate[fracId])
                {
                    ...
                    if (g.Minute >= 5)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WarTimeMafHours, g.Hour, g.Minute, g.Second), 6000);
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WarTimeMaf, g.Minute, g.Second), 6000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoOnline), 6000);
                    return;
                }
                
                if (World.War.Repository.Wars.Values.Any(w => w.Type == WarType.Mafia))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WarBizGoing), 6000);
                    return;
                }
                ...
                Trigger.SendToAdmins(1, LangFunc.GetText(LangType.De, DataName.AdminBizWar, ChatColors.Red, player.Name, player.Value, Manager.GetName(fracId), Manager.GetName(biz.Mafia)));
                ...
                Manager.sendFractionMessage(biz.Mafia, LangFunc.GetText(LangType.De, DataName.BizWarDefendStart, Manager.GetName(fracId)));
                Manager.sendFractionMessage(fracId, LangFunc.GetText(LangType.De, DataName.YouStartBizWar));
        ...
        public static void CMD_tpbizwar(ExtPlayer player)
        {
            try
            {
                ...
                if (war != null && (fracId == war.ProtectingId || fracId == war.AttackingId))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyGoToWarZone), 6000);
                        return;
                    }
                    if (sessionData.UsedTPOnCaptureOrBizwar == 2)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyInWarZone), 6000);
                        return;
                    }
                    ...
                    if (fracId == war.ProtectingId)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WastpCenterWar), 5000);
                    }
                    else
                    {
                        sessionData.PositionCaptureOrBizwar = player.GetPosition();
                        ...
                        sessionData.UsedTPOnCaptureOrBizwar = 1;
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WasTPNearWar), 7000);
                    }
                }
            }
            catch (Exception e)
