    ...
    class ElectionsSystem : Script
    {
        ...
        public static void OnElectionPoint(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.MinLvlVote, Main.ServerSettings.MinVoteLvl));
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MinLvlVote, Main.ServerSettings.MinVoteLvl), 6000);
                    return;
                }
                if (CandidateList.Count >= 2)
                {
                    if (!CheckPlayerVoted(accountData.Login, ElectionPoint.Election)) OpenElectionMenu(player);
                    else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.AlreadyVoted));
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.NoCandidates));
            }
            catch (Exception e)
            {
        ...
        public static void OpenElectionMenu(ExtPlayer player)
        {
            try
            {
                ...
                frameList.Header = LangFunc.GetText(LangType.De, DataName.Elections); 
                    ...
                    if (ElectionPoint.Opened)
                    {
                        ...
                        if (!CheckPlayerVoted(accountData.Login, ElectionPoint.Election))
                        {
                            if (CandidateList[elecid].Name.Equals(name))
                            {
                                ...
                                Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.MadeVote, ElectionPoint.Election, name));
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MadeVote, ElectionPoint.Election, name), 6000);
                            }
                        }
                    }
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TooFar), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void ElectionReload(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Reloading), 6000);
                if (ElectionPoint.Opened)
                {
            ...
            try
            {
                ...
                if (resultPoints != null && resultPoints.Rows.Count != 0)
                {
                    ...
                    if (ElectionPoint.Opened)
                    {
                        ...
                        ElectionPoint.Info = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel($"Abstimmungspunkt\nWahlen Nr{ElectionPoint.Election}\n~w~Drücke\n~r~'Interaktion'", new Vector3(ElectionPoint.Position.X, ElectionPoint.Position.Y, ElectionPoint.Position.Z + 1), 10f, 1f, 0, new Color(255, 255, 255), dimension: 0);
                    }
                }
