...
namespace NeptuneEvo.Fractions
{
    class AlcoFabrication : Script
    {
        ...
        {
            { (int) Models.Fractions.LCN, "Club Bahama Mamas West" },
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(121, "Club Bahama Mamas West", new Vector3(-1388.761, -586.3921, 29.09945), 0, true));
                ...
                //Main.CreateBlip(new Main.BlipData(121, "Club Diamond", new Vector3(945.4526, 16.871, 115.0442), 0, true));
                
                Main.CreateBlip(new Main.BlipData(136, "Cafe Stand Up", new Vector3(-439.601, 271.704, 82.015), 0, true));
                Main.CreateBlip(new Main.BlipData(136, "China Imbiss", new Vector3(-158.242, 303.829,98.822), 0, true));
                ...
                foreach (KeyValuePair<int, Vector3> pair in ExitAlcoShop)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~w~Ausgang", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
                #region Unloadpoints
                foreach (KeyValuePair<int, Vector3> pair in UnloadPoints)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel($"~w~Mats Entladen", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
                #region BuyPoints
                foreach (KeyValuePair<int, Vector3> pair in BuyPoints)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~w~Alchohol kaufen", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
            }
            catch (Exception e)
                ...
                if (player.GetFractionId() != index)
                {
                    switch (index)
                    {
                        case 10:
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ClubDoorClosed), 6000);
                                return;
                            }
                            break;
                        case 12:
        ...
        public static void OnUnloadPoints(ExtPlayer player, int index)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ClubNotInFraction, Manager.GetName(index)), 6000);
                    return;
                }
                ...
                if (matCount == 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarNoMats), 6000);
                    return;
                }
                if (ClubsStocks[club].Materials >= MaxMats)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WarehouseFilled), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FromCarToClubWarehouse), 6000);
                Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeMedkits, LangFunc.GetText(LangType.De, DataName.FtableLoadedFromCarToClubWarehouse, matCount));
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (action)
                {
                    case 0: // buy
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Nicht genug Ware", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du kaufst {Chars.Repository.ItemsInfo[invItem].Name}", 3000);
                        return;
                    case 1: // take
                        if (alcoCounts[index] <= 0)
                        {
                        ...
                        ClubsStocks[fractionId].UpdateLabel();
                        //OpenBuyAlcoholMenu(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du nimmst {Chars.Repository.ItemsInfo[invItem].Name}. Im Lager verbleiben {alcoCounts[index] - 1}stk", 3000);
                        return;
                    case 2: // craft
                        if (alcoCounts[index] >= 80)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Im Lager Maximum: {Chars.Repository.ItemsInfo[invItem].Name}", 3000);
                            return;
                        }
                        if (ClubsStocks[fractionId].Materials < DrinksMats[index])
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Nicht genug Mats im Lager", 3000);
                            return;
                        }
                        ...
                        ClubsStocks[fractionId].UpdateLabel();
                        //OpenBuyAlcoholMenu(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du erstellst {Chars.Repository.ItemsInfo[invItem].Name}. Auf Lager: {alcoCounts[index] + 1}stk", 3000);
                        return;
                    case 3: // setprice
                        Trigger.ClientEvent(player, "openInput", "Preis festlegen", "Geben Sie den Preis in % an", 3, "club_setprice");
                        return;
                    default:
                        // Not supposed to end up here. 
                        break;
        ...
        public static void SetAlcoholPrice(ExtPlayer player, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Legen Sie den Preis fest zwischen 50% bis 150%", 3000);
                    return;
                }
                ClubsStocks[fracId].PriceModifier = price / 100.0f;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie haben den Preis auf {price}% geändert", 3000);
            }
            catch (Exception e)
            {
        ...
        internal class Stock
        {
            ...
            {
                Label = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel($"~w~Mats: ~r~{mats}\n~w~Alcohol: ~r~{a1 + a2 + a3}", pos, 30f, 0.3f, 0, new Color());
