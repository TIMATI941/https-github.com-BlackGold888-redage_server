    ...
    class LsNewsSystem : Script
    {
        ...
        public void OnResourceStartHandler()
        {
            try
            {
                ...
                { 
                    ...
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Aufzug"), new Vector3(FirstNewsLiftTP[i].X, FirstNewsLiftTP[i].Y, FirstNewsLiftTP[i].Z), 5F, 0.3F, 0, new Color(255, 255, 255)); 
                ...
                { 
                    ...
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Aufzug"), new Vector3(SecondNewsLiftTP[i].X, SecondNewsLiftTP[i].Y, SecondNewsLiftTP[i].Z), 5F, 0.3F, 0, new Color(255, 255, 255)); 
                ...
                PedSystem.Repository.CreateQuest("ig_englishdave_02", new Vector3(-588.256, -936.6138, 23.867977), -4.159257f, title: "~y~NPC~w~ Journalist Stefan\nMitarbeiter rufen", colShapeEnums: ColShapeEnums.CallNewsMember);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (interact)
                {
                    case 1:
                        ...
                        else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoNEWS), 6000);
                        return;
                    case 2:
                    case 3:
        ...
        { 
            ...
            { 
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GoingToFloor, index + 1), 6000); 
            } 
            catch (Exception e) 
            { 
        ...
        { 
            ...
            { 
                ...
                Trigger.ClientEvent(player, "openDialog", "CallNewsMemberDialog", LangFunc.GetText(LangType.De, DataName.AreYouWantToCallGov)); 
            } 
            catch (Exception e) 
            { 
        ...
        public static void AdvertTake(ExtPlayer player, int index)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AdAlreadyTaken), 6000);
                    return;
                }
        ...
        public static void AdvertSend(ExtPlayer player, int ID, string answer)
        {
            try
            {
                ...
                {
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.AdUnavaibleForEdit));
                    Remove(ID, player);
                }
            }
            catch (Exception e)
        ...
        public static async Task AddAdvert(ExtPlayer player, string text, string link, int type, bool isPremium, int price)
        {
            try
            {
                ...
                //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AdDone), 6000);
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.News, LangFunc.GetText(LangType.De, DataName.AdDone), DateTime.Now);
        ...
        public static void UpdateAnswer(ExtPlayer player, int repID, string response, bool deleted = false)
        {
            try
            {
                ...
                if (characterData.AdminLVL == 0 && memberFractionData.Id == (int) Models.Fractions.LSNEWS && memberFractionData.Rank <= 1)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoAccess), 6000);
                    return;
                }
                if (!AdvertList.ContainsKey(repID))
                {
                    if (deleted) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindAdNumber), 6000);
                    return;
                }
                ...
                {
                    AdvertTake(player, repID);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AdMustBe150), 6000);
                    return;
                }
                ...
                if (!deleted)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AdToList, advert.Author, CompletedAdverts.Count), 6000);
                    ...
                    if (target.IsCharacterData()) 
                        Players.Phone.Messages.Repository.AddSystemMessage(target, (int)DefaultNumber.News, LangFunc.GetText(LangType.De, DataName.AdToList, advert.Author, CompletedAdverts.Count), DateTime.Now);
                }
                else
                {
                    if (characterData.AdminLVL != 0) GameLog.Admin($"{player.Name}", $"delAd", $"{advert.Author}");
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DelAd, advert.Author), 6000);
                    var target = (ExtPlayer) NAPI.Player.GetPlayerFromName(advert.Author);
                    response += LangFunc.GetText(LangType.De, DataName.deletedad);
                    if (target.IsCharacterData())
                        Players.Phone.Messages.Repository.AddSystemMessage(target, (int)DefaultNumber.News, LangFunc.GetText(LangType.De, DataName.DelAdReason, player.Name, response), DateTime.Now);
                }
                Remove(repID);
        ...
        public static void CheckCompletedAds()
        {
            try
            {
                if (CompletedAdverts.Count >= 1)
                {
                    ...
                    if (ad.AuthorSIM >= 1) NAPI.Chat.SendChatMessageToAll(color + LangFunc.GetText(LangType.De, DataName.AdWithPhone, ad.EditedAD, ad.AuthorSIM));
                    else NAPI.Chat.SendChatMessageToAll(color + LangFunc.GetText(LangType.De, DataName.Ad, ad.EditedAD));
                    NAPI.Chat.SendChatMessageToAll(color2 + LangFunc.GetText(LangType.De, DataName.AdRewriter, ad.Editor));
                }
