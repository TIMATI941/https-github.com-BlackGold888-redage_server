...
namespace NeptuneEvo.Fractions.LSNews.LiveStream
{
    public class LiveStream
    {
        ...
        public void Start()
        {
            Manager.sendFractionMessage(LsNewsFracid, $"FF8C00 [F] {_owner.Name} ({_owner.Id}) Livestream gestartet");
        }

        public void Stop()
        {
            Manager.sendFractionMessage(LsNewsFracid, $"FF8C00 [F] {_owner.Name} ({_owner.Id}) Livestream beendet");
        }
        ...
        private void SendMessageToListeners(IEnumerable<ExtPlayer> listeners, string msg, bool isOwner)
        {
            ...
            {
                string prefix = isOwner ? "#FAFAD2 [Weazel News] Moderator: " : " #FAFAD2 [Weazel News] Gast: ";
