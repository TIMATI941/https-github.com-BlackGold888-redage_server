...
namespace NeptuneEvo.Fractions
{
    class Police : Script
    {
        ...
        private static Vector3 ArrestPosition = new Vector3(435.78714, -978.1241, 25.573385); //arrest stecken
        public static Vector3 GunsPosition = new Vector3(479.0797, -996.76697, 30.69196);
        public static Vector3 CloakroomPosition = new Vector3(436.51547, -982.11237, 32.147907); //umkleide
        private static Vector3 CloakroomSpecialPosition = new Vector3(440.84494, -994.9458, 32.14776);// umkleide SWAT
        public static Vector3 PrisonPosition = new Vector3(422.2721, -979.8623, 25.573387); //knast zelle
        public static Vector3 ExitPrisonPosition = new Vector3(424.83127, -980.834, 25.573368); //knast ausgang
        private static Vector3 StockPosition = new Vector3(437.2119, -989.5931, 32.14776); //lager
        private static Vector3 VehicleBoostPosition = new Vector3(1408.2571, 183.5331, -49.280865);//463.32733, -1019.0836, 28.10166
        private static Vector3 VehicleRepairPosition = new Vector3(455.49554, -978.81506, 25.22283); //autos reparieren
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Fahrzeug reparieren"), VehicleRepairPosition, 5F, 0.3F, 0, new Color(255, 255, 255));

                Ped ped = PedSystem.Repository.CreateQuest("s_m_y_cop_01", new Vector3(428.05142, -986.46063, 29.073233), 89.897484f, title: "~y~NPC~w~ Officer Lenny\nWache", colShapeEnums: ColShapeEnums.FracPolic);
                PedSecretaryId = ped.Value;

                PedSystem.Repository.CreateQuest("s_m_m_fibsec_01", new Vector3(437.4441, -993.462, 32.147697), 88.88336f, title: "~y~NPC~w~ Agent Jacob\nLager", colShapeEnums: ColShapeEnums.FracPolic);
                PedSystem.Repository.CreateQuest("s_f_y_cop_01", new Vector3(442.5702, -978.62854, 30.689486), 100.0675f, title: "~y~NPC~w~ Officer Jenny\nMitarbeiter rufen", colShapeEnums: ColShapeEnums.CallPoliceMember); //GOV
            }
            catch (Exception e)
            {
        ...
        { 
            ...
            { 
                ...
                Trigger.ClientEvent(player, "openDialog", "CallPoliceMemberDialog", LangFunc.GetText(LangType.De, DataName.AreYouWantToCallGov)); 
            } 
            catch (Exception e) 
            { 
        ...
        public static void Open(ExtPlayer player, int index)
        {
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoPolice), 6000);
                    return;
                }
        ...
        public static void Perform(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoBagDrillMoney), 6000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Reward, Main.PoliceAward), 6000);
            return;
        }
        ...
        public static string OnCallPolice(ExtPlayer player, string reason)
        {
            try
            {
                ...
                if (Manager.FractionMembersCount(new [] {(int) Models.Fractions.POLICE, (int) Models.Fractions.FIB, (int) Models.Fractions.SHERIFF}) == 0)
                    return LangFunc.GetText(LangType.De, DataName.NoPoliceNear);
                
                if (characterData.AdminLVL == 0 && DateTime.Now < sessionData.TimingsData.NextCallPolice)
                    return LangFunc.GetText(LangType.De, DataName.AlreadyCallPolice);
                ...
                if (characterData.Unmute > 0)
                {
                    ...
                    return "Wir können leider Ihr Anruf nicht entgegen nehmen";
                }
                ...
                Manager.sendFractionMessage((int) Models.Fractions.POLICE, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.PoliceCallFrom, player.Name, player.Value, reason), true);
                //Manager.sendFractionMessage(9, $"Поступил вызов от ({player.Value}) - {reason}");
                Manager.sendFractionMessage((int) Models.Fractions.FIB, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.PoliceCallFrom, player.Name, player.Value, reason), true);
                Manager.sendFractionMessage((int) Models.Fractions.SHERIFF, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.PoliceCallFrom, player.Name, player.Value, reason), true);

                if (characterData.Gender) 
                    Commands.RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.HeCallPolice));
                else 
                    Commands.RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.SheCallPolice));
                
                return "Dispatch angenommen";
            }
            catch (Exception e)
            {
        ...
        public static void acceptCall(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (target != null && target.IsCharacterData() && PoliceCalls.ContainsKey(target))
                {
                    ...
                    {
                        Manager.sendFractionMessage(fracId, LangFunc.GetText(LangType.De, DataName.CallAccept, player.Name.Replace("_"," "), target.Value));
                        Manager.sendFractionMessage(fracId, "!{#F08080}[F] " + LangFunc.GetText(LangType.De, DataName.CallAccept, player.Name.Replace("_"," "), target.Value), true);
                ...
                else if (PoliceHouseCalls.ContainsKey(id))
                {
                    ...
                    if (Configs.IsFractionPolic(fracId))
                    {
                        Manager.sendFractionMessage(fracId, $"{player.Name.Replace('_', ' ')} hat den Dispatch angenommen von Haus Nr{id}.");
                        Manager.sendFractionMessage(fracId, "!{#F08080}[F] " + $"{player.Name.Replace('_', ' ')} hat den Dispatch angenommen von Haus Nr{id}", true);
                        player.AddTableScore(TableTaskId.Item5);
                    }
                }
                else if (PoliceCarsCalls.ContainsKey(id))
                {
                    ...
                    if (Configs.IsFractionPolic(fracId))
                    {
                        Manager.sendFractionMessage(fracId, $"{player.Name.Replace('_', ' ')} hat den Dispatch angenommen von Fahrzeug Alarmanlage Nr{id}.");
                        Manager.sendFractionMessage(fracId, "!{#F08080}[F] " + $"{player.Name.Replace('_', ' ')} hat den Dispatch angenommen von Fahrzeug Alarmanlage Nr{id}", true);
                        player.AddTableScore(TableTaskId.Item5);
                ...
                else if (PoliceSafesCalls.ContainsKey(id))
                {
                    ...
                    if (Configs.IsFractionPolic(fracId))
                    {
                        Manager.sendFractionMessage(fracId, $"{player.Name.Replace('_', ' ')} hat den Dispatch angenommen von Tresor raub");
                        Manager.sendFractionMessage(fracId, "!{#F08080}[F] " + $"{player.Name.Replace('_', ' ')} hat den Dispatch angenommen von Tresor raub", true);
                        player.AddTableScore(TableTaskId.Item5);
                        player.AddTableScore(TableTaskId.Item7);
                    }
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindCall), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void clearWantedLvl(ExtPlayer sender, params object[] arguments)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight, LangFunc.GetText(LangType.De, DataName.PassNumberDoesnt), 6000);
                        return;
                    }
                ...
                catch
                {
                    ...
                    if (!Main.PlayerNames.Values.Contains(target))
                    {
                        ...
                        {
                            Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight, LangFunc.GetText(LangType.De, DataName.CantFindPlayerOrVeh), 6000);
                            return;
                        }
                    }
                ...
                if (vehiclenumb.Equals("null"))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindMan), 6000);
                        return;
                    }
                    ...
                    Notify.Send(sender, NotifyType.Success, NotifyPosition.TopCenter, LangFunc.GetText(LangType.De, DataName.YouRemoveWantedPass, target), 6000);
                    Fractions.Table.Logs.Repository.AddLogs(sender, FractionLogsType.Su, $"Fahndung von {target} ({tauuid}) wurde gelöscht");
                    ...
                    foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    {
                        ...
                        if (foreachMemberFractionData.Id == (int) Models.Fractions.POLICE || foreachMemberFractionData.Id == (int) Models.Fractions.FIB)
                            Trigger.SendChatMessage(foreachPlayer, "!{#FF8C00}" + LangFunc.GetText(LangType.De, DataName.RemoveWantedFrom, sender.Name, target));
                    }
                }
                else
                {
                    if (WantedVehicles.ContainsKey(vehiclenumb))
                    {
                        ...
                        Notify.Send(sender, NotifyType.Success, NotifyPosition.TopCenter, LangFunc.GetText(LangType.De, DataName.YouRemoveWantedVeh, vehiclenumb), 6000);
                        Fractions.Table.Logs.Repository.AddLogs(sender, FractionLogsType.Su, $"Fahndung vom Fahrzeug mit dem Kennzeichen ({vehiclenumb}) wurde gelöscht");
                    }
                    else
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight, LangFunc.GetText(LangType.De, DataName.PersonalVehWanted), 6000);
                        return;
                    }
                }
            }
        ...
        public static void checkNumber(ExtPlayer sender, params object[] arguments)
        {
            try
            {
                ...
                if (vehicleData == null)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoCarWithThisNumber), 6000);
                    return;
                }
        ...
        public static void checkPerson(ExtPlayer player, params object[] arguments)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (!Main.PlayerNames.ContainsKey(pasport))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomRight, LangFunc.GetText(LangType.De, DataName.PassNumberDoesnt), 6000);
                        return;
                    }
                ...
                catch
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomRight, LangFunc.GetText(LangType.De, DataName.CantFindPlayer), 6000);
                        return;
                    }
                ...
                try
                {
                    ...
                    fraction_name = fraction_name == String.Empty ? "keine" : fraction_name;

                    string numberInfo = targetCharacterData.Sim == -1 ? "keine" : targetCharacterData.Sim.ToString();

                    int wantedLvl = (targetCharacterData.WantedLVL == null) ? 0 : targetCharacterData.WantedLVL.Level;
                    string gender = (targetCharacterData.Gender) ? LangFunc.GetText(LangType.De, DataName.Mans) : LangFunc.GetText(LangType.De, DataName.Womens);
                ...
                catch
                {
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            var gender = (genderBool) ? LangFunc.GetText(LangType.De, DataName.Mans) : LangFunc.GetText(LangType.De, DataName.Womens);

                            var fraction_name = "keine";
                            ...
                            var numberInfo = sim == -1 ? "keine" : sim.ToString();
                ...
                if (vehicleLocalData != null)
                {
                        ...
                        sender.VehicleSeat == (int)VehicleSeat.Driver)
                    {
                        if (Configs.IsFractionPolic(sender.GetFractionId()))
                        {
                            ...
                            if (characterData.Gender) Commands.RPChat("sme", sender, LangFunc.GetText(LangType.De, DataName.HeOnComputer));
                            else Commands.RPChat("sme", sender, LangFunc.GetText(LangType.De, DataName.SheOnComputer));
                        }
                    }
                }
            }
        ...
        public static void PlayerSoloArrest(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouAlreadyInPrison), 6000);
                    return;
                }
                else if (characterData.WantedLVL == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouNotWanted), 6000);
                    return;
                }
                else if (Main.IHaveDemorgan(player, true)) return;
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouArrested, minutes), 6000);

                FractionCommands.arrestPlayer(player);

                Manager.sendFractionMessage((int) Models.Fractions.POLICE, "!{#FF8C00}[F] " + LangFunc.GetText(LangType.De, DataName.PoliceSdalsya, player.Name, minutes), true);
                Manager.sendFractionMessage((int) Models.Fractions.SHERIFF, "!{#FF8C00}[F] " + LangFunc.GetText(LangType.De, DataName.PoliceSdalsya, player.Name, minutes), true);
                Manager.sendFractionMessage((int) Models.Fractions.FIB, "!{#FF8C00}[F] " + LangFunc.GetText(LangType.De, DataName.PoliceSdalsya, player.Name, minutes), true);
                Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.Arrest, $"{player.Name} ({characterData.UUID}) hat sich gestellt und wurde ins Gefängnis für {minutes} Minuten gesteckt");
            }
            catch (Exception e)
            {
        ...
        public static void TakeIllegalStuff(ExtPlayer player)
        {
            try
            {
                ...
                if (rewardSumm > 0)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucSdalNelegal, rewardSumm), 6000);
                    Manager.sendFractionMessage((int) Models.Fractions.POLICE, "!{#FF8C00}[F] " + LangFunc.GetText(LangType.De, DataName.SdalNelegal, player.Name, rewardSumm), true);
                    Manager.sendFractionMessage((int) Models.Fractions.SHERIFF, "!{#FF8C00}[F] " + LangFunc.GetText(LangType.De, DataName.SdalNelegal, player.Name, rewardSumm), true);
                    Manager.sendFractionMessage((int) Models.Fractions.FIB, "!{#FF8C00}[F] " + LangFunc.GetText(LangType.De, DataName.SdalNelegal, player.Name, rewardSumm), true);
                }
                else
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoNelegal), 6000);
                }
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (interact)
                {
                    ...
                    case 3:
                        ...
                        if (!is_warg)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoEmergency), 6000);
                            return;
                        }
                    ...
                    case 5:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Warehouse2Closed), 6000);
                            return;
                        }
                    ...
                    case 6:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustCarPolice), 6000);
                            return;
                        }
                        player.Vehicle.SetSharedData("BOOST", 20);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CarBoost), 6000);
                        return;
                    case 9:
                        if (!sessionData.WorkData.OnDuty)
                        {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RepairFlatVeh), 6000);
                            return;
                        }
                        if (DateTime.Now < Main.NextFixcarPoliceVeh)
                        {
                            ...
                            if (g.Hour >= 1) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NextRepair1h, g.Hour, g.Minute, g.Second), 6000);
                            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NextRepair,  g.Minute, g.Second), 6000);
                            return;
                        }
                        ...
                        if (vehicleLocalData != null)
                        {
                            if (vehicleLocalData.Access == VehicleAccess.Fraction && vehicleLocalData.Fraction == (int) Models.Fractions.POLICE)
                            {
                                ...
                                Commands.RPChat("sme", player, LangFunc.GetText(LangType.De, DataName.RepairedVeh));
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucRepairVeh), 6000);
                            }
                            else
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustCarPolice), 6000);
        ...
        public static void OpenSpecialPoliceMenu(ExtPlayer player)
        {
            try
            {
                ...
                if (!is_warg)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Alarmstufe nicht eingeschaltet", 3000);
                    return;
                }
