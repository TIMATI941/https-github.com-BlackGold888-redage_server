...
namespace NeptuneEvo.Fractions
{
    class Ticket : Script
    {
        ...
        public static async void OnResourceStart()
        {
            ...
            //
            PedSystem.Repository.CreateQuest("s_m_m_ciasec_01", PedPos, PedRot, questName: QuestName, title: "~y~NPC~w~ Jacob\nOrdnungsamt", colShapeEnums: ColShapeEnums.ImpoundLotPed, pedBlipData: new PedBlipData
            {
                BlipId = 68,
                BlipName = "Abschlepphof",
                BlipColor = 4
        ...
        public static void OnImpoundLot(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MustEvac), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucEvac,payment), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void OpenTicket(ExtPlayer player, ExtVehicle vehicle)
        {
            ...
            if (vehicle.Model == NAPI.Util.GetHashKey("flatbed"))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoEvacEvac), 5000);
                return;
            }
            if (CarException(vehicle.Model))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantEvac), 5000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoEvac), 6000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoEvacPlace), 6000);
                return;
            }
            ...
            switch (vehicleLocalData.Access)
            {
                ...
                case VehicleAccess.Fraction:
                    ...
                    switch ((SafeZones.ZoneName) sessionData.InsideSafeZone)
                    {
                        ...
                        case SafeZones.ZoneName.EMSpark:
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantEvac), 6000);
                                return;
                            }
                            break;
                ...
                case VehicleAccess.Organization:
                    ...
                    data["holder"] = LangFunc.GetText(LangType.De, DataName.OrgWithName, organizationData.Name);
                    break;
                default:
        ...
        public void OnTicketEnd(ExtPlayer player, string ticketText, int ticketPrice, string cameraLink, bool isEvac)
        {
            ...
            if (ticketPrice < 100 || ticketPrice > 500) {
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,LangFunc.GetText(LangType.De, DataName.EvacSum), 5000);
                return;
            }
            if (cameraLink.Length < 1 || ticketText.Length >= 45) {
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,LangFunc.GetText(LangType.De, DataName.EvacSymbols), 6000);
                return;
            }
            ...
            if (isEvac)
            {
                ...
                {                    
                    ...
                    if (g.Hour >= 1) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoEvac1Hours, g.Hour, g.Minute, g.Second), 10000);
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoEvac0Hours, g.Minute, g.Second), 10000);
                    return;
                }
                if (vehicleLocalData.Occupants.Count > 0 || player.Position.DistanceTo(vehicleTicket.Position) > 10)
                {                    
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoEvacSidit), 7000);
                    return;
                }
            ...
            switch (vehicleLocalData.Access)
            {
                ...
                case VehicleAccess.Fraction:
                    ...
                    if (isEvac)
                    {
                        ...
                        Manager.sendFractionMessage(fractionData.Id, LangFunc.GetText(LangType.De, DataName.EvacShtrafS, player.Name.Replace("_"," "), vehicleTicket.NumberPlate));
                    }
                    else 
                        Manager.sendFractionMessage(fractionData.Id, LangFunc.GetText(LangType.De, DataName.EvacShtraf, ticketPrice));
                ...
                case VehicleAccess.Organization:
                    ...
                    if (isEvac)
                    {
                        ...
                        fbAttach.Hash = vehicleTicket.Model;
                        VehicleStreaming.DeleteVehicle(vehicleTicket);
                        Manager.sendOrganizationMessage(orgId, LangFunc.GetText(LangType.De, DataName.EvacShtrafS, player.Name.Replace("_"," "), vehicleTicket.NumberPlate));
                    }
                    else
                        Manager.sendOrganizationMessage(orgId, LangFunc.GetText(LangType.De, DataName.EvacShtraf, ticketPrice));
                    player.AddTableScore(TableTaskId.Item9);
                    player.AddTableScore(TableTaskId.Item15);
            ...
            if (target.IsCharacterData()) 
                Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.EvacShtraf, ticketPrice), 6000);
            player.AddTableScore(TableTaskId.Item9);
            player.AddTableScore(TableTaskId.Item15);
            ...
            {
                if (target.IsCharacterData()) 
                    Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.EvacShtrafS, player.Name.Replace("_"," "), vehicleTicket.NumberPlate), 10000);
        ...
        public void OnTicketPayment(ExtPlayer player, int autoId)
        {
                ...
                if (vehicleTicket.Type == VehicleTicketType.Player)
                {
                    ...
                    if (vehiclesNumber.Contains(vehicleTicket.VehNumber))
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            $"Sie haben Ihr Strafzettel bezahlt", 3000);
            ...
            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SomethingWrong), 5000);
        }
