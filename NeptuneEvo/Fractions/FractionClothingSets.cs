    ...
    public class FractionClothingSets : Script
    {
        ...
        public void OnResourceStart()
        {
            try
            {
                ...
                {
                    string sql = $"UPDATE fractions SET clothingsets='{JsonConvert.SerializeObject(item.Value)}' WHERE id={(int)item.Key};";
                    // Führen Sie das UPDATE-Statement mit Ihrer Datenschnittstelle, z. B. MySqlCommand, aus
                    // Beispiel:
        ...
        public static void OpenFractionClothingSetsMenu(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCantGetForm), 6000);
                    return;
                }

                var frameList = new FrameListData();
                frameList.Header = LangFunc.GetText(LangType.De, DataName.WorkClothes);
                ...
                if (sessionData.WorkData.OnDuty)
                    frameList.List.Add(new ListData(LangFunc.GetText(LangType.De, DataName.UnwearForm), "takeoff"));
                    ...
                    switch (type)
                    {
                        case "takeoff":
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                LangFunc.GetText(LangType.De, DataName.EndWorkDay), 6000);
                        ...
                        default:
                                ...
                                if (clothingList.Count > 0)
                                {
                                    ...
                                    if (setReadyCount < 3)
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                            LangFunc.GetText(LangType.De, DataName.FormNotReady),
                                            3000);
                                        return;
                                    }
                                    // Чек на готовность сета, временно так...
                            ...
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantUseThisForm), 6000);
                            return;
                    }
                }
            ...
            try
            {
                    ...
                    FractionSets[(Models.Fractions)memberFractionData.Id].ContainsKey(gender))
                {
                    ...
                    if (clothingList.Count > 0)
                    {
                        ...
                        if (isDutySet && !onDuty)
                        {
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.StartWorkDay), 6000);
