    ...
    public class Repository
    {
        ...
        public static void UpdateRankScore(ExtPlayer player, int id, int score)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Mindestwert {NeptuneEvo.Table.Repository.MinScore} Maximalwert {NeptuneEvo.Table.Repository.MaxScore}", 4500);
                    return;
                }
        ...
        public static void DeletePlayer(ExtPlayer player, int uuid)
        {
            try
            {
                ...
                else
                {
                    ...
                    Logs.Repository.AddLogs(player, FractionLogsType.UnInvite, $"Entlässt {targetMemberFractionData.Name} ({targetMemberFractionData.UUID})");
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TiUvolil, targetMemberFractionData.Name), 6000);
                }
        ...
        public static void DefaultRanks(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Rang Einstellungen zurückgesetzt!", 5000);
                var ranksName = NeptuneEvo.Table.Repository.GetRanksData(fractionData.Ranks);
                Trigger.ClientEvent(player, "client.frac.main.setRanks", JsonConvert.SerializeObject(ranksName));
            }
            catch (Exception e)
