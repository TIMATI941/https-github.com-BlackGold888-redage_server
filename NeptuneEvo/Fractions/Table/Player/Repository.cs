    ...
    public class Repository
    {
        ...
        public static void Reprimand(ExtPlayer player, int uuid, string name, string text)
        {
            try
            {
                ...
                Logs.Repository.AddLogs(player, FractionLogsType.Reprimand, LangFunc.GetText(LangType.De, DataName.GivenVigovor, name, uuid, text));
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
        ...
        public static void Leave(ExtPlayer player)
        {
            try
            {
                ...
                Manager.sendFractionMessage(memberFractionData.Id, "!{#FF8C00}[F] " + $"{player.Name} ({player.Value}) kündigte auf seinen eigenenen Wunsch", true);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouLeaveFraction), 6000);
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
