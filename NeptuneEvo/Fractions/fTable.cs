    ...
    class fTable : Script
    {
        ...
        public void ClothingSetRank(ExtPlayer player, bool isUp, string name, bool gender)
        {
            ...
            {				
                    ...
                    FractionClothingSets.FractionSets[(Models.Fractions) memberFractionData.Id].ContainsKey(gender))
                {
                    ...
                    if (actionResult == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCantEditRankFormToThis), 6000);
                        return;
                    }
                    if (actionResult == 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindForm), 6000);
                        return;
                    };
                
                
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucEditFormRank), 6000);
                }
            }
            catch (Exception e)
            {
        ...
        public void StartEditClothingSet(ExtPlayer player, string oldName, string newName, bool gender)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCantEditForm), 6000);
                    return;
                }
                ...
                if (FractionClothingSets.FractionMainCloakrooms.ContainsKey(fracid) && player.Position.DistanceTo(FractionClothingSets.FractionMainCloakrooms[fracid]) < 10 || FractionClothingSets.FractionSecondCloakrooms.ContainsKey(fracid) && player.Position.DistanceTo(FractionClothingSets.FractionSecondCloakrooms[fracid]) < 10)
                {
                        ...
                        FractionClothingSets.FractionSets[(Models.Fractions) memberFractionData.Id].ContainsKey(gender))
                    {
                        if (!oldName.Equals(newName))
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, gender ? LangFunc.GetText(LangType.De, DataName.ThisNameMaleFormUsed) : LangFunc.GetText(LangType.De, DataName.ThisNameFemaleFormUsed), 6000);
                                return;
                            }
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucEditFormName), 6000);
        ...
        public void EditClothingSet(ExtPlayer player, string name, int id, int texture)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FormEditError), 6000);
                    return;
                }
                ...
                if (FractionClothingSets.FractionMainCloakrooms.ContainsKey(fraction) && player.Position.DistanceTo(FractionClothingSets.FractionMainCloakrooms[fraction]) < 10 || FractionClothingSets.FractionSecondCloakrooms.ContainsKey(fraction) && player.Position.DistanceTo(FractionClothingSets.FractionSecondCloakrooms[fraction]) < 10)
                {
                        ...
                        FractionClothingSets.FractionSets[(Models.Fractions) memberFractionData.Id].ContainsKey(gender))
                    {
                        ...
                        if (id == -1 && texture == 0)
                        {
                            ...
                            foreach (var item in toDeleteClothingList)
                            {
                                ...
                                if (aSet.Component == dictionary)
                                {
                                    ...
                                    //player.ClearAccessories();
                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucDeleteFormComponent), 6000);
                                    return;
                                }
                            }
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FormError), 6000);
                            return;
                        }
                        ...
                        if (availableSet.value != null)
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucEditForm), 6000);  
                            FractionClothingSets.SetPlayerFactionClothingSet(player, memberFractionData.Id, setName, gender, isDutySet: false); 
                            //player.ClearAccessories();
                            
                            return;
                        }
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantEditThisForm), 6000);
                        return;
                    }
        ...
        public static void reprimand(ExtPlayer player, int uuid, string name, string text)
        {
            try
            {
                ...
                if (text.Length > 100)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MaxVigovorLength), 4500);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindPlayerFraction, name), 6000);
                    return;
                }
                else if (targetMemberFractionData.Rank >= memberFractionData.Rank)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCantVigovor, name), 6000);
                    return;
                };
                addLogs(player, FractionLogsType.Reprimand, LangFunc.GetText(LangType.De, DataName.GivenVigovor, name, uuid, text));
            }
            catch (Exception e)
            {
