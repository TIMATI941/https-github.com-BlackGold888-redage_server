    ...
    class CarDelivery : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                foreach (KeyValuePair<int, Vector3> pos in MafiaStartDelivery)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel("~w~Mission: Lieferung", pos.Value, 5f, 0.4f, 0, new Color(255, 255, 255));
                }
                #endregion
        ...
        public static void InPoliceDropDelivery(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Du hast das Fahrzeug beschlagnahmt", 3000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void InGangEndDelivery(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Versuchen Sie es später erneut", 3000);
                        return;
                    }
                    VehicleStreaming.DeleteVehicle(vehicleDelivery);
                    ...
                    Trigger.SendChatMessage(player, "Fahrzeug abgabe: !{#00FF00}" + halfof + "$ ~w~wurden in die Gang Kasse überwiesen und der gleiche Betrag blieb in Ihrer Tasche. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()}" + ")");
        ...
        public static void InMafiaEndDelivery(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    fractionData.Money += halfof;
                    GameLog.Money($"server", $"frac({fractionData.Id})", halfof, "dropCar");
                    Trigger.SendChatMessage(player, "Fahrzeug abgabe: !{#00FF00}" + halfof + "$ ~w~wurden an den Gemeinschaftsfonds der Gruppe überwiesen und der gleiche Betrag blieb in Ihrer Tasche. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()})");
                    player.AddTableScore(TableTaskId.Item27);
                }
            }
            catch (Exception e)
                    ...
                    switch (vehicleLocalData.Access)
                    {
                        case VehicleAccess.DeliveryGang:
                            if (Configs.IsFractionPolic(fracId))
                            {
                                ...
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Fahre das Fahrzeug zum Abschlepphof", 3000);
                            }
                            ...
                            else
                            {
                                ...
                                Trigger.SendChatMessage(player, "Wenn du die alte Markierung verloren hast, dann schreibe /mypoint in den Chat");
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Fahre das Fahrzeug zum angegebenen Ziel", 3000);
                            }
                            return;
            ...
            try
            {
                ...
                switch (id)
                {
                    ...
                    case 1:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Versuchen Sie es später erneut", 3000);
                            return;
                        }
                        if (sessionData.DeliveryData.Vehicle != null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie müssen zuerst das vorherige Auto liefern", 3000);
                            return;
                        }
                        ...
                        Trigger.SendChatMessage(player, "Wenn du die alte Markierung verloren hast, dann schreibe /mypoint in den Chat");
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Du hast die Koordinaten für das Fahrzeug bekommen!", 3000);
                        player.AddTableScore(TableTaskId.Item27);
                        return;
                    default:
                        // Not supposed to end up here. 
        ...
        public static void Event_mafiaMission(ExtPlayer player, int id)
        {
            try
            {
                ...
                NextDelivery[fracId] = DateTime.Now.AddMinutes(5);
                Trigger.SendChatMessage(player, "Wenn du die alte Markierung verloren hast, dann schreibe /mypoint in den Chat");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Du bekommst einen Lieferwagen {text} für den Transport. Fahre zum gennanten Ort", 3000);
                player.AddTableScore(TableTaskId.Item27);
                Trigger.ClientEvent(player, "createWaypoint", MafiaEndDelivery[end].X, MafiaEndDelivery[end].Y);
