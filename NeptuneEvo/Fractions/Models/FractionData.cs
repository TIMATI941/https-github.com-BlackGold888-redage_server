...
namespace NeptuneEvo.Fractions.Models
{
    public class FractionData : TableData
    {
        ...
        public void UpdateLabel()
        {
            ...
            if (Drugs == 0 && Materials == 0 && MedKits == 0) text += "Leeres Lager";
            else
            {
                if (Drugs > 0) text += $"Drogen: ~r~{Drugs}/10000\n";
                if (Materials > 0) text += $"Mats: ~r~{Materials}/{MaxMats}\n";
                if (MedKits > 0) text += $"Verbandskasten: ~r~{MedKits}";
            }

            if (HaveSpecialStock == true) text += $"\n\nrohes Erz:\n~w~Kohle ~r~{CoalOre} ~w~\nEisenerz ~r~{IronOre} ~w~\nSchwefelerz ~r~{SulfurOre} ~w~\nEdelsteine ~r~{PreciousOre} ~w~";
