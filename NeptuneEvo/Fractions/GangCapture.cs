    ...
    class GangsCapture : Script
    {
        ...
        public static void CMD_startCapture(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Ihnen wurde der Zugang zum capture und bizwar verwehrt. Grund: {characterData.BanCrimeReason}.", 5000);
                    return;
                }
                ...
                if (shapeData == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouAreNotInRegion), 6000);
                    return;
                }
                ...
                switch (shapeData.Index)
                {
                    ...
                    case 57:
                    case 89:
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DontClaimTitulRegion), 6000);
                        return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCantClaimYourRegion), 6000);
                    return;
                }
                
                if (Main.ServerNumber != 0 && (DateTime.Now.Hour < 13 || DateTime.Now.Hour >= 23))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TimeYouCanClaimRegion), 6000);
                    return;
                }
                ...
                if (Main.ServerNumber != 0 && DateTime.Now < nextCaptDate[fracId])
                {
                    ...
                    if (g.Minute >= 5)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCanClaim1h, g.Hour, g.Minute, g.Second), 6000);
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCanClaim,  g.Minute, g.Second), 6000);
                ...
                if (Main.ServerNumber != 0 && DateTime.Now < protectDate[region.GangOwner])
                {
                    ...
                    if (g.Minute >= 5)
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCanClaimGangRegion1h, g.Hour, g.Minute, g.Second), 6000);
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouCanClaimGangRegion,  g.Minute, g.Second), 6000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DontHaveOnlineEnemyGang), 6000);
                    return;
                }
                
                if (World.War.Repository.Wars.Values.Any(w => w.Type == WarType.Gangs))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CaptIsGoingSomeGang), 6000);
                    return;
                }
                ...
                Trigger.SendToAdmins(1, $"{ChatColors.Red}[A] {player.Name} ({player.Value}) von {Manager.GetName(warData.AttackingId)} hat einen Kampf um Gebiet {Manager.GetName(warData.ProtectingId)} provoziert.");
                GameLog.FracLog(fracId, characterData.UUID, -1, player.Name, "-1", $"capture({region.ID}, {Manager.FractionNames[region.GangOwner]})");

                Manager.SendCoolFractionMsg(warData.ProtectingId, "Eroberung", "Sie werden angegriffen!", LangFunc.GetText(LangType.De, DataName.CaptureAlertDefend, Manager.GetName(warData.AttackingId)), "", 10000);
                Manager.SendCoolFractionMsg(warData.AttackingId, "Eroberung", "Angriff", LangFunc.GetText(LangType.De, DataName.CaptureAlertAttack), "", 10000);
                //Manager.sendFractionMessage(warData.ProtectingId, LangFunc.GetText(LangType.De, DataName.CaptureAlertDefend, Manager.GetName(warData.AttackingId)));
                //Manager.sendFractionMessage(warData.AttackingId, LangFunc.GetText(LangType.De, DataName.CaptureAlertAttack));
        ...
        public static void CMD_tpcapture(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData.IsBannedCrime == true)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Ihnen wurde der Zugang zu capture und bizwar verwehrt. Grund: {characterData.BanCrimeReason}.", 5000);
                    return;
                }
                ...
                if (war != null && (fracId == war.ProtectingId || fracId == war.AttackingId))
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyTpToClaimRegion), 6000);
                        return;
                    }
                    if (sessionData.UsedTPOnCaptureOrBizwar == 2)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouWasInClaimRegion), 6000);
                        return;
                    }
                    if (sessionData.WarData.IsWarZone)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouAlreadyInClaimRegion), 6000);
                        return;
                    }
                    ...
                    if (fracId != war.ProtectingId)
                    {
                        ...
                        EventSys.SendCoolMsg(player,"Eroberung", "Teleport", LangFunc.GetText(LangType.De, DataName.YouWasTpCenterClaimRegion), "", 5000);   
                        //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouWasTpCenterClaimRegion), 6000);
                    }
                    else
                    {
                        ...
                        if (nearestCaptureCoord != null)
                        {
                            ...
                            sessionData.UsedTPOnCaptureOrBizwar = 1;
                            EventSys.SendCoolMsg(player,"Eroberung", "Teleport", LangFunc.GetText(LangType.De, DataName.YouWasTpNearClaimRegion), "", 5000);   
                            //Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouWasTpNearClaimRegion), 6000);
                        }
                        else
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindNearClaimRegion), 6000);
