...
namespace NeptuneEvo.PedSystem.Pet
{
    class Repository
    {
        ...
        public static void LoadInGame()
        {
            ...
            PedSystem.Repository.CreateQuest("a_m_y_soucent_03", new Vector3(268.109, -641.3529, 42.01984), 60.41674f, title: "~y~NPC~w~ Vitaly\nHaustierverkäufer", colShapeEnums: ColShapeEnums.PetShop);
            Main.CreateBlip(new Main.BlipData(141, LangFunc.GetText(LangType.De, DataName.PetSeller), new Vector3(268.109, -641.3529, 42.01984), 9, true));
        ...
        public static void Damage(ExtPed ped, int amount)
        {
            ...
            if (petData.Health <= 0)
            {
                ...
                //Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PetDying), 10000);
                Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Ems,LangFunc.GetText(LangType.De, DataName.PetDying), DateTime.Now);
            }
            else if (player != null)
        ...
        public static void Create(ExtPlayer player, int index)
        {
            ...
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    if (isPet)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyHavePet), 5000);
                        return;
                    }
                    ...
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                            ...
                            {
                                UpdateData.RedBucks(player, -pet.Price, msg: LangFunc.GetText(LangType.De, DataName.BoughtPet));
                            }
        ...
        public static void RespawnPet(ExtPlayer player)
        {
            ...
            if (createPets > 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PetIsOk), 5000);
                return;
            }
            ...
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    if (pet == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHaveNoPet), 5000);
                        return;
                    }
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        //Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouPetIsOk, price), 10000);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Ems, LangFunc.GetText(LangType.De, DataName.YouPetIsOk, price), DateTime.Now); 
        ...
        public static void AddBall(ExtPlayer player)
        {
            ...
            if (characterData == null)
                return;
            Commands.RPChat("do", player, $"Dein Haustier bringt dir den Ball");
        ...
        public static void OnSniff(ExtPlayer player, ExtPlayer target)
        {
                ...
                foreach (string Location in Chars.Repository.ItemsData[locationName].Keys)
                {
                    ...
                    foreach (var itemData in Chars.Repository.ItemsData[locationName][Location])
                    {
                        if (Location == "inventory" || Location == "fastSlots")
                        {
                            ...
                            {
                                Commands.RPChat("do", player, LangFunc.GetText(LangType.De, DataName.PetSniffed));
                                return;
                            }
                        }
                    }
            ...
            Commands.RPChat("do", player, LangFunc.GetText(LangType.De, DataName.PetSniffs, target.Name));
        }
        ...
        /*public static void OpenPetMenu(Player player)
        {
            try
            {
                ...
                menuItem.Text = LangFunc.GetText(LangType.De, DataName.back);
                menu.Add(menuItem);
        ...
        public static void OpenPetMenu(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.De, DataName.Name), LangFunc.GetText(LangType.De, DataName.InputPetName), 36, "make_ped_name");
        ...
        public static void UpdateName(ExtPlayer player, ExtPed ped, string name)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PetNameChanged, name), 5000);
            }
            catch (Exception e)
            {
        ...
        public static void Sell(ExtPlayer player)
        {
            try
            {
                ...
                if (pet.isDonate)
                    Trigger.ClientEvent(player, "openDialog", "sell_pet", LangFunc.GetText(LangType.De, DataName.PetSellRb, Wallet.Format(pet.Price / 2)));
                else
                    Trigger.ClientEvent(player, "openDialog", "sell_pet", LangFunc.GetText(LangType.De, DataName.PetSellMoney, Wallet.Format(pet.Price / 2)));
        ...
        public static void ConfirmSell(ExtPlayer player, ExtPed ped)
        {
            try
            {
                ...
                if (!pet.isDonate)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PetGivenRb, Convert.ToInt32(pet.Price / 2)), 5000);
                }
                else
                {
                    UpdateData.RedBucks(player, Convert.ToInt32(pet.Price / 2), msg: LangFunc.GetText(LangType.De, DataName.PetSell));
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PetGivenMoney, Convert.ToInt32(pet.Price / 2)), 5000);
                }
        ...
        public static void SetEat(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PetTooFar), 5000);
                    return;
                }
                else if (petData.Health >= 100)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PetNoHungry), 5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHaveNoFood), 5000);
                    return;
                }
                ...
                Timers.StartOnce(5000, () =>
                {
                    try
                    {
                        ...
                        Damage(ped, eatData);
                        Commands.RPChat("sme", player, $"füttert" + (characterData.Gender ? "" : "e") + LangFunc.GetText(LangType.De, DataName.Pets));
                    }
                    catch (Exception e)
                    {
