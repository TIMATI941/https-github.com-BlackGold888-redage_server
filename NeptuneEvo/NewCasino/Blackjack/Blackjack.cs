    ...
    class Blackjack : Script
    {
        ...
        public static void BlackjackOpen(ExtPlayer player, int Index, int SlotId, float PosX, float PosY, float PosZ, float Rotation)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Table500Bucks), 6000);
                    return;
                }
                if (DealerData[Index].Chairs.Contains(SlotId))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlaceOwned), 6000);
                    return;
                }
        ...
        public static void SetBet(ExtPlayer player, int money)
        {
            ...
            try
            {
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BetSuc), 1000);
