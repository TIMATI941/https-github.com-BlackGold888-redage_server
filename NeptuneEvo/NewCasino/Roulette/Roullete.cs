    ...
    class Roullete : Script
    {
        ...
        public static void ContinueGame(int index)
        {
            try
            {
                ...
                { // В случае, если все гуд (и игроки сидят, и ставки стоят)
                    ...
                    SendRouletteNotify(index, LangFunc.GetText(LangType.De, DataName.CantDoStavka));
        ...
        public static void OnCharacterOccupyPlace(ExtPlayer player, int tableIndex, int placeID)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.PlaceOwned), 6000);
                    return;
                }
        ...
        public static void OnCharacterLeavePlace(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantBetByGame), 5000);
                    return;
                }

                if (rData.AllBets.Count > 0)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NowDeleteStavkas), 6000);
                    return;
                }
        ...
        public static void OnCreateBet(ExtPlayer player, int bet, int spot, float x, float y, float z)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OneGameMaxBets, MaxBetsLimit), 6000);
                    return;
                }
                if (RouletteTables.Count <= rData.SelectedTable) return;
                Table table = RouletteTables[rData.SelectedTable];
                ...
                if (table.Process)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WaitUntilGameEnds), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouBet, bet), 6000);
                    ...
                    foreach (ExtPlayer foreachPlayer in table.Seats)
                    {
                        ...
                        if (foreachCharacterData != null && PlayerData.ContainsKey(foreachPlayer))
                        {
                            ...
                            if (targetrData.AllBets.Count > 0)
                            {
                                ...
                                string message = $"Ergeben [{table.WinNum}]. Sie {(targetrData.WinMoney > 0 ? $"Gewinnen ${MoneySystem.Wallet.Format(targetrData.WinMoney)}" : "Verlieren")}"; // harddd
