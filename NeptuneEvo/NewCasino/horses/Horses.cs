    ...
    class Horses : Script
    {
                        ...
                        lock (PlayerData)
                        {
                            foreach (ExtPlayer foreachPlayer in PlayerData.Keys)
                            {
                                ...
                                if (rData.Spot == WinHorse)
                                {
                                    ...
                                    Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouWinHorse, MoneySystem.Wallet.Format(payment), rData.Spot), 7000);
                                    bets.Add(payment);
                                }
                                else
                                {
                                    Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouLoseHorse, rData.Spot), 5000);
                                    bets.Add(-rData.Bet);
                                }
                                gametags.Add(LangFunc.GetText(LangType.De, DataName.Horse, foreachSessionData.Name, rData.Spot));
                            }
                        }
        ...
        private static void sit_slot(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HorseBetAlready, PlayerData[player].Spot), 6000);
                    return;
                }
            }
            catch (Exception e)
        ...
        public static void CMD_SelectHorse(ExtPlayer player, int horse, int money)
        {
            try
            {
                ...
                if (curentScreen != 0)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantDoStavka), 6000);
                    return;
                }
                if (!HorsesWorking)
                {
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.BetOnHorse, horse), 6000);
                tears_slot(player);
            }
            catch (Exception e)
            {
