    ...
    class Donations : Script
    {
        ...
        private static void CheckPromoDonate(string login, ulong reds, ExtPlayer player)
        {
            try
            {
                ...
                if (reds >= 1)
                {
                    ...
                    {
                        GameLog.AccountLog(dlogin, "-", "-", "-", LangFunc.GetText(LangType.De, DataName.RefTopUp, login, reds));
                        using MySqlCommand cmd = new MySqlCommand
                        {
                    ...
                    {
                        UpdateData.RedBucks(target, (int)reds, msg: "Referal Gutschrift");
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.ComeRbFromRef, reds, login), DateTime.Now);
                        //Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ComeRbFromRef, reds, login), 6000);
                        using MySqlCommand cmd = new MySqlCommand
                        {
                    ...
                    foreach (DataRow Row in result.Rows)
                    {
                        ...
                        {                       
                            ...
                            if (!DonatePack.IsDonate(player, name.ToLower(), (int)reds))
                            {
                                ...
                                if (accountData == null)
                                {
                                    GameLog.AccountLog(name, "-", "-", "-", $"Aufladung (+{reds} RedBucks)");
                                    using MySqlCommand cmd = new MySqlCommand
                                    {
                                ...
                                {
                                    UpdateData.RedBucks(player, (int)reds, msg: "Aufladung");
                                    ...
                                    Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.UvGotReds, reds), DateTime.Now);
                                }
                            }
        ...
        public void GiftDonate(ExtPlayer player, string login, int sum)
        {
            try
            {
                ...
                if (sum < 25 || sum > 9999)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RbTransferLimit), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Cooldown5min), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RecoveryCantFind), 6000);
                    return;
                }
                int targetrb = Convert.ToInt32(result.Rows[0]["redbucks"]);
                UpdateData.RedBucks(player, -sum, msg: $"Geschenk {login}");
                ...
                if (targetAccountData == null)
                {
                    UpdateData.RedBucks(target, sum, msg: $"Geschenk von {accountData.Login}");
                    ...
                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"Sie bekommen ein Geschenk: {sum} Rockford-Coins vom {player.Name.Replace('_', ' ')} ({player.Value})!", 5000);
                }
                else
                {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucGiftRb, sum, login), 5000);
            }
            catch (Exception e)
            {
                    ...
                    case Type.Slot:
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RecoveryError), 6000);
                                return;
                            }
                            slotId -= 1;
