    ...
    class Bank : Script
    {
        ...
        public static async void Init()
        {
            try
            {
                ...
                var banks = await db.Money.ToListAsync();
        ...
        public static bool Transfer(int firstAccID, int lastAccID, long amount)
        {
            try
            {
                ...
                {
                    if (firstAcc.Type == 1)
                        BankNotify((ExtPlayer) NAPI.Player.GetPlayerFromName(firstAcc.Holder), BankNotifyType.InputError, "Dieses Bankkonto existiert nicht!");
                ...
                {
                    if (firstAcc.Type == 1)
                        BankNotify((ExtPlayer) NAPI.Player.GetPlayerFromName(firstAcc.Holder), BankNotifyType.PayError, "Nicht genug Geld!");
            ...
            try
            {
                switch (type)
                {
                    case BankNotifyType.InputError:
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ErrorInput), 6000);
                        return;
                    case BankNotifyType.PayError:
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ErrorWithdraw), 6000);
                        return;
                    case BankNotifyType.PayIn:
                        int money = Convert.ToInt32(info);
                        //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MoneyIncome, Wallet.Format(money)), 6000);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.MoneyIncome, Wallet.Format(money)), DateTime.Now);
                        if (money >= 10)
                            BattlePass.Repository.UpdateReward(player, 5);
                        return;
                    case BankNotifyType.PayOut:
                        money = Convert.ToInt32(info);
                        //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MoneyOutcome, Wallet.Format(money)), 6000);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.MoneyOutcome, Wallet.Format(money)), DateTime.Now);
                        return;
                    default:
    ...
    class ATM : Script
    {
        ...
        public static void OnAtm(ExtPlayer player, int _)
        {
            try
            {
                ...
                // {
                //     Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.WantedNOATM), 6000);
                //     return;
                // }
                // if (characterData.Bank == 0 || !Bank.Accounts.ContainsKey(characterData.Bank))
                // {
                //     Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoBanks), 6000);
                //     return;
                // }
        ...
        [RemoteEvent("atmOpenBlackGold")]
        public static void atmOpenBlackGold(ExtPlayer player)
        {
            try
            {
                var characterData = player.GetCharacterData();
        ...
        public static void ClientEvent_ATMVAL(ExtPlayer player, params object[] args)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ServerCant), 6000);
                    return;
                }
                ...
                switch (type)
                {
                    ...
                    case 2:
                        ...
                        if (myamount <= 0)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CanNotTransact), 6000);
                            return;
                        }
                        ...
                        //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucTransact), 6000);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.SucTransactH, myamount, (bankBalance+myamount), maxMoney), DateTime.Now);

                        switch (accountData.VipLvl)
                        {
                            case 0:
                            case 1:
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(house.Price / 100 * 0.026 * 24 * 7)}$','Bareinzahlungsbetrag']");
                                break;
                            case 2:
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(house.Price / 100 * 0.026 * 24 * 14)}$','Bareinzahlungsbetrag']");
                                break;
                            case 3:
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(house.Price / 100 * 0.026 * 24 * 21)}$','Bareinzahlungsbetrag']");
                                break;
                            case 4: // Platinum
                            case 5: // Media Platinum
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(house.Price / 100 * 0.026 * 24 * 28)}$','Bareinzahlungsbetrag']");
                                break;
                            default:
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(house.Price / 100 * 0.026 * 24 * 7)}$','Bareinzahlungsbetrag']");
                                break;
                    ...
                    case 3:
                        ...
                        //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.SucTransact), 6000);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.De, DataName.SucTransactB, myamount, (bankBalance+myamount), maxMoney), DateTime.Now);
                        switch (accountData.VipLvl)
                        {
                            case 0:
                            case 1:
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 7)}$','Kreditbetrag']");
                                break;
                            case 2:
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 14)}$','Kreditbetrag']");
                                break;
                            case 3:
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 21)}$','Kreditbetrag']");
                                break;
                            case 4: // Platinum
                            case 5: // Media Platinum
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 28)}$','Kreditbetrag']");
                                break;
                            default:
                                Trigger.ClientEvent(player, "atmOpen", $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 7)}$','Kreditbetrag']");
                                break;
                    ...
                    case 4:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantFindBankAccount), 6000);
                            Trigger.ClientEvent(player, "closeatm");
                            Trigger.StopAnimation(player);
                            return;
                        }
                        sessionData.ATMData.Amount = amount; // ATM2ACC
                        Trigger.ClientEvent(player, "atmOpen", "[2,0,'Überweisung']");
                    ...
                    case 44:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NextTransactionSoon), 6000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TransactionCancelled), 6000);
                            Trigger.ClientEvent(player, "closeatm");
                            Trigger.StopAnimation(player);
                            return;
                        }*/
                        ...
                        Trigger.ClientEvent(player, "closeatm");
                        Trigger.StopAnimation(player);
                        Trigger.ClientEvent(player, "openDialog", "AcceptBankTransfer", LangFunc.GetText(LangType.De, DataName.TransactionConfirm1, myamount) + (Bank.Accounts[bank].Holder.Length > 3 ? LangFunc.GetText(LangType.De, DataName.TransactionConfirm2, Bank.Accounts[bank].Holder) : "на неизвестный счет?"));
            ...
            try
            {
                ...
                switch (type)
                {
                    case 1:
                        ...
                        {
                            case 0:
                                Trigger.ClientEvent(player, "atmOpen", "[2,' ','Bareinzahlungsbetrag']");
                                sessionData.ATMData.Type = index;
                                break;
                            case 1:
                                Trigger.ClientEvent(player, "atmOpen", "[2,' ','Abheben']");
                                sessionData.ATMData.Type = index;
                                break;
                            case 2:
                                ...
                                Trigger.ClientEvent(player, "setatm", LangFunc.GetText(LangType.De, DataName.House), $"Eigentum #{house.ID}", bankBalance, "");
                                sessionData.ATMData.Type = index;
                                break;
                            case 3:
                                AtmBizGen(player);
                                ...
                                break;
                            case 4:
                                Trigger.ClientEvent(player, "atmOpen", "[2,0,'Einzahlen']");
                                sessionData.ATMData.Type = index;
                                break;
                            default:
                                // Not supposed to end up here. 
                        ...
                        switch (accountData.VipLvl)
                        {
                            ...
                            case 1:
                                Trigger.ClientEvent(player, "atmOpen",
                                    $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 7)}$','Kreditbetrag']");
                                break;
                            case 2:
                                Trigger.ClientEvent(player, "atmOpen",
                                    $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 14)}$','Kreditbetrag']");
                                break;
                            case 3:
                                Trigger.ClientEvent(player, "atmOpen",
                                    $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 21)}$','Kreditbetrag']");
                                break;
                            case 4: // Platinum
                            case 5: // Media Platinum
                                Trigger.ClientEvent(player, "atmOpen",
                                    $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 28)}$','Kreditbetrag']");
                                break;
                            default:
                                Trigger.ClientEvent(player, "atmOpen",
                                    $"[2,'{bankBalance}/{Convert.ToInt32(biz.SellPrice / 100 * biz.Tax * 24 * 7)}$','Kreditbetrag']");
