    ...
    class Lottery : Script
    {
        ...
        public static void CMD_CheckLottery(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LotteryAnnounce, LotteryBought.Count(), Wallet.Format(Price)), 6000);
                    return;
                }
                else
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ZeroTicketsLottery), 7000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LotteryMy, mytotal, shans.ToString("0.##"), Wallet.Format(Price)), 7000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.JackPotLottery), 7000);
                    if (shans >= 70.0) Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Chance70Lottery), 7000);
                    else if (shans >= 50.0) Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Chance50Lottery), 7000);
                }
            }
            catch (Exception e)
            {
