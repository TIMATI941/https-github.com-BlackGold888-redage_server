    ...
    class Drone : Script
    {
        ...
        public static void CreateDroneFraction(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Only2DronesAvaible, FractionDroneData[fracId].Item2), 5000);
                    return;
                }
                ...
                if (vehicleLocalData.Access == VehicleAccess.Fraction && vehicleLocalData.Fraction == fracId)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DroneControls), 10000);
                }
            }
            catch (Exception e)
            {
        ...
        public void Event_OnPlayerExitVehicle(ExtPlayer player, ExtVehicle vehicleDrone)
        {
            try
            {
                ...
                {
                    VehicleStreaming.DeleteVehicle(vehicleDrone);
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.DroneDeleted), 6000);
                }
            }
            catch (Exception e)
            {
