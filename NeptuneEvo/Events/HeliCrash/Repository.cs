    ...
    public class Repository
    {
        ...
        public static void Create()
        {
            try
            {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    Trigger.SendChatMessage(foreachPlayer, "~o~[HeliCrash] In 10 min fliegt ein Hubstrauber über den " + $"{(foreachSessionData.IsRadioInterceptor ? _heliCords.Name : "Staat")} {HeliDisplayName[_heliType]} und sendet ein Notsignal");
                    EventSys.SendCoolMsg(foreachPlayer,"HeliCrash", $"In 10 min!", $"fliegt ein Hubstrauber über den {(foreachSessionData.IsRadioInterceptor ? _heliCords.Name : "Staat")} {HeliDisplayName[_heliType]} und sendet ein Notsignal.", "", 10000);
                }
                ...
                if (heliBox.Health == 0)
                {
                    ...
                    if (hack_chance <= 50)
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.De, DataName.SucZamok), 6000);
                    ...
                    else
                    {
                        ...
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter,
                            LangFunc.GetText(LangType.De, DataName.FailZamok), 6000);
                    }
        ...
        private static (int, int) GetHealth(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Jemand bricht es gerade auf", 3000);
                    return (0, 0);
                }
