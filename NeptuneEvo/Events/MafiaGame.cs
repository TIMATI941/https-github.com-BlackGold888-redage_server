    ...
    class MafiaGame : Script
    {
        ...
        public static void CheckMafiaLobby(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.lvl3forgame), 6000);
                    return;
                }*/
            ...
            try
            {
                ...
                if (lobbyId >= 0)
                {
                    if (LobbyList.ContainsKey(lobbyId) && LobbyList[lobbyId].GameStatus <= 1 && LobbyList[lobbyId].MafiaPlayers.Contains(player))
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LeftLobby), 6000);
        ...
        public static void CreateMafiaLobby(ExtPlayer player, string lobby_name, int lobby_price, string lobby_password, int gameMap)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LobbyAlreadyCreated), 6000);
                    return;
                }
                
                if (characterData.Money < lobby_price)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameStartPrice, lobby_price), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Damit das Spiel startet, benötigen Sie mindestens 6 Teilnehmer in der Lobby. Wenn Sie die Zone verlassen, wird das Spiel abgesagt.", 5000);
        ...
        public static void RemoteEvent_joinMafiaLobby(ExtPlayer player, int index, string password)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LobbyNotFound), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LobbyIncorrectPassowrd), 6000);
                    return;
                }
                
                if (LobbyList[index].MafiaPlayers.Count >= LobbyList[index].MaxPlayers)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MaxPlayersOn), 6000);
                    return;
                }

                if (LobbyList[index].GameStatus == 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoGameStarted), 5000);
                    return;
                }

                if (LobbyList[index].GameStatus >= 2)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameIsOn), 5000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameStartCost, LobbyList[index].LobbyPrice), 5000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameReg), 8000);
        ...
        public static void RemoteEvent_voteMafiaGame(ExtPlayer player, int target_id)
        {
            try
            {
                ...
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.CantVoteYourself), 6000);
                        return;
                    }

                    if (player.Value == target_id && player.GetSharedData<int>("mafiaGameRole") == 3 && LobbyList[lobbyId].doctorHealedYourself)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyHealYourself), 6000);
                        return;
                    }

                    if (!LobbyList[lobbyId].MafiaGamePlayersInfo.ContainsKey(target_id))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoPlayerGame), 6000);
                        return;
                    }
                    ...
                    if (LobbyList[lobbyId].mafiaGameVoteStatus == 4 && player.GetSharedData<int>("mafiaGameRole") == 1)
                    {
                        ...
                        SendTextInfoForPlayer(player, LangFunc.GetText(LangType.De, DataName.VoteOk));
                        ...
                        foreach (ExtPlayer MafiaPlayer in LobbyList[lobbyId].MafiaPlayers)
                        {
                            try
                            {
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    ...
                                    {
                                        if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 1) SendTextInfoForPlayer(player, LangFunc.GetText(LangType.De, DataName.PlayerIsMafia), 10000);
                                        else SendTextInfoForPlayer(player, LangFunc.GetText(LangType.De, DataName.PlayerNotMafia), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                    ...
                    else if (LobbyList[lobbyId].mafiaGameVoteStatus == 7 && player.GetSharedData<int>("mafiaGameRole") == 3)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouHealedPlayerNight), 6000);
                            return;
                        }
                    ...
                    if (LobbyList[index].MafiaPlayers.Count < 6) // change to 6
                    {
                        foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                        {
                            try
                            {
                                ...
                                {
                                    Notify.Send(MafiaPlayer, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoPlayersMafia), 6000);
                ...
                else if (LobbyList[index].GameStatus == 2)
                {
                    ...
                    foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                    {
                        try
                        {
                            ...
                            {
                                SendTextInfoForPlayer(MafiaPlayer, $"Deine Rolle:: {role_names[MafiaPlayer.GetSharedData<int>("mafiaGameRole")]}", 10000);
                        ...
                        foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                        {
                            try
                            {
                                if (MafiaPlayer.IsCharacterData() && LobbyList[index].MafiaGamePlayersInfo.ContainsKey(MafiaPlayer.Value))
                                {
                                    ...
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 1)
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu", JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true, false, false, LangFunc.GetText(LangType.De, DataName.MafiaKillsMaf), LangFunc.GetText(LangType.De, DataName.YouHave30Sec));
                                }
                            }
                            catch (Exception e)
                            {
                            ...
                            try
                            {
                                if (MafiaPlayer.IsCharacterData() && LobbyList[index].MafiaGamePlayersInfo.ContainsKey(MafiaPlayer.Value))
                                {
                                    ...
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 1)
                                    {
                                        ...
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu", JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true, false, false, LangFunc.GetText(LangType.De, DataName.MafiaKillsMaf), LangFunc.GetText(LangType.De, DataName.YouHave1Min));

                                        SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.YouHave1MinDiscuss), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                        ...
                        try
                        {
                            if (MafiaPlayer.IsCharacterData() && LobbyList[index].MafiaGamePlayersInfo.ContainsKey(MafiaPlayer.Value))
                            {
                                if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 1)
                                {
                                    ...
                                    SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MafiaTakeChoice), 10000);
                                }
                            }
                        }
                        catch (Exception e)
                        ...
                        foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                        {
                            try
                            {
                                if (MafiaPlayer.IsCharacterData() && LobbyList[index].MafiaGamePlayersInfo.ContainsKey(MafiaPlayer.Value))
                                {
                                    ...
                                    {
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu", JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), false, true, false, LangFunc.GetText(LangType.De, DataName.TryToFindMaf), LangFunc.GetText(LangType.De, DataName.YouHave30Sec));
                                        SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.YouCanProverit), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                        ...
                        foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                        {
                            try
                            {
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    ...
                                    {
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu", JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), false, true, false, LangFunc.GetText(LangType.De, DataName.MafYouCanHeal), LangFunc.GetText(LangType.De, DataName.YouHave30Sec));
                                        SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MafTryHeal), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                                ...
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    ...
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 3)
                                    {
                                        if (LobbyList[index].wasHealed == -1)
                                            SendTextInfoForPlayer(MafiaPlayer, "Zeit abgelaufen. Sie haben niemanden geheilt.");
                                    ...
                                    {
                                        Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu", JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), false, true, false, LangFunc.GetText(LangType.De, DataName.MafSexGo), LangFunc.GetText(LangType.De, DataName.YouHave30Sec));
                                        SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MafSexTry), 10000);
                                    }
                                }
                            }
                            catch (Exception e)
                        ...
                        foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                        {
                            try
                            {
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    ...
                                    {
                                        if (LobbyList[index].wasHealed == -1)
                                            SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MafHealTimeOut));
                        ...
                        try
                        {
                            if (MafiaPlayer.IsCharacterData())
                            {
                                if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 4 && LobbyList[index].wasFucked == -1)
                                    SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MafSexTimeout));
                    ...
                    foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                    {
                        try
                        {
                            if (MafiaPlayer.IsCharacterData())
                            {
                                ...
                                {
                                    SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MafDied, LobbyList[index].wasKilled), 10000);

                                    if (MafiaPlayer.Value == LobbyList[index].wasKilled)
                                        playerToKick = MafiaPlayer;
                                    BattlePass.Repository.UpdateReward(MafiaPlayer, 131);
                                }
                                else SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MafNoDied), 10000);
                            }
                        }
                        catch (Exception e)
                        {
                        ...
                        try
                        {
                            if (MafiaPlayer.IsCharacterData())
                            {
                                if (LobbyList[index].mafiaGameDiscussionStatus >= LobbyList[index].MafiaPlayers.Count)
                                {
                                    ...
                                    SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.EveryHave30Sec), 10000);
                                }

                                if (LobbyList[index].mafiaGameDiscussionStatus > 0)
                                {
                                    Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu", JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true, false, true, LangFunc.GetText(LangType.De, DataName.DayMafiaVote), LangFunc.GetText(LangType.De, DataName.VseHave30Sec));
                                    ...
                                    Notify.Send(MafiaPlayer, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OcheredChela, turnPlayerValue), 6000);
                                }
                                else
                                {
                                    Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu", JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true, false, true, LangFunc.GetText(LangType.De, DataName.DayVoteing), LangFunc.GetText(LangType.De, DataName.VseHave1Min));
                                    Trigger.ClientEvent(MafiaPlayer, "mafia_updateMicroStatus", 2);
                                }
                            }
                        }
                    ...
                    foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                    {
                        try
                        {
                            if (MafiaPlayer.IsCharacterData())
                            {
                                ...
                                Trigger.ClientEvent(MafiaPlayer, "setTimeCmd", 12, 0, 0);
                                Trigger.ClientEvent(MafiaPlayer, "showMafiaGameMenu", JsonConvert.SerializeObject(LobbyList[index].MafiaGamePlayersInfo), true, true, true, LangFunc.GetText(LangType.De, DataName.DayPlayerChoice), LangFunc.GetText(LangType.De, DataName.VseHave15Sec));
                                Trigger.ClientEvent(MafiaPlayer, "mafia_updateMicroStatus", 1);
                                SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.VoteJail15), 10000);
                            }
                        }
                        catch (Exception e)
                        {
                            ...
                            foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                            {
                                try
                                {
                                    if (MafiaPlayer.IsCharacterData())
                                    {
                                        ...
                                        if (mostCommon >= 0)
                                        {
                                            SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.SomebodyJail, mostCommon), 10000);
                                            
                                            if (MafiaPlayer.Value == mostCommon)
                                                playerToKick = MafiaPlayer;
                                            BattlePass.Repository.UpdateReward(MafiaPlayer, 131);
                                        }
                                        else SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.VoteEquals), 10000);
                                    }
                                }
                                catch (Exception e)
                                {
                        ...
                        else
                        {
                            foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                            {
                                try
                                {
                                    ...
                                    {
                                        Trigger.ClientEvent(MafiaPlayer, "mafia_closeGameMenu");
                                        SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.VoteEquals), 10000);
                                    }
                                }
                                catch (Exception e)
                                {
                    ...
                    else
                    {
                        foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                        {
                            try
                            {
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.NoJail));
                                    Trigger.ClientEvent(MafiaPlayer, "mafia_closeGameMenu");
                                }
                            }
                            catch (Exception e)
                    ...
                    try
                    {
                        if (MafiaPlayer.IsCharacterData())
                        {
                            ...
                            if (state == 1)
                            {
                                ...
                                SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MafiaWin, current_reward), 10000);
                            ...
                            else if (state == 2)
                            {
                                ...
                                SendTextInfoForPlayer(MafiaPlayer, LangFunc.GetText(LangType.De, DataName.MirWin, current_reward), 10000);
                        ...
                        if (LobbyList[index].MafiaPlayers.Count < 6 && LobbyList[index].GameStatus > 1)
                        {
                            ...
                            foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                            {
                                try
                                {
                                    ...
                                    if (MafiaPlayer.IsCharacterData() && foreachSessionData != null)
                                    {
                                        Notify.Send(MafiaPlayer, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.OstalosMaloPla), 6000);
