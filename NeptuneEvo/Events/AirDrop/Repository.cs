    ...
    public class Repository : Script
    {
        ...
        public static void StartEvent(bool needStart = false, bool needStop = false)
        {
            try
            {
                ...
                if (hour == 8 || hour == 15 || hour == 21 || needStart)
                {
                    ...
                    {
                        Trigger.SendChatMessage(foreachPlayer, "~o~" + LangFunc.GetText(LangType.De, DataName.AirDropZakazan));
                        EventSys.SendCoolMsg(foreachPlayer,"AirDrop", $"Event beginnt bald!", LangFunc.GetText(LangType.De, DataName.AirDropZakazan), "", 10000);
                        ...
                        Trigger.ClientEvent(foreachPlayer, "phone.notify", (int)DefaultNumber.Informer, $"Gringo!!! In 20 Minuten werden Kisten abgeworfen: Anzahl: {dropCount}. Schreibe /airdropum teilzunehmen", 20);
                    }
            ...
            try
            {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    try
                    {
                        ...
                        Trigger.ClientEvent(foreachPlayer, "phone.notify", (int) DefaultNumber.Informer, $"Hey! Laut meiner Infos wurde der AirDrop beendet. Ich hoffe du hattest erfolg! Bis zum nächsten mal", 15);
        ...
        public static void AirdropChangePlayerDimension(ExtPlayer player, int state)
        {
            try
            {
                ...
                if (state == 1)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AirGo), 15000);
                ...
                else if (state == 2)
                {
                    ...
                    if (AirDropEventStatus > 0)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LeaveAirTer), 6000);
                    }
                }
            }
            catch (Exception e)
        ...
        public static void BuyInfo(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AirdropBuyed), 6000);
                    return;
                }
                else if (AirDropEventStatus == 2)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AirdropAlready), 6000);
                    return;
                }
                ...
                if (DateTime.Now.Hour >= 12 && DateTime.Now.Hour <= 23 || DateTime.Now.Hour == 0 && DateTime.Now.Minute <= 10)
                    Trigger.ClientEvent(player, "openDialog", "PAY_AIRDROP_INFO", LangFunc.GetText(LangType.De, DataName.AidropOffer, Main.PricesSettings.AirdropInfoPrice));
                else
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AidropTime), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void Perform(ExtPlayer player)
        {
            try
            {
                ...
                {
                    string nextAirDropTime = "Nächster Airdrop möglich in: " + (AirDropEndEventTime.Hour < 10 ? $"0{AirDropEndEventTime.Hour}" : $"{AirDropEndEventTime.Hour}") + ":" + (AirDropEndEventTime.Minute < 10 ? $"0{AirDropEndEventTime.Minute}" : $"{AirDropEndEventTime.Minute}");
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AirdropWaiting, nextAirDropTime), 6000);
                    return;
                }

                if (DateTime.Now.Hour >= 12 && DateTime.Now.Hour <= 23)
                    Trigger.ClientEvent(player, "openDialog", "PAY_AIRDROP_ORDER", LangFunc.GetText(LangType.De, DataName.AirdropBuyingg, Main.PricesSettings.AirdropOrderPrice));
                else
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AidropTime), 6000);
            }
            catch (Exception e)
            ...
            try
            {
                ...
                if (DateTime.Now.Hour >= 12 && DateTime.Now.Hour <= 23 || DateTime.Now.Hour == 0 && DateTime.Now.Minute <= 10)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AirdropSub), 6000);
                            return;
                        }
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AirdropSubbed), 6000);

                    if (AirDropEventStatus == 1)
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AidropFractions), 10000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AidropTime), 6000);
            }
            catch (Exception e)
            {
                Log.Write($"Perform Exception: {e.ToString()}");
        ...
        public static void PayAirdropOrder(ExtPlayer player)
        {
            try
            {
                ...
                if (DateTime.Now.Hour >= 12 && DateTime.Now.Hour <= 23)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AirdropZakazan), 6000);
        ...
        public static void CMD_airlist(ExtPlayer player)
        {
            try
            {
                ...
                if (AirDropEventStatus == 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AirNeZapushen), 6000);
                    return;
                }
        ...
        public static void CMD_starttestairdrop(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Adminstopaird), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_stopairdrop(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData.AdminLVL < 8) return;
                StopAirDropEvent();
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Adminstopped), 6000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_airdrop(ExtPlayer player)
        {
            try
            {
                ...
                if (!IsPlayerToEvents(player))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoUchastnik), 6000);
                    return;
                }
                ...
                if (airDropsTimeInfo.Count > 0)
                {
                    ...
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.Air1));
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.Air2));
                    Trigger.SendChatMessage(player, LangFunc.GetText(LangType.De, DataName.Air3));

                    Trigger.SendChatMessage(player, "=== AIRDROP TIME ===");
                }
                else Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoInfo), 6000);
            }
            catch (Exception e)
            {
