    ...
    class TankRoyale : Script
    {
        ...
        public static void CreateLobby(ExtPlayer player, string lobby_name, int lobby_price, string lobby_password, int gameMap)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NowWaitOthers), 5000);
        ...
        public static void DeathCheck(ExtPlayer player)
        {
            try
            {
                ...
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId) && LobbyList[lobbyId].TR_Players.ContainsKey(player) && LobbyList[lobbyId].TR_State == 2)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouLeftFromMp), 6000);
                }
            }
            catch (Exception e)
            {
                            ...
                            if (killlist)
                            {
                                lock (LobbyList[lobbyId].TR_Players.Keys)
                                {
                                    foreach (ExtPlayer foreachPlayer in LobbyList[lobbyId].TR_Players.Keys)
                                    {
                                        ...
                                        NAPI.Notification.SendNotificationToPlayer(foreachPlayer, $"~r~{player.Name.Replace('_', ' ')} ~s~hat das Spiel verlassen", true);
                                    }
                                }
                            }
                        }
        ...
        public static void RegisterAccept(ExtPlayer player, int index, string password)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TankRoyaleMaxP), 5000);
                    return;
                }
                
                if (characterData.Money < LobbyList[index].LobbyPrice)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RegisterRoyaleCost, Wallet.Format(LobbyList[index].LobbyPrice)), 6000);
                    return;
                }

                if (LobbyList[index].TR_State != 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.GameIsOn), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TankSucRegister), 5000);
                    ...
                    if (lobby.Key >= 0 && LobbyList.ContainsKey(lobby.Key) && LobbyList[lobby.Key].TR_State == 2 && LobbyList[lobby.Key].Vehicles.Contains(vehicle))
                    {
                        ...
                        if (target.IsCharacterData())
                        {
                            ...
                            if (target.Health <= 7 || PlayerInfo.Value.Health <= 5)
                            {
                                ...
                                if (target != null)
                                    Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouLeftFromMp), 5000);
                                ...
                                lock (LobbyList[lobby.Key].TR_Players.Keys)
                                {
                                        ...
                                        if (LobbyList[lobby.Key].TR_Players.Keys.Count >= 1)
                                        {
                                            ...
                                            foreach (ExtPlayer foreachPlayer in Players)
                                            {
                                                ...
                                                Trigger.SendChatMessage(foreachPlayer,
                                                    "~o~[Tank Royale] Das Event wurde ohne einen Sieger beendet");
                                            }
                                        }
                                    ...
                                    else if (count >= 2)
                                    {
                                        ...
                                        {
                                            if (!foreachPlayer.IsCharacterData()) continue;
                                            Trigger.SendChatMessage(foreachPlayer, $"~o~[Tank Royale] Im Spiel verbleiben {LobbyList[lobby.Key].TR_Players.Count} Panzer!");
                                        }
                                    }
                                }
                            }
            ...
            try
            {
                ...
                if (winnerCharacterData == null)
                {
                    if (LobbyList[index].TR_Players.Keys.Count >= 1)
                    {
                        ...
                        {
                            if (!foreachPlayer.IsCharacterData()) continue;
                            Trigger.SendChatMessage(foreachPlayer, "~o~[Tank Royale] Das Event wurde ohne einen Sieger beendet");
                        }
                    }
                    EndGame(index);
                    return;
                ...
                if (LobbyList[index].TR_Players.Keys.Count >= 1)
                {
                    ...
                    {
                        if (!foreachPlayer.IsCharacterData()) continue;
                        Trigger.SendChatMessage(foreachPlayer, $"~o~[Tank Royale] Gewinner: {winner.Name.Replace('_', ' ')} überlebt mit {LobbyList[index].TR_Players[winner].Health}HP und bekommt {Wallet.Format(LobbyList[index].TR_Bank)}$.");
                    }
                }
                EndGame(index);
            }
                    ...
                    case 1:
                        if(LobbyList[index].TR_TimerTicks == 19) // 9:30
                        {
                            ...
                            foreach (ExtPlayer foreachPlayer in players)
                            {
                                ...
                                if (!SafeZones.IsSafeZone(foreachSessionData.InsideSafeZone, SafeZones.ZoneName.newarena)) Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter, $"Wenn du dich nicht in der SafeZone befindest, kommst du nicht zum Event!", 4500);
                                else if (foreachPlayer.IsInVehicle) Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter, $"Wenn du dich im Fahrzeug befindest, kommst du nicht zum Event!", 4500);
                            }
                        ...
                        else if(LobbyList[index].TR_TimerTicks == 20) // 10
                        {
                            if (LobbyList[index].TR_Players.Count >= 2)
                            {
                                ...
                                foreach (ExtPlayer foreachPlayer in players)
                                {
                                    ...
                                    {
                                        Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MpLeftCuzFar), 6000);
                                        LobbyList[index].TR_Players.Remove(foreachPlayer);
                                        foreachSessionData.InTanksLobby = -1;
                                        continue;
                                    }
                                    ...
                                    {
                                        Notify.Send(foreachPlayer, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MpLeftCuzVeh), 6000);
                                        LobbyList[index].TR_Players.Remove(foreachPlayer);
                                        foreachSessionData.InTanksLobby = -1;
                                        continue;
                                    }
                                    ...
                                    else
                                    {
                                        ...
                                        Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.TargetLastAlive), 6000);
                                        Notify.Send(foreachPlayer, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LoseIfDamage), 5000);
                                    }
                                    ...
                                    lock (LobbyList[index].TR_Players.Keys)
                                    {
                                        foreach (ExtPlayer foreachPlayer in LobbyList[index].TR_Players.Keys)
                                        {
                                            ...
                                            if (!foreachCharacterData.Achievements[27])
                                            {
                                                ...
                                                Notify.Send(foreachPlayer, NotifyType.Alert, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.HowToJump), 5000);
                                            }
                                            id++;
                                        }
                                    }
                                ...
                                else
                                {
                                    if (LobbyList[index].TR_Players.Keys.Count >= 1)
                                    {
                                        ...
                                        {
                                            if (!foreachPlayer.IsCharacterData()) continue;
                                            Trigger.SendChatMessage(foreachPlayer, LangFunc.GetText(LangType.De, DataName.TooNotManyPlayers));
                                        }
                                    }
                        ...
                        switch (LobbyList[index].TR_TimerTicks)
                        {
                            case 21: // 10:30
                                ...
                                lock (LobbyList[index].TR_Players.Keys)
                                {
                                    ...
                                    {
                                        if (!foreachPlayer.IsCharacterData()) continue;
                                        Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.RedCircleHp), 10000);
                                    }
                                }
                                break;
                            case 22: // 11
                            ...
                            case 40: // 20
                                ...
                                if (LobbyList[index].TR_Players.Keys.Count >= 1)
                                {
                                    ...
                                    {
                                        if (!foreachPlayer.IsCharacterData()) continue;
                                        Trigger.SendChatMessage(foreachPlayer, LangFunc.GetText(LangType.De, DataName.NoWinnerWin));
                                    }
                                }
                                EndGame(index);
                                break;
