    ...
    class Airsoft : Script
    {
        ...
        public static void RemoteEvent_airsoft_createLobby_server(ExtPlayer player, string lobby_name, int lobby_price, string lobby_password, int lobby_mode, int lobby_weapon, int lobby_map)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.lvl2forgame), 6000);
                    return;
                }*/
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Map1x1only), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LobbyNeedMoney, lobby_price), 6000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LobbyCreated), 6000);
        ...
        public static void RemoteEvent_airsoft_joinLobby_server(ExtPlayer player, int index, string password)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MatchStarted), 6000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LobbyFilled), 6000);
                    return;
                }

                if (characterData.Money < LobbyList[index].LobbyPrice)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ToEnterNeedMoney, LobbyList[index].LobbyPrice), 1500);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LobbyConnected), 6000);
                    ...
                    if (LobbyList[index].LobbyPlayers.Count == 2)
                    {
                        ...
                        foreach (ExtPlayer foreachPlayer in LobbyList[index].LobbyPlayers)
                        {
                            if (foreachPlayer.IsCharacterData())
                            {
                                ...
                                Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.MinLobbyOk), 10000);
                            }
                        }
                    }
                ...
                else if (LobbyList.ContainsKey(index) && LobbyList[index].LobbyMode != 5 && LobbyList[index].LobbyPlayers.Count >= LobbyList[index].MaxPlayers) // 1x1, 2x2, 3x3, 5x5
                {
                    ...
                    foreach (ExtPlayer foreachPlayer in LobbyList[index].LobbyPlayers)
                    {
                        if (foreachPlayer.IsCharacterData())
                        {
                            ...
                            if (LobbyList[index].LobbyMode >= 2) SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchOk, NextPlayerTeam + 1));
                            else SendTextInfoForPlayer(foreachPlayer, "Match hat begonnen");

                            SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MexitHelp));
                                ...
                                foreach (ExtPlayer foreachPlayer in LobbyList[lobbyId].LobbyPlayers)
                                {
                                    ...
                                    if (foreachCharacterData != null)
                                    {
                                        ...
                                        if (foreachPlayer.Name == GunGameWinner)
                                        {
                                            ...
                                            SendTextInfoForPlayer(foreachPlayer, $"Sie haben das Match gewonnen und bekommen {amount}$");
                                        }
                                        else
                                        {
                                            if (GunGameWinner == null)
                                            {
                                                ...
                                                SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchNoWinner, LobbyList[lobbyId].LobbyPrice));
                                            }
                                            else
                                            {
                                                SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.GungameWinner, GunGameWinner));
                                            }
                                        ...
                                        foreach (ExtPlayer foreachPlayer in LobbyList[lobbyId].LobbyPlayers)
                                        {
                                            ...
                                            if (foreachCharacterData != null)
                                            {
                                                if (winner_team == Convert.ToInt32(foreachPlayer.GetSharedData<int>("PlayerAirsoftTeam")))
                                                {
                                                    ...
                                                    SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchWinner, Convert.ToInt32(LobbyList[lobbyId].GameFinalReward * 0.9)));
                                                }
                                                else if (winner_team == -1)
                                                {
                                                    Wallet.Change(foreachPlayer, LobbyList[lobbyId].LobbyPrice);
                                                    GameLog.Money($"server", $"player({foreachCharacterData.UUID})", LobbyList[lobbyId].LobbyPrice, $"lobbyDraw(DUEL_PVP)");
                                                    SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchNichya, LobbyList[lobbyId].LobbyPrice));
                                                }
                                                else
                                                {
                                                    SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.YouLose));
                                                }
                            ...
                            foreach (ExtPlayer foreachPlayer in LobbyList[index].LobbyPlayers)
                            {
                                try
                                {
                                    if (foreachPlayer.IsCharacterData())
                                    {
                                        ...
                                        Trigger.ClientEvent(foreachPlayer, "airsoft_lobbyMenuHandler", 2);
                                        Trigger.ClientEvent(foreachPlayer, "airsoft_updateStats_client", 0);
                                        Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoPlayersMatch), 6000);
                                    }
                                }
                                catch (Exception e)
                                {
                            ...
                            try
                            {
                                ...
                                if (foreachCharacterData != null)
                                {
                                    ...
                                    if (foreachPlayer.Name == GunGameWinner)
                                    {
                                        ...
                                        Wallet.Change(foreachPlayer, amount);
                                        GameLog.Money($"server", $"player({foreachCharacterData.UUID})", amount, $"lobbyWin(GG)");
                                        SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchWinner, amount));
                                    }
                                    else
                                    {
                                        if (GunGameWinner == null)
                                        {
                                            ...
                                            SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchNoWinner, LobbyList[index].LobbyPrice));
                                        }
                                        else
                                        {
                                            SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.GungameWinner, GunGameWinner));
                            ...
                            try
                            {
                                ...
                                if (foreachCharacterData != null)
                                {
                                    ...
                                    if (winner_team == foreachPlayer.GetSharedData<int>("PlayerAirsoftTeam"))
                                    {
                                        ...
                                        if (LobbyList[index].LobbyMode == 1) SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchWinner, Convert.ToInt32(LobbyList[index].GameFinalReward * 0.9)));
                                        else SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.YouTeamWin, Convert.ToInt32(LobbyList[index].GameFinalReward * 0.9)));
                                    }
                                    else if (winner_team == -1)
                                    {
                                        Wallet.Change(foreachPlayer, LobbyList[index].LobbyPrice);
                                        GameLog.Money($"server", $"player({foreachCharacterData.UUID})", LobbyList[index].LobbyPrice, $"lobbyDraw(TPVP)");
                                        SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchNichya, LobbyList[index].LobbyPrice));
                                    }
                                    else
                                    {
                                        if (LobbyList[index].LobbyMode == 1) SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.YouLose));
                                        else SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.YouTeamLose));
                                    }
        ...
        public static void CMD_mexit(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.ErrorMatch), 6000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void LeaveLobbyFunction(ExtPlayer player)
        {
            try
            {
                ...
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId))
                {
                                    ...
                                    foreach (ExtPlayer foreachPlayer in LobbyList[lobbyId].LobbyPlayers)
                                    {
                                        ...
                                        if (foreachCharacterData != null && foreachPlayer != player)
                                        {
                                            ...
                                            if (winner_team == foreachPlayer.GetSharedData<int>("PlayerAirsoftTeam"))
                                            {
                                                ...
                                                if (LobbyList[lobbyId].LobbyMode == 0 || LobbyList[lobbyId].LobbyMode == 1) SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.MatchWinner, Convert.ToInt32(LobbyList[lobbyId].GameFinalReward * 0.9)));
                                                else SendTextInfoForPlayer(foreachPlayer, LangFunc.GetText(LangType.De, DataName.YouTeamWin, Convert.ToInt32(LobbyList[lobbyId].GameFinalReward * 0.9)));
                                            }
                    ...
                    if (player.GetSharedData<int>("PlayerAirsoftTeam") == 0 || player.GetSharedData<int>("PlayerAirsoftTeam") == 1 || AirsoftPlayerData.ContainsKey(player) && AirsoftPlayerData[player].IsGunGamePlayer == true) Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LeftMatch), 6000);
                    else Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.LeftLobby), 6000);
