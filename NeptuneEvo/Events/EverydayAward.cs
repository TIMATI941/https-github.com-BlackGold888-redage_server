        ...
        */
        ...
        public void CMD_RefresEverydayAward(ExtPlayer player)
        {
            try
            {
                ...
                UpdateEverydayAwardItems(IsStart: true);
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Aufgaben erneuert", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void OnTakeDay(ExtPlayer player, int slotId)
        {
            ...
            if (!Convert.ToBoolean(accountData.ReceivedAward[slotId]))
            {
                if (characterData.Time.TodayTime >= GameTime)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Everyday6h), 6000);
            }
            else 
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.AlreadyEveryDayGet), 6000);
        }
        ...
        public static void OnTakeDonate(ExtPlayer player, int slotId)
        {
            ...
            if (!Convert.ToBoolean(accountData.ReceivedAward[slotId]))
            {
                if (accountData.ReceivedAwardDonate >= donateData)
                {
                    ...
                    OnDonateList(player);
                    Accounts.Save.Repository.SaveReceived(player);
                    Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.YouGetPopBonus, MoneySystem.Wallet.Format(donateData)), DateTime.Now);
                }
                else 
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Everyday6h), 6000);
            }
        ...
        public static void LoadEverydayAwardInfo(ExtPlayer player, bool isInit = false)
        {
            try
            {
                if (!FunctionsAccess.IsWorking("everydayaward"))
                {
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.FunctionOffByAdmins), 6000);
                    return;
                }
                var accountData = player.GetAccountData();
        ...
        public static void TakeDayAward(ExtPlayer player)
        {
            try
            {
                ...
                if (!Convert.ToBoolean(accountData.ReceivedAward[SlotId]))
                {
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.Everyday6h), 6000);
                }
                if (!Convert.ToBoolean(accountData.ReceivedAward[WeekBonusSlotId]))
                {
                    if (characterData.Time.WeekTime >= WeekGameTime)
                ...
                foreach (var donateData in DonateToBonus)
                {
                    ...
                    if (accountData.ReceivedAward[day] == 0 && accountData.ReceivedAwardDonate >= donateData.Value)
                    {
                        ...
                        isUpdate = true;
                        //Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.YouGetPopBonus, MoneySystem.Wallet.Format(donateData.Value)), 10000);
                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.RedAge, LangFunc.GetText(LangType.De, DataName.YouGetPopBonus, MoneySystem.Wallet.Format(donateData.Value)), DateTime.Now);
                        break;
                    }                    
                }
            ...
            try
            {
                ...
                switch (award.Type)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        UpdateData.RedBucks(player, award.Count, msg: LangFunc.GetText(LangType.De, DataName.EverydayOnlineLog));
