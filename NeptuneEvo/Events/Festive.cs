...
namespace NeptuneEvo.Events
{
    ...
    class Festive : Script
    {
        ...
        public static void Init ()
        {
            if (isEvent)
            {
                PedSystem.Repository.CreateQuest("dedmoroz", new Vector3(-1640.6166, -1096.9696, 13.024864), -42f, title: "~y~NPC~w~ Santa Klaus\nGeschenke", colShapeEnums: ColShapeEnums.Festive);
                Main.CreateBlip(new Main.BlipData(607, "Santa Klaus", new Vector3(-1640.6166, -1096.9696, 13.024864), 0, true));
        ...
        public static void OnFestive(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openInput", $"Tausch {EventCoinsName} | 3 = 1 RB", "Geben Sie Zahlen ein: Beispiel: 3, 6, 9 usw.", 3, "sell_festive");
            }
            catch (Exception e)
            {
        ...
        public static void EventsCollect(ExtPlayer player, int index)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du hast dieses Päckchen bereits gefunden. Finde andere!", 8000);
                    return;
                }
        ...
        public static void CMD_givehc(ExtPlayer player, int id, int amount)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast {target.Name} {amount} Geschenke gegeben", 5000);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(player.Position, (float)radius, UpdateData.GetPlayerDimension(player)))
                {
                    ...
                    if (foreachPlayer.Value != player.Value)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.De, DataName.NoSpaceInventory), 5000);
                            return;
                        }
                        //GameLog.Money($"player({characterData.UUID})", $"player({targetCharacterData.UUID})", amount, "admin");
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast allen im Radius von {radius} {amount} Geschenke verteilt", 6000);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (index)
                {
                    case 0:
                        if (99999 > count)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Du hast nicht soviele Geschenke", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast deine Geschenke gegen ein Weihnachtshose eingetauscht", 6000);
                        break;
                    case 1:
                        if (650 > count)
                        {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast deine Geschenke gegen ein Weihnachts-Oberteil eingetauscht", 6000);
                        break;
                    case 2:
                        if (1000 > count)
                        {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast deine Geschenke gegen Schmuck eingetauscht", 6000);
                        break;
                    case 3:
                        if (650 > count)
                        {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast deine Geschenke gegen eine Weihnachtshose eingetauscht", 6000);
                        break;
                    case 4:
                        if (650 > count)
                        {
                    ...
                    case 6:
                        if (1500 > count)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Sie haben nicht genug Münzen", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Du hast deine Geschenke gegen ein Schneemobil eingetauscht!", 6000);
                        break;
                    default:
                        return;
                }
